...
V0.16.60 turns Scion's measured quality gap into a reproducible teaching loop. A source-only teacher receives the immutable instructor facts, anonymous failure diagnoses, and an exact per-case schema in a fresh read-only session. The schema pins the lesson id, original artifact hash, and every numbered fact; the current compiler then decides whether the candidate is safe enough to reach anonymous A/B plus B/A judgment. This creates training evidence from diagnosed failures without letting the teacher see unrelated course material or rewrite the acceptance gate.

The compiler itself also became more precise. It no longer mistakes a rejected inverse statement—such as explaining that “lower is younger” reverses a geology relationship—for affirmative support of that wrong answer. When support is lexical rather than exact, relevant tokens must appear in semantic order, so scattered words about gross investment, capital stock, and change cannot manufacture a second supported option. A true explanation that endorses two choices remains rejected by the regression ruler. Because this boundary is model-neutral, compatible paid-model output benefits from the same correction.

The first repair wave converted five previously rejected paid-reference winners into five compiler-admitted, stable, score-qualified preferences with zero winner critical defects. A balanced second wave selected fourteen unseen lessons—two in each of seven domains, fourteen distinct course groups and source kernels, and all nine diagnosed failure families. Seven qualified directly and six qualified after targeted source-only repair. One User Experience Design pair changed preference when presentation order reversed and remains quarantined. Scion therefore gains thirteen—not fourteen—new expansion rows.

Together with the two V0.16.59 rows, the current corpus contains **20 unique source-ledger full-lesson preferences across all seven training domains**, or 20% of the minimum 100-row gate. Each accepted row preserves exact source facts, passes the current compiler, wins both anonymous presentation orders, clears the score floor, and has no winner critical defect. Reference capture can now run four isolated sessions concurrently while serialized atomic checkpoints keep a partially completed campaign resumable; the shared browser-local model stays serial because it owns one runtime.

This is meaningful compiler and training-data progress, not a model-weight win. The pinned public Gemma base admitted only 1/14 raw expansion artifacts, while the paid reference admitted 8/14 before repair. No adapter has been trained on the new rows, no adapter is active in the website, no Gemma weight changed, and no candidate has faced the frozen V10 held-out benchmark. Teacher and judge ran in separate cleanrooms but used the same GPT-5.6-sol identity, so this is neither human review nor independent-model validation. The production path remains public Gemma 4 E2B plus the Scion compiler until a small downloadable adapter earns a multi-domain held-out win without worse repair, download, memory, or runtime burden.

The release also repeats the full browser experience instead of treating evaluation work as separate from the product. Setup preview now preserves meaningful title connectors, so **Spanish for Healthcare Professionals** and **Writing with AI** survive intact. The deep grader accepts `min`, `mins`, `minute`, and `minutes` in exported lesson outlines instead of falsely converting visible timing rows to a zero-minute P0. The repaired desktop and 390×844 flows cover full-screen preview, current-tab XLSX/PDF/CSV/DOCX/PPTX, Google authentication failure copy, selected-material ZIP contents, lesson scope, and `.coursemapper` backup.

GitHub Fast verification run `29694477954` passed formatting, lint, and release-history checks before correctly stopping on a stale source-compiler replay receipt. V0.16.60 rebuilds that receipt from the unchanged retained responses: the result remains 121 admitted atoms, 71 retry seats, 10 source-grounded key repairs, and zero response mutations. The grader fix changes a transitive evaluation input, so V10 preserves the same five disjoint held-out courses and task route while binding grader 1.10.20 and its twelve-file implementation receipt. V9 remains historical; no earlier score or promotion decision carries forward.
