...
The v0.16.48 release was replayed frame by frame in the real in-app browser with the browser-local Gemma runtime, not a mocked provider. A difficult one-lesson Bayesian prompt drove the local lesson kernel through its bounded retries and handled fallback, then completed all five selected materials in 109 seconds at 89/B and texture 90. The observed progress remained monotonic through 16 → 38 → 41 → 46 → 100, named the current local attempt, and reserved 100% for the verified package.

That replay caught defects a final screenshot could not. Nested dark-mode backdrop filters intermittently painted giant black Chromium GPU tiles during transitions; the dark workspace now uses a stable glass paint path, and six captures sampled at 250 ms intervals were byte-identical. The completed build duration also has its own clock, so a later Scion Agent or image call cannot turn “Ready in 109s” into a larger number.

The handled model failure exposed a content boundary too. The previous deterministic quiz used course-process language in 64% of its stems and options. The repaired fallback recognizes Bayesian inference and decision analysis and asks six subject questions about priors, likelihood ratios, posterior odds, weak evidence, and product experiments; its deterministic process-language share is 0%, answer rationales diagnose real misconceptions, and Bloom labels follow the work named in each stem. Those frames are an independently cached workspace-only data chunk, keeping the compiler implementation below its existing raw-size ratchet and adding nothing to the landing download. Browser-local Agent replies now unwrap their shared `chatReply` envelope before display, so Scion answers as prose instead of showing raw JSON.

V0.16.48 also strengthens model-neutral semantic admission. On the frozen seventy-eight-loss replay, the source-strict gate catches sixty-four failures instead of fifty, including twenty-three of thirty-four weak key terms instead of nine, with zero preferred-artifact regressions. Across ninety-one unjudged cross-domain pairs, the stronger rule adds eight retry seats to local Scion and two to GPT-5.4-mini. Both model paths benefit from the compiler; outputs with more source-grounding defects benefit more.
