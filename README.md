...
The independent-review campaign freezes 160 of those candidates—40 each in Computer Science, Geology, Music Theory, and User Experience Design. Each randomized A/B case, the complete packet, the source candidate ledger, and the held-out benchmark are hash-bound. Two domain-qualified working instructors must agree and clear factual and teachability floors before any case becomes a training preference. There are currently zero completed reviews. A new `research-ready` tier permits a strictly non-promotable adapter only after at least 100 blind-instructor-approved pairs, with at least 20 approved pairs in each of four domains and three separate course groups per domain. Public candidate requirements remain 3,000 pairs across five domains and fifteen groups, with at least 20 of the 100 blind-instructor-approved pairs in each qualifying domain, plus every promotion gate.
...
`npm run audit:scion:review-packet` first verifies all source-capture projects, then derives only exact-input neutral pairs from matched artifacts. The current 372-candidate ledger produces 160 anonymized A/B cases, balanced 40 each across computer science, geology, music theory, and user-experience design and three exact course groups per domain. Sixty-three selected cases carry a neutral, hash-bound source context that the offline reviewer shows above both anonymous candidates. World Literature is excluded because its retained model runs used different prompts and because it is part of the frozen held-out ruler. Every domain packet includes a self-contained offline review page that saves drafts locally and downloads digest-bound JSON without sending data to a server or revealing the organizer key. Two distinct working instructors who currently teach that domain must independently agree, attest no conflict of interest, and clear factual-correctness and teachability floors before `audit:scion:reviews` can write an approved row. Research training additionally requires at least 20 approved instructor pairs and three isolated course groups in each of four domains, and can create only a non-promotable `research` adapter. Candidate and production training still refuse to start below 3,000 verified pairs across five domains and fifteen groups, with instructor evidence distributed across all five qualifying domains. See [docs/SCION_NEXT_LEVEL_PLAN.md](docs/SCION_NEXT_LEVEL_PLAN.md) for the verified-learning roadmap and promotion gates.
...
v0.16.13 makes that protocol the required quality ruler for Scion promotion. The 13-case corpus and nine-dimension, 26-criterion rubric pass structural audit, while held-out validation remains locked and red without an explicit access record and qualified evidence. A Scion adapter/base comparison must predeclare every case, use distinct repeated trials with exact arm identities, byte-verify every scorecard against the source and output, retain failures/cost/latency, bind two independent domain-matched instructors to every blind pair, and report compiler burden separately. Promotion additionally requires a positive held-out score interval in each of five domains, a blind-preference Wilson lower bound above 0.5, and a strictly lower compiler-call interval. Legacy 99/A deterministic grades and hand-shaped win counts cannot satisfy this gate.
