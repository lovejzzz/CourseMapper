...
V0.16.49 was replayed frame by frame in the real in-app browser across resume, restore, lesson regeneration, refresh recovery, every selected deliverable, slide visuals, concept maps, Agent revision, dark mode, finalization, grading, and export. For repeatable interruption and UI timing, this release replay used a local provider-compatible deterministic transport fixture while exercising the production compiler, persistence, finalizer, grader, and ZIP exporter. It is workflow evidence, not a new public-Gemma inference-speed claim.

That replay caught a defect a polished final screen could hide: refreshing during lesson regeneration could restore a saved entry as permanently streaming and leave the workspace at 8/9 ready. Restore now promotes saved content to done, returns empty interrupted content to idle, clears the abandoned regeneration marker, and preserves the exact enriched lesson IDs used by final grading. Manual Scion regeneration stays on the deterministic compiler path, so it completes without accidentally entering a second provider-streaming loop.

The same audit refined the whole workspace. Ready and Ready with notes no longer share warning colors; the header's duplicate Download ZIP action is gone; the package panel owns the single export action; native slide visuals and concept-map frames render in context; and the Scion Agent can execute a bounded “Improve slides” action without printing raw pseudo-tool JSON. The Living Course Compiler still advances only from observed Model → Map → Enrich → Compile → Verify → Grade state and reserves 100% for a terminal ready or review outcome.

The bounded audit course was a two-lesson undergraduate music-theory package. Interval classification and inversion now receive distinct lesson plans, slides, practice, discussions, FAQs, study terms, assessment frames, rubrics, course-map actions, and observable evidence instead of renamed generic prose. A source-trust boundary quarantines attractive title-level false friends and retains two exact Open Music Theory interval resources with complete sourceRef coverage across outcomes, activities, examples, assessments, rubric criteria, and factual claims.

After content variation, the texture metric was also corrected to ignore only structural export chrome—question metadata, document titles, and page footers—while still scoring learner-facing prose. The live package reached 100/A with texture 98 and zero findings. Its exact twenty-one-entry ZIP passed integrity and expected-folder checks, then independently regraded from nineteen extracted files at 100/A with zero P0, P1, or P2 findings. That proves this bounded package; it does not claim that every prompt or domain will receive the same score.

V0.16.49 advances model-neutral semantic admission again. Judge-informed source-strict V3 detects 68 of 78 frozen stable losses—four more than V0.16.48—with zero preferred-artifact regressions. On 192 retained local response seats and both arms of the 91-row frozen comparison, its additional issue signals overlap artifacts already rejected by older rules, so measured retry burden does not increase. The shared compiler benefits Scion and user-selected paid providers because source checks, schema repair, compilation, grading, and export sit after model generation; model-specific prompts and retries can still produce different gains. Scion additionally gets the browser-local runtime, compact-kernel contract, deterministic recovery, and versioned product experience.
...
No v0.16.49 adapter is active on the hosted website. The reproducible research run produced one validation-selected adapter, but the diagnostic held-out comparison rejected promotion and exposed additional compiler work. Scion therefore remains the pinned public Gemma base plus the model-neutral compiler. A future candidate must beat base-only Scion on the complete implementation-bound v2 holdout before it can be activated, and a loss remains evidence for compiler or data improvement—not a hidden promotion.
