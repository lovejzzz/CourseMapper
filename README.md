...
Each domain now has two deliberately separate offline review lanes over those same cases. `review.html` is only for a working instructor who currently teaches the domain; two distinct instructors must independently agree, attest no conflict, and clear factual-correctness and teachability floors before `audit:scion:reviews` can write an approved row. `founder-review.html` is the solo-founder research lane: it shows one case at a time with progress, jumps, flags, and local autosave, then exports `founder-review` evidence marked non-independent, conflict-declared, and claim-ineligible. The instructor validator rejects that export. Founder judgment can expose answer-key defects, contradictions, ambiguity, weak distractors, unsupported generalization, and overclaim for compiler repair or a non-promotable research experiment, but it cannot enter the approved production corpus or promote an adapter.

Research training additionally requires at least 20 approved instructor pairs and three isolated course groups in each of four domains, and can create only a non-promotable `research` adapter. Candidate and production training still refuse to start below 3,000 verified pairs across five domains and fifteen groups, with instructor evidence distributed across all five qualifying domains. See [docs/SCION_NEXT_LEVEL_PLAN.md](docs/SCION_NEXT_LEVEL_PLAN.md) for the verified-learning roadmap and promotion gates.
