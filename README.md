...
**Course Mapper is free—and will stay free. There is no paid tier, subscription, or concierge upsell.**
...
```text
explicit lesson sequence + exact evidence → Scion compiler → Scion Vx
ambiguous or incomplete brief → public Gemma 4 E2B base + optional Scion adapter → Scion compiler → Scion Vx
```
...
the complete finding set through one shared policy, so a P0 cannot become
downloadable because a P1 was inserted first or because display truncation hid
...
therefore remain visible alongside review notes, and the component fixture now
matches the real finalizer shape.
...
V0.16.99 turns V0.16.98’s causal measurement into a measured generated-course quality repair. Sparse briefs now use course-aware teaching lenses and lesson context instead of relying on a small generic sentence inventory.

On the frozen 12-course thin panel, input-mask/path-free repeated clusters fall from **536 to 502**, pair-local K=2 clusters from **195 to 181**, reader exposure from **9.78% to 9.01%**, and cross-package excess from **6.75% to 6.23%**. The strong-source gold panel also holds: reader exposure is **18.87%**, cross-package excess is **12.93%**, no retained cluster grows, and no new universal high-salience frame appears. Both panels pass the V0.16.98 comparator.

The most important changes are semantic, not cosmetic:

- unclassified courses can no longer borrow an unrelated Curriculum Genome shard;
- Database Systems distinguishes transactions from enterprise BTM, data integrity from moral integrity, and relational algebra from mathematics answer-check language;
- Community Oral History uses interviews, narrator context, consent, recordings, transcripts, archives, and public-history decisions instead of Western-civilization survey, UX prototype, or spoken-language rubric fallbacks;
- exact compact subsets of a larger verified evidence ledger retain source receipts, while paraphrases and added facts still fail closed;
- current-source title families prioritize canonical disciplinary pages before ambiguous lesson-title fragments;
- research cache V19 removes previously admitted weak candidates.

The browser proof uses fresh sparse briefs, real current-source requests, and physical ZIP downloads:

- **Database Systems:** ready in **35 seconds**, 8/8 grounded lessons, 9/9 material families, **65/100 Automated Readiness**, **100/100 evidence grounding**, **99/A package conformance**, zero encoded findings, 22 trusted source receipts, and complete source-reference coverage across outcomes, activities, examples, assessments, rubric criteria, and factual claims.
- **Community Oral History Methods:** on the identical brief, ready time improves **48→29 seconds**, Automated Readiness **58→62**, evidence grounding **61→84**, specificity **97→98**, and texture **95→96**. The final 51-file ZIP has zero encoded findings and none of the observed Western-civilization, wastewater, UX prototype/portfolio, pronunciation, or fluency false friends.

The oral-history run is especially important because its first package looked structurally green while being semantically wrong. V0.16.99 fixes the owners that created those errors and adds regression tests for each collision instead of suppressing the warning or inflating the score.

This is a shared compiler and evidence-routing improvement, so compatible paid providers also benefit from the discipline repair, scenario framing, source admission, assessment classification, and export path. Scion additionally owns the free browser-local model/runtime and its evidence-before-inference route. Gemma weights remain unchanged, the optional adapter remains inactive, and the release does not claim factual certification, instructor approval, accessibility certification, classroom outcomes, or paid-model superiority.

The exact measurements, browser runs, source reports, remaining evidence boundary, and reproduction commands are documented in [docs/SCION_V01699_DISCIPLINE_SAFE_SPARSE_BRIEFS.md](docs/SCION_V01699_DISCIPLINE_SAFE_SPARSE_BRIEFS.md).

### V0.16.98 historical release — causal texture, enforced ratchets

V0.16.98 turns V0.16.97’s useful evaluator into an honest merge gate. The generated report now shows total repeated clusters, the K=2 pair-local bucket, the complete support distribution, exact compiler-frame coverage, the consumed-slot causal view, and its divergence from broad input masking.

The comparator now uses two explicit references. The immutable pre-repair baseline still governs the broad rates and universal-cluster improvement. The retained V0.16.97 V8 snapshot freezes the repaired support shape, so a change fails if K=2 rises, an existing cluster gains package support or occurrences, a new universal high-salience frame appears, or a visible path goes unclassified. This avoids the dishonest alternative of comparing K=2 to the older baseline, which would make the already-shipped repaired state fail immediately.

Opt-in finalized-string receipts raise causal teaching-prose coverage from **3.7% to 90.01%** on the 12-course thin panel and from **1.3% to 92.13%** on the 10-course gold panel. Complete trace-on and trace-off material packages remain byte-identical in ordinary JSON; the website does not enable these receipts during normal generation.

The causal result changes the next quality decision:

- thin-panel reader exposure is **38.47%** when only source slots the compiler actually consumed are masked, versus **9.78%** under broad input masking—a **28.70-point** gap;
- gold-panel exposure is **19.40%** versus **19.32%**, a **0.08-point** gap.

That thin-panel result is evidence of generic fallback pressure under sparse briefs. It is not a real-production repetition rate, and V0.16.98 does not claim that wording quality improved. It gives the next repair a causal target instead of rewarding a larger template inventory.

Fast verification now recompiles both retained panels and applies the real comparator before merge. The repository ratchet also counts `courseBlueprintCompiler.js` and every `courseCompiler*.js` leaf as one compiler family, so moving code into a cacheable module cannot register as a false reduction. The exact implementation, measured results, and remaining production-panel work are documented in [docs/SCION_V01698_CAUSAL_TEXTURE_ENFORCEMENT.md](docs/SCION_V01698_CAUSAL_TEXTURE_ENFORCEMENT.md).

### V0.16.97 historical release — measured texture, receipted realization
...
V0.16.96 makes the coverage forecast authoritative. Whenever Scion finds unsupported lessons and research is still off, it offers the same explicit opt-in source action on every device. The copy names the providers and explains that only the course title and uncovered lesson topics leave the device; verified passages are cached locally and supplied to the browser-local writer. Private mode remains available, and device capability still chooses whether the authoring step uses Gemma or the exact-source compiler.

The expanded Scion card now leads with the durable product promise—**Scion is fully free, and always will be**—instead of describing the underlying Gemma/compiler composition in the primary setup journey. The technical boundary remains documented here for users who want it.

The evidence itself now keeps its identity through the full pipeline. Provider, topic, source tier, concept links, revision metadata, source kind, and support receipts survive normalization. A researched ledger can displace an older genome kernel only when the candidate copies its admitted facts exactly; stale partial overlays cannot overwrite current research. The official W3C/WAI catalog routes WCAG, structure, forms, and evaluation sources by lesson family, tolerates an individual page fetch failure, and requires actual evaluation-method evidence when a lesson promises audit, testing, or remediation.

The Agent, `PACKAGE_MANIFEST.json`, and `SOURCE_REPORT.md` now read the complete exact evidence overlay before any compact classroom-resource projection. In the release acceptance course, all three Lesson 4 receipts—Evaluating web accessibility, Easy Checks, and WCAG-EM—remain available with distinct use boundaries. Package-review notes retain exact grader findings and affected evidence instead of reducing them to a generic warning. Research cache V17 and lesson-kernel contract V11 prevent older, weaker evidence projections from silently returning.

One deterministic language defect found in real output is fixed for every compatible provider: evidence, feedback, and instructor routines now use a valid imperative-first form such as “Trace whose evidence is represented during Urban biodiversity surveys,” instead of the malformed “During Urban biodiversity surveys trace…”. Unfinished-sounding “draft” labels found in generated instructional surfaces were replaced with submission/current-work language.

The exact browser acceptance completed the four-lesson Digital Accessibility course in 4 seconds on the warm exact-source route with 4/4 knowledge kernels, 9/9 material families, 69/100 Automated Readiness, 99/A package conformance, texture 96, zero encoded findings, one ZIP action, and no inspected application-console errors or warnings. The 689,702-byte physical ZIP and every nested Office container passed integrity; its exported ledger retained 14 source receipts and complete source-reference coverage. These are automated route and artifact results—not factual, accessibility, teaching, or classroom certification.

This release improves Scion's research trigger, source adequacy, fact/citation continuity, Agent and export evidence, compiler language, and review-action clarity. It does **not** change Gemma weights, activate the optional adapter, or prove instructor approval, classroom outcomes, factual certification, accessibility certification, or superiority over a paid model. The implementation and release boundary are documented in [docs/SCION_V01696_EVIDENCE_FIRST_END_TO_END.md](docs/SCION_V01696_EVIDENCE_FIRST_END_TO_END.md).

### V0.16.95 historical release — the Agent reads the evidence that ships
...
V0.16.94 makes persistence presentation follow the complete Living Course Compiler workflow instead of only its initial generation phase. Generation, Smart Sync, the sync-to-regrade handoff, verification, and grading now share one in-flight boundary. A recoverable local write reads **Saving locally…** with a neutral treatment while any of those phases is active. This does not hide durable data loss: if the compiler settles and the local error remains, **Local save failed** still appears. Cloud failures remain immediate because local compiler work cannot repair a remote write.

The same replay also caught a completed-state contradiction: the header synchronized all nine materials while the Agent retained the old **9 Deliverables Need Syncing** card with live **Sync All** and **Skip** actions. The header now approves the durable Agent suggestion through the shared sync router, so one action both executes the plan and makes its card terminal. Direct regeneration remains available for restored stale workspaces that have no Agent suggestion.

Focused tests pin active local recovery, settled local failure, immediate cloud failure, and one-owner sync completion. The required deployed-origin proof replays a real Course Map edit and **Sync all stale** frame by frame, rejects every false red persistence frame and stale action card, then verifies Resume, Agent grounding, a clean console, and a physical ZIP. This patch changes save-status truth and sync-action completion only; it does not change Scion output, Gemma weights, the inactive adapter, evidence research, source admission, readiness scoring, or model routing. The release contract is documented in [docs/SCION_V01694_WORKFLOW_SAVE_TRUTH.md](docs/SCION_V01694_WORKFLOW_SAVE_TRUTH.md).

### V0.16.93 historical release — exact resume without a pointer
...
V0.16.92 gives a current-attempt storage failure a bounded five-second confirmation window. A follow-on save that starts during that window invalidates the pending red state; if no newer attempt arrives, a permanent failure still surfaces honestly and returns to the existing idle treatment after five seconds. This is not cosmetic suppression: Scion keeps the previous exact snapshot safe, continues the serialized persistence queue, and shows red only after the browser has failed to recover within the confirmation boundary.

Unit tests prove both branches: a newer save suppresses the transient verdict, while a permanent current-attempt failure becomes visible after exactly five seconds. The quota-saturated Chromium test now performs a real Course Map edit, observes beyond the three-second autosave debounce and five-second confirmation window, verifies that no failure frame appeared, checks **Autosaved locally**, reloads, and restores the revised title. The complete release and deployed-origin proof are documented in [docs/SCION_V01692_CONFIRMED_AUTOSAVE.md](docs/SCION_V01692_CONFIRMED_AUTOSAVE.md).

This patch changes browser persistence status arbitration only. It does not change Gemma weights, the inactive adapter, evidence research, source admission, compiler output, readiness scoring, or the zero-model course route.

### V0.16.91 historical release — grounded teaching surfaces and calm browser state
...
V0.16.91 attached every asynchronous browser save to a monotonically increasing attempt identity. Only the newest attempt could paint saved, failed, or idle; a new save cleared the older timer, and a workspace reset invalidated every queued callback. Exact IndexedDB writes also retried once after a transient transaction abort. Production later showed that a current attempt could still fail transiently before a follow-on exact snapshot recovered, which is the confirmation gap closed by V0.16.92.
...
V0.16.89 passed its required production audit with the exact four-lesson Digital Accessibility brief, 4/4 evidence kernels, 9/9 material families, all ten public surfaces, zero findings, 69/100 Automated Readiness, 99/A package conformance, texture 97, and a physically valid ZIP. Its exact evidence enrichment no longer activated the browser runtime. But the run still took **32 seconds** and made **two cached-Gemma calls** because Scion asked the model to restate the instructor’s already explicit ordered lesson sequence and later sampled a cosmetic voice rewrite.

V0.16.90 resolves that structure contract before importing Wllama, opening model storage, loading the 3.35 GB public base, preparing an adapter route, or calling inference. A strict exact-count brief with a complete named sequence becomes a typed course skeleton through `scion-compiler-explicit-sequence-route-v1`. The compiler preserves the instructor’s title, order, and count; a missing, mismatched, or ambiguous sequence fails closed to the existing Gemma path. This is structural compilation, not fixed course copy. The provisional workspace identity uses the same contract boundary, so an instruction such as “— create exactly 4 lessons” never appears as part of the course title during the opening animation frame.

When that route meets V0.16.89’s `scion-compiler-exact-source-route-v1`, Scion can own the whole observed course-map and knowledge-enrichment path without a model. V0.16.90 also skips the sampled voice pass on this specific zero-download route so a cosmetic rewrite cannot silently reactivate Gemma. Compiler projections no longer manufacture estimated token usage or masquerade as provider responses: the Living Course Compiler digest and exported manifest report **zero model starts, zero model calls, zero tokens, zero downloaded weights, and zero rewrite cost**.

A fresh final local browser replay reached full ready in **25 seconds** on its first run and **16 seconds** with the evidence cache warm, with no model activation frame or browser-runtime warning. The corrected title was exact in the first captured build frame at 387 ms and stayed exact through completion. The run preserved the same four named lessons, 4/4 kernels, 9/9 materials, 69/100 readiness, 99/A conformance, texture 97, 38/38 export checks, six trusted sources, and complete 48/48 source-reference coverage. The downloaded 47-entry ZIP and all 34 nested Office containers passed integrity checks. These are observed route and package results; network/cache conditions differ from the V0.16.89 production baseline, so they are not presented as a perfectly matched cold-latency experiment.
...
V0.16.89 moved exact evidence resolution ahead of browser-runtime import and gave the compiler-owned projection its own `scion-compiler-exact-source-route-v1` receipt. Its production audit proved the evidence route did not activate Gemma, but also revealed that explicit course-map restatement and the sampled voice pass still required two cached-Gemma calls. V0.16.90 removes those remaining model boundaries only when strict compiler contracts fully cover the requested work.

The complete V0.16.89 production proof is preserved in [docs/SCION_V01689_EVIDENCE_BEFORE_WEIGHTS.md](docs/SCION_V01689_EVIDENCE_BEFORE_WEIGHTS.md).

### V0.16.88 historical production proof — consent and named topics survive the handoff

The first live V0.16.87 professor-acceptance run was intentionally rejected even though its archive checks passed. The Landing action said **Use current sources & generate**, but its consent depended on persistent browser storage. In the clean-origin audit session, the lazy Landing → AppFlow transition entered private mode, prepared only **1/4** lesson kernels, and honestly scored **53/100** Automated Readiness. Green export structure was not allowed to conceal weak course content.
...
A fresh production replay forecasts one locally supported lesson and three source gaps, discloses the bounded public-source route, and follows it after one explicit click. Scion admits **4/4** source ledgers, maps **4/4** lessons, compiles **9/9** material families, and reaches **0 blockers and 0 warnings**. Automated Readiness is **69/100** under the unchanged automation-only ceiling; package conformance is separately **99/A**, texture is **97**, and the archive verifier passes **38/38** checks. Its 35-file ZIP contains six trusted concept-linked sources and complete **48/48** source-reference coverage across outcomes, activities, examples, assessments, rubric criteria, and factual claims. The archive passes `unzip -t` and offline regrading with SHA-256 `be9c3206ed19de45e089aa813d154a8cad64e21484e7979967cb4d050f2a0bed`. The run uses zero model downloads, zero model inference, zero model tokens, and `$0.000` rewrite cost on the observed adapter-less device.

The frame-by-frame audit found one final honesty defect even though the package itself was green: the ribbon said **Ready in 10s** when generation stopped, while export verification and grading kept the ZIP unavailable until about **20 seconds**. The completed timestamp now belongs to the immutable package-quality result after checking, grading, and export preparation. Later Agent chat cannot make it grow. The displayed duration therefore covers the whole professor-facing workflow, not only its generation half.
...
The production browser audit inspected every Living Course Compiler stage and all ten public surfaces: Course Map, Syllabus, Lesson Plans, Slide Decks, Assignment Briefs, Rubrics, Discussion Prompts, Quiz & Exam Bank, Study Guides, and Course FAQ. The Scion Agent named the assigned W3C Accessible Forms and Labels sources, connected Lesson 3 evidence to Lesson 4 testing and remediation, and preserved the boundary that one passing component check does not prove whole-product conformance. Desktop, tablet, 390×844 mobile, light mode, and dark mode remained usable; the workspace exposed one ZIP action; and the outer archive plus all 34 nested Office containers passed integrity checks. The console contained no application errors, but five low-level Wllama activation warnings exposed the needless cached-runtime boundary corrected in V0.16.89. These are production workflow and package observations, not instructor approval or classroom-effectiveness evidence.
...
The complete local proof passes **5,879 active unit tests across 470 passing files**, with 16 files and 162 tests intentionally skipped; the **151/151 Chromium suite**; the **40/40 layered evaluation**; the **18/40 PR compiler contract profile**; **40/40 physical gold packages** with zero blockers or warnings; **6/6 Firestore rules tests**; the headless CurriculumOS source-provenance proof; format, lint, build, bundle, constitution, and release-history audits; and the generated-runtime digest check. The evaluation claim remains `compiler-contract-only`: these checks establish encoded behavior and regression coverage, not instructor approval or classroom effectiveness.
...
Historical V1 remains in [`evaluation/automated-readiness/v1/cases.json`](evaluation/automated-readiness/v1/cases.json) with its original score windows. The current exact-score audit uses the immutable successor in [`evaluation/automated-readiness/v2/cases.json`](evaluation/automated-readiness/v2/cases.json); run `npm run audit:automated-readiness` to verify its scores, ordering, ceiling, claim boundary, and conformance separation.
