...
The same run identifies the next quality ceiling instead of hiding it: 54 assessment items still use conservative, source-review-required compiler recovery; only 26% of multiple-choice stems meet the applied-evidence depth target; and one lesson lacks a visible Hanzi–Pinyin pair across all learner surfaces. An attempted whole-kernel pairing gate regressed coverage to 5/15 and was rejected before release. Pair visibility will be repaired at the compiler projection layer; it will not be allowed to discard otherwise-admitted lesson knowledge.
