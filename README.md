...
The frozen adapter benchmark then exposed a deeper reason the compiler must remain part of Scion. Its first Mandarin package was structurally exportable and received 89/B from the previous deterministic grader, yet a semantic read found Korean greetings, Hangul, native Korean and Sino-Korean number systems, and Korean question syntax inside the Mandarin materials. The frozen course request itself contains none of those concepts. The Living Course Compiler now treats that cross-language teaching leakage as a course-identity violation at three boundaries: the compact lesson kernel is rejected before projection, native course-map objectives and activities from the same response are quarantined, and an independent final package check records a P0 blocker if any leakage survives. On the real captured package, the strengthened grader changes 89/B to 74/C with the exact offending text quoted. The live ribbon calmly reports “Protecting course identity” while the bounded recovery runs. A bibliography phrase such as “Mandarin tones for Korean speakers” remains allowed, and a course explicitly naming both Mandarin and Korean remains comparative by design.

That result does **not** yet prove the research adapter caused the confusion. The adapter arm is still running and must be compared with the identical pinned base-only arm before attribution. It already proves that a valid ZIP and a polished numeric score are insufficient without semantic identity checks—and that benchmark failures are valuable compiler evidence even when the adapter itself ultimately loses.
