...
V0.16.51 hardens the human handoffs around that compiler. The visible aggregate number now says **Overall**, so it cannot be confused with the simultaneous model-download percentage. A deployment refresh preserves the course brief and intended setup step; if source files were attached, only their names are retained and the user is asked to reattach the private bytes. Agent Review and Check starters run the deterministic package audit instead of asking the advisory model to pretend it completed one. Recommended materials and lesson scope expose real selected state, the quality report behaves as an accessible modal, informational Scion copy is no longer styled as a warning, and a phone-width replay keeps every setup action legible without horizontal overflow. A texture-perfect surface also skips the final voice provider call because its bounded score cannot improve beyond 100.
