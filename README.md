...
V0.16.72 freezes the exact base-only five-domain ruler after repairing the learner-facing defects that broke its first run. Mandarin, World Literature, Psychology, Nutrition, and Astronomy now each complete every requested lesson and all ten material sections at **99/A**, with **zero blockers, warnings, scored P0/P1/P2 findings, retries, or failed export checks**. Each package passes 38/38 export verification and physical ZIP testing; together the five archives contain 587 extracted files.
