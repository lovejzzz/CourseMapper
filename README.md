...
**v0.20.0 is in development** on `codex/v0200-teaching-system`. Shared tasks connect source records, reference answers, scoring and material revisions. Saved drafts and interrupted private feedback receipts can resume. Sourced generation without a shared task now opens the existing assignment review. A new reviewed claim-attribution contract separates recorded observations, assertions and explanations without inventing how a speaker knows something.

Natural generation still has demonstrated attribution and reference-answer failures. The new attribution contract has transaction and output-replay checks; its local Scion proposal and full classroom exports are not yet accepted. The frozen v3 corpus contains 36 development and 24 reserved cases. Complete-course, fresh-model and production acceptance remain unfinished; this branch is not a published v0.20.0 release. See the [roadmap](docs/v0.20.0-roadmap.zh-CN.md), [implementation evidence](docs/v0.20.0-implementation.zh-CN.md) and [current handoff](docs/v0.20.0-handoff.zh-CN.md).
