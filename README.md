...
Direct document inspection then found defects the 99/A score did not: a World Literature assessment weight rendered twice; a Psychology misconception option began with a severed narrator fragment; a short-answer key copied the whole case prompt instead of selecting its decisive evidence; and shortened references produced article seams, lowercase sentence starts, preposition-only labels such as “to Earth Systems,” and broken curly-apostrophe possessives. The compiler now deduplicates visible weights, removes narrator phrases without breaking grammar, writes compact evidence-backed model answers, and repairs shortened references at the final reader-visible boundary. These repairs are deterministic, regression-tested, and shared with compatible paid-model routes. Current Scion-facing copy says **AI-generated materials**, never **AI draft**, and the package action remains the single premium **Download ZIP** control.
