...
V0.16.71 closes the supplied Physical Geology failure at the artifact, compiler, browser, and product-language layers. The received V0.16.67 archive was valid but graded 74/C because its registered `Final (50%)` assessment never appeared in the exam content. The corrected exact pinned-base run completes in 55 seconds with three provider calls, 14/14 lesson kernels, all nine compiled material families, 38/38 export checks, a valid 118-file archive, and 99/A with zero P0/P1/P2 findings, blockers, warnings, retries, console errors, or archive errors.

A clean score is now expressed honestly and confidently. A zero-finding A/B package no longer becomes an amber review state merely because its score or texture meter is below a mathematically perfect 100; its exact score remains visible while the package receives the green **Ready to download** state. Actual findings, source caveats, export warnings, and low grades still remain visible and never borrow the success treatment. Primary package UX uses one **Download ZIP** action and finished-product language; Scion provenance now says **AI-generated**, not **AI-drafted**.

The same release deepens World Literature without hiding the remaining quality gap. Passage-based tasks, stronger reading-specific concepts, criterion-varied rubrics, a meaningful 16% final paper, and less repeated mechanical copy produce a fresh 98/A zero-finding archive. In reversed anonymous complete-artifact review, however, the paid reference still wins both orders (71–44 and 63–45). Scion is operationally reliable and materially better, but it has not yet earned paid-reference quality parity; richer, safer concept ledgers and less circular assessment content are the next compiler target.

The final V0.16.71 gate passes **434 test files and 5,361 tests**, with 16 files and 162 tests intentionally skipped, plus **147/147 browser tests**, formatting, lint, production build, locked bundle budgets, and the release-history audit. During that browser pass, Codex found and fixed a real export-state defect: an assignment linked to Lesson 1 but due in Week 4 was being scoped as Lesson 4, which falsely made the current tab look empty. Related-lesson identity now outranks scheduling metadata. The initial landing remains 250.9 KiB raw / 79.4 KiB gzip, while the lazy compiler stays below its ceiling at 825.6 KiB raw / 229.8 KiB gzip.
