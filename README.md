...
**v0.20.0 is in development** on `codex/v0200-teaching-system`. It adds shared source-grounded tasks, linked scoring, persistent review drafts and synchronized material revisions. Response-led feedback now retains its private notebook association when a matching draft is reopened and applied. Verified-runtime union and pooling proposals now use bounded local source questions with reviewed evidence and count-pair repair; the v3 corpus is frozen at 36 development and 24 reserved cases, the first natural development run failed its requested task and exposed lesson-count/source parsing defects, now repaired with an unchanged-input replay. a single local rerun fixed scope and source ingestion but still produced no shared task, so classroom export remained blocked. Natural task construction, broader Scion source-role reliability and complete-course acceptance remain unfinished; this branch is not a published v0.20.0 release. See the [roadmap](docs/v0.20.0-roadmap.zh-CN.md), [implementation evidence](docs/v0.20.0-implementation.zh-CN.md) and [current handoff](docs/v0.20.0-handoff.zh-CN.md).
