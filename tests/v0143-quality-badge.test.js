...
describe('B2 — WorkspaceQualityChip header states', () => {
  ...
  it('renders the emerald graded chip only for a clean 100/100 package, as a ≥32px button', () => {
    ...
    expect(html).toContain(
      'aria-label="Package quality: automated conformance 100 out of 100, 0 issues — open the quality report"',
    );
  });
  ...
  it('keeps a zero-finding A green while findings or a low grade remain amber', () => {
    ...
    expect(withP0).toContain('Conformance 88/100');
    ...
    expect(gradeC).toContain('Conformance 74/100');
  });
  ...
  it('does not foreground a 100/100 grade when the finish pass is blocked', () => {
    ...
    expect(html).toContain('automated conformance 100 out of 100');
...
describe('B2 — ExportSidePanel compact download-card stamp', () => {
  ...
  it('stamps automated conformance without presenting a teaching grade', () => {
