...
function summarize(r) {
  const lines = [];
  lines.push(`iterations=${r.iterations}  loopBroken=${r.loopBroken ? JSON.stringify(r.loopBroken) : 'none'}  customTools=${r.customTools.map(t=>t.name).join(',') || 'none'}`);
  ...
  if (final.chatReply) lines.push(`  FINAL chatReply: ${final.chatReply.slice(0, 220)}${final.chatReply.length > 220 ? '…' : ''}`);
...
function summarizeArgs(toolName, args) {
  if (!args) return '';
  if (toolName === 'read_deliverable') return `${args.featureId}${args.lessonIndex != null ? `#${args.lessonIndex}` : ''}`;
...
describeWithKey('Multi-turn TRACE DUMP', { timeout: 600_000 }, () => {
  const scenarios = [
    ...
    { tag: 'S2 rewrite', msg: 'The first question in Lesson 1 quiz is too easy. Rewrite it at Apply level with four plausible options. Just do it.', activeTab: 'quizBank' },
    { tag: 'S3 bulk', msg: 'Add an Analyze-level short answer question to every lesson in the quiz bank. Consider it done.', activeTab: 'quizBank' },
    { tag: 'S4 missing', msg: 'Add a rubric criterion about code quality to Lesson 2.' },
    { tag: 'S5 align', msg: 'Are my quizzes aligned with the lesson plan objectives? Check and tell me in plain English.', activeTab: 'quizBank' },
    { tag: 'S6 make-tool', msg: "I have a lot of low-Bloom's questions and I'll keep needing to fix them across deliverables in the future. Please CREATE A REUSABLE TOOL called `audit_bloom_floor` that takes a featureId and runs validate_course plus read_deliverable on that feature, so I can re-audit any deliverable later with one call. Then run it on quizBank and tell me what you found. Make sure the tool is registered via create_tool before you call it.", activeTab: 'quizBank', maxIterations: 12 },
  ];
  ...
  it('dump all scenarios', async () => {
    for (const s of scenarios) {
      const r = await runMultiTurn({
        ...
        userMessage: s.msg, activeTab: s.activeTab || 'quizBank', maxIterations: s.maxIterations || 20,
      });
