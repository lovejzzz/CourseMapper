...
test('release page preserves latest details and the complete historical changelog', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.getByRole('link', { name: 'v0.20.09', exact: true })).toBeVisible();
  await page.getByRole('link', { name: 'v0.20.09', exact: true }).click();
  await expect(page).toHaveURL(/#\/changelog$/);
  await expect(page.locator('[id="release-0.20.09"]')).toContainText('Who said what');
  await expect(page.locator('[id="release-0.20.09"]')).toContainText('Retain v0.20.08 and all earlier release notes');
  await page.getByRole('button', { name: 'Browse previous releases' }).click();
  await expect(page.locator('[id="release-0.20.08"]')).toBeInViewport();
  ...
  await page.reload();
  await expect(page.locator('[id="release-0.20.09"]')).toHaveCount(1);
  ...
  expect(await response.json()).toMatchObject({ version: '0.20.9', displayVersion: '0.20.09' });
