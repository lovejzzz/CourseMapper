...
async function call(prompt, activeTab = 'quizBank') {
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: MODEL, max_tokens: 4096, temperature: 0.3, system: systemPrompt, tools, messages: [{ role: 'user', content: prompt }] }),
  });
...
const SCENARIOS = [
  ...
  { tag: '[L] long-request',      prompt: 'Please redesign lesson 3 so it covers more modern architectures like transformers, make the quiz harder, and rewrite the objectives to Apply level' },
];
