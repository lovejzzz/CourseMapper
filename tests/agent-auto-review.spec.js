...
test.describe('Agent auto-review', () => {
  test('runs deterministic final pass silently without showing a user-authored Review my course bubble', async ({
    ...
    await expect(agentPanel.getByText(/Package (ready|needs attention|needs review)/)).toBeVisible({ timeout: 30000 });
    await expect(agentPanel.getByTestId('progress-phase-label')).toHaveText(/Ready to download|Finish package/);
