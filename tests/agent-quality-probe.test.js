...
describeWithKey(`Anthropic (${ANTHROPIC_MODEL}) Quality Probes`, { timeout: TIMEOUT * 15 }, () => {
  ...
  it('Q2: proposal options have meaningful (non-generic) titles', { timeout: TIMEOUT }, async () => {
    ...
    if (respond?.args?.proposal) {
      ...
      const generic = titles.filter(
        (t) => /^option\s+[a-z]$/i.test(t) || t === 'option 1' || t === 'option 2' || t.length < 4,
      );
      ...
      const blooms = respond.args.proposal.options.map((o) => o.action?.item?.bl).filter(Boolean);
  ...
  it('Q4: batches bulk edits into a single tool call with multiple actions', { timeout: TIMEOUT }, async () => {
    const r = await callClaude('Add a short answer Analyze-level question to every lesson in the quiz bank', {
      activeTab: 'quizBank',
    });
    const edit = find(r.toolCalls, 'edit_deliverables');
    const respond = find(r.toolCalls, 'respond');
  ...
  it('Q5: handles requests against non-existent deliverables gracefully', { timeout: TIMEOUT }, async () => {
    ...
    if (respond?.args?.chatReply) {
      ...
      const acknowledgesMissing =
        /rubric/i.test(lower) && /(not|haven't|generate|missing|don't exist|yet)/i.test(lower);
      expect(acknowledgesMissing, `Reply should mention rubrics aren't generated: ${respond.args.chatReply}`).toBe(
        true,
      );
    }
  });
  ...
  it('Q7: does not punt with clarifying questions for clear requests', { timeout: TIMEOUT }, async () => {
    ...
    if (respond?.args?.chatReply) {
      ...
      const askingForClarification =
        /(could you clarify|which|can you specify|do you mean|what typo)/i.test(reply) &&
  ...
  it('Q8: diagram response has clean mermaid syntax (no markdown fences)', { timeout: TIMEOUT }, async () => {
    ...
    if (respond?.args?.diagram?.syntax) {
      ...
      expect(syntax.trim(), `bad mermaid start: ${syntax.slice(0, 60)}`).toMatch(
        /^(graph|flowchart|sequenceDiagram|gantt|mindmap|classDiagram|stateDiagram|erDiagram|timeline|quadrantChart|journey)\b/,
      );
    }
  });
  ...
  it('Q9: answers facts-already-in-prompt without wasteful read calls', { timeout: TIMEOUT }, async () => {
    ...
    const wastefulReads = (r.toolCalls || []).filter(
      (tc) => tc.name === 'read_lesson' || tc.name === 'read_deliverable',
    );
    expect(
      wastefulReads.length,
      `agent wasted tool calls on facts already in prompt: ${wastefulReads.map((t) => t.name).join(',')}`,
    ).toBe(0);
  });
  ...
  it('Q10: editItem path structure matches deliverable schema', { timeout: TIMEOUT }, async () => {
    const r = await callClaude('Change the answer of question 1 in Lesson 1 quiz to "A data-driven method"', {
      activeTab: 'quizBank',
    });
    const edit = find(r.toolCalls, 'edit_deliverables');
    if (edit) {
