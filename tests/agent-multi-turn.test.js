...
const FIXTURE_DELIV = {
  quizBank: { status: 'done', data: { quizzes: [
    ...
    { lt: 'Neural Networks Fundamentals', qs: [
      { q: 'Activation function?', ty: 'short_answer', bl: 'Remember', df: 'easy', pt: 2, an: 'makes things non-linear' },
    ]},
  ]}},
...
describeWithKey(`Multi-turn agent loop (${MODEL}) — "Consider it done"`, { timeout: TIMEOUT * 8 }, () => {
  ...
  it('S2: reads the quiz first, then rewrites the easy Lesson 1 question', { timeout: TIMEOUT }, async () => {
    ...
    const r = await runMultiTurn({
      ...ctx(), activeTab: 'quizBank',
      userMessage: 'The first question in Lesson 1 quiz is too easy. Rewrite it at Apply level with four plausible options. Just do it.',
    });
    ...
    expect(evidence, `Expected Apply-level evidence. Edited q=${JSON.stringify(editedQ)} reply=${textReply.slice(0,200)}`).toBe(true);
  });
  ...
  it('S3: adds an Analyze-level question to every lesson in the quiz bank', { timeout: TIMEOUT }, async () => {
    ...
    if (proposed) {
      ...
      for (let i = 0; i < after.length; i++) {
        ...
        expect(any, `Lesson ${i+1} new questions should be Analyze short_answer, got ${JSON.stringify(newItems)}`).toBe(true);
      }
    }
  });
  ...
  it('S4: explains rubrics aren\'t generated instead of corrupting state', { timeout: TIMEOUT }, async () => {
    ...
    expect(/rubric/i.test(text) && /(not|haven't|generate|don't exist|yet)/i.test(text),
      `Expected explanation that rubrics aren't generated. Got: ${text.slice(0,300)}`).toBe(true);
  });
  ...
  it('S6: creates a custom macro and invokes it to upgrade Bloom\'s levels', { timeout: TIMEOUT }, async () => {
    ...
    const r = await runMultiTurn({
      ...ctx(), activeTab: 'quizBank', maxIterations: 12,
      userMessage:
    ...
    expect(createdNames.length, `Agent should register a custom tool. trace=${JSON.stringify(r.trace.map(t => t.calls.map(c => c.name)))}`).toBeGreaterThanOrEqual(1);
    ...
    expect(calls.includes('run_tool') || calls.some(c => createdNames.includes(c)),
      `Expected run_tool invocation after create_tool. Calls: ${calls.join(',')}`).toBe(true);
