      ...
      quizzes: [
        {
          ...
          qs: [{ q: 'a', ty: 'multiple_choice', bl: 'Remember', df: 'easy', pt: 1, op: ['A', 'B', 'C', 'D'], an: 'A' }],
        },
        {
          lt: 'Decision Trees and Random Forests',
          qs: [{ q: 'b', ty: 'multiple_choice', bl: 'Remember', df: 'easy', pt: 1, op: ['A', 'B', 'C', 'D'], an: 'A' }],
        },
        {
          lt: 'Neural Networks Fundamentals',
          qs: [{ q: 'c', ty: 'short_answer', bl: 'Remember', df: 'easy', pt: 2, an: 'x' }],
        },
      ],
    },
  },
...
describeWithKey(`Partial-failure recovery (${MODEL})`, { timeout: 600_000 }, () => {
  it('a batch with invalid lesson indices yields both successes and failures in tool details', async () => {
    ...
    const editCall = r.trace.flatMap((t) => t.calls).find((c) => c.name === 'edit_deliverables');
    expect(
      editCall,
      `agent should have issued exactly one edit_deliverables call. Trace: ${JSON.stringify(r.trace.map((t) => t.calls.map((c) => c.name)))}`,
    ).toBeTruthy();
    ...
    const badTargets = actions.filter(
      (a) => typeof a.lessonIndex === 'number' && a.lessonIndex >= COURSE.lessons.length,
    );
    expect(
      badTargets.length,
      `agent should have kept the invalid lesson indices so the user sees the partial-failure UX. actions=${JSON.stringify(actions)}`,
    ...
    expect(
      gained.filter((g) => g >= 1).length,
      `expected at least 2 lessons to gain a question; gained=${gained}`,
    ...
    expect(
      reply.toLowerCase(),
      `final reply should acknowledge the failed lessons. reply=${reply.slice(0, 300)}`,
    ).toMatch(/(lesson\s+9|98|99|don['’]t\s+exist|couldn['’]t|failed|unable|out\s+of\s+range)/i);
  });
});
