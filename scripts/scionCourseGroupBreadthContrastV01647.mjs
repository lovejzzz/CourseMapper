...
const CAMPAIGN = 'evaluation/scion-source-capture-course-group-breadth-v0.16.47.json';
const PROJECTS = 'evaluation/scion-source-capture-course-group-breadth-evidence';
export const SCION_COURSE_GROUP_BREADTH_CONTRAST =
  'evaluation/scion-contrast-matrix-course-group-breadth-v0.16.47.json';
...
export async function buildScionCourseGroupBreadthContrastV01647({ cwd = process.cwd() } = {}) {
  ...
  for (const group of campaign.groups) {
    ...
    if (
      ...
      throw new Error(`Course-group breadth project lineage mismatch: ${group.id}`);
    }
    if (
      ...
      throw new Error(`Course-group breadth model identity mismatch: ${group.id}`);
    }
    artifacts.push(
  ...
  return {
    ...
    claimBoundary:
      'These six neutral source-grounded capture pairs add two course contexts per novel domain and replay thirty-four already measured semantic kernels under the v2 authoring policy. They supply no preference label, new-kernel claim, adapter authorization, or quality win.',
  };
}

export async function runScionCourseGroupBreadthContrastV01647({ cwd = process.cwd(), write = false } = {}) {
  const report = await buildScionCourseGroupBreadthContrastV01647({ cwd });
  const output = path.resolve(cwd, SCION_COURSE_GROUP_BREADTH_CONTRAST);
  if (write) {
    await fs.writeFile(output, canonical(report));
  } else {
    const tracked = await fs.readFile(output, 'utf8');
    if (tracked !== canonical(report)) throw new Error('Tracked course-group breadth contrast is stale');
  }
...
async function main() {
  ...
  const result = await runScionCourseGroupBreadthContrastV01647({ write: args.has('--write') });
  console.log(
    `Scion course-group contrast: ${result.report.pairs.length} pairs / ${result.report.evidence.sourceKernels} source kernels.`,
  );
  console.log(`${result.wrote ? 'Wrote' : 'Verified'}: ${SCION_COURSE_GROUP_BREADTH_CONTRAST}`);
}
