...
} from './lib/scionLessonKernelExpansionBatch.mjs';
import { scionLessonKernelSha256, stableScionLessonKernelJson } from './lib/scionLessonKernelCampaign.mjs';

const DEFAULT_CAMPAIGN = 'evaluation/scion-adapters/lesson-kernel-campaign-v0.16.56.json';
const DEFAULT_OUTPUT = 'evaluation/scion-adapters/lesson-kernel-expansion-batch-v0.16.57.json';
const DEFAULT_EXCLUSIONS = [
  'evaluation/scion-adapters/evidence/lesson-kernel-pilot-v0.16.56/current/local-capture.json',
  'evaluation/scion-adapters/evidence/lesson-kernel-pilot-v0.16.56/current/reference-capture.json',
];
const DEFAULT_GENERATED_AT = '2026-07-19T18:00:00.000Z';
...
async function main() {
  ...
  if (args.help) {
    console.log('Usage: node scripts/scionLessonKernelExpansionBatchV01657.mjs [--write|--audit]');
    return;
  }
  const tracked = args.mode === 'audit' ? JSON.parse(await fs.readFile(args.output, 'utf8')) : undefined;
  const { campaign, batch } = await build(args, tracked);
  const validation = validateScionLessonKernelExpansionBatch(batch, campaign);
  ...
  console.log(JSON.stringify({ status: args.mode === 'write' ? 'written' : 'verified', ...batch.summary }, null, 2));
}
