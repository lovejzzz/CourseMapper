...
import { auditScionAdapterProductionCanaryEvidence } from './lib/scionAdapterCanaryPromotion.mjs';
...
export async function runScionAdapterProductionCanaryPromotionAudit({
  root = process.cwd(),
  evidencePath,
  manifestPath,
  outputPath,
  ...
  const report = await auditScionAdapterProductionCanaryEvidence({
    root,
    evidencePath,
    evidence,
    adapterManifest,
...
async function main() {
  ...
  if (args.help) {
    console.log(
      'Usage: node scripts/scionAdapterProductionCanaryPromotionAudit.mjs --manifest adapter.json --evidence production-canaries.json [--root repo] [--output report.json]',
    );
    return;
  }
  const report = await runScionAdapterProductionCanaryPromotionAudit(args);
