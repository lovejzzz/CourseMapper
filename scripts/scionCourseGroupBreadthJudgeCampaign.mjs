...
const PACKET = 'verification-output/scion-source-review-course-group-breadth-v0.16.47';
const PACKET_RECEIPT = 'evaluation/scion-adapters/evidence/source-review-packet-course-group-breadth-v0.16.47.json';
const PIPELINE_EVIDENCE = 'evaluation/scion-adapters/evidence/authoring-v2-comparison-v0.16.47.json';
const AB_ENVELOPE = 'evaluation/scion-adapters/evidence/codex-review-course-group-breadth-v0.16.47-a-b.sealed.json';
const BA_ENVELOPE = 'evaluation/scion-adapters/evidence/codex-review-course-group-breadth-v0.16.47-b-a.sealed.json';
const AB_WORKBOOK = 'evaluation/scion-adapters/handoffs/fresh-a-b-workbook-course-group-breadth-v0.16.47';
const BA_WORKBOOK = 'evaluation/scion-adapters/handoffs/fresh-b-a-workbook-course-group-breadth-v0.16.47';
const AB_WORKBOOK_RECEIPT = 'evaluation/scion-adapters/evidence/fresh-a-b-workbook-course-group-breadth-v0.16.47.json';
const BA_WORKBOOK_RECEIPT = 'evaluation/scion-adapters/evidence/fresh-b-a-workbook-course-group-breadth-v0.16.47.json';
const AB_STATUS_REPAIR =
  'evaluation/scion-adapters/evidence/codex-decision-status-repair-course-group-breadth-v0.16.47-a-b.json';
const BA_STATUS_REPAIR =
  'evaluation/scion-adapters/evidence/codex-decision-status-repair-course-group-breadth-v0.16.47-b-a.json';
const AB_TEMPLATE_REPAIR =
  'evaluation/scion-adapters/evidence/codex-decision-template-repair-course-group-breadth-v0.16.47-a-b.json';
const BA_TEMPLATE_REPAIR =
  'evaluation/scion-adapters/evidence/codex-decision-template-repair-course-group-breadth-v0.16.47-b-a.json';
const PRIOR_CORPUS = 'evaluation/scion-adapters/evidence/codex-approved-preferences-v0.16.47-expanded.jsonl';
const PRIOR_CAMPAIGN = 'evaluation/scion-adapters/evidence/judge-campaign-novel-breadth-v0.16.47.json';
export const SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS =
  'evaluation/scion-adapters/evidence/codex-approved-preferences-v0.16.47-course-groups.jsonl';
export const SCION_COURSE_GROUP_BREADTH_JUDGE_RECEIPT =
  'evaluation/scion-adapters/evidence/judge-campaign-course-group-breadth-v0.16.47.json';

const PRIMARY_KEYS = [
  path.join(os.homedir(), '.codex/scion-secrets/CourseMapper/course-group-breadth-v0.16.47-a-b.key'),
  path.join(os.homedir(), '.codex/scion-secrets/CourseMapper/course-group-breadth-v0.16.47-b-a.key'),
];
const BACKUP_KEYS = [
  path.join(os.homedir(), '.codex/scion-secrets/CourseMapper/course-group-breadth-v0.16.47-a-b.backup.key'),
  path.join(os.homedir(), '.codex/scion-secrets/CourseMapper/course-group-breadth-v0.16.47-b-a.backup.key'),
];
...
export function buildScionCourseGroupBreadthJudgeReceipt(inputs) {
  ...
  return {
    schemaVersion: 1,
    protocol: 'scion-course-group-breadth-paired-order-campaign-v1',
    release: RELEASE,
    ...
    pipelineEvidence: {
      ...
      status: inputs.pipelineEvidence.status,
      controlledSourceKernels: inputs.pipelineEvidence.semanticKernelControl?.kernelsPerCampaign,
      exactKernelPayloadMatch: inputs.pipelineEvidence.semanticKernelControl?.exactPayloadMatch,
      combinedCourseGroupsByDomain: inputs.pipelineEvidence.semanticKernelControl?.combinedCourseGroupsByDomain,
      reviewCandidateRows: inputs.pipelineEvidence.candidateSurfaces?.v2?.rows,
      localEligibleCandidates: inputs.pipelineEvidence.candidateSurfaces?.v2?.localEligible,
      referenceEligibleCandidates: inputs.pipelineEvidence.candidateSurfaces?.v2?.referenceEligible,
      bothEligibleCandidates: inputs.pipelineEvidence.candidateSurfaces?.v2?.bothEligible,
      compiledReferenceLead: inputs.pipelineEvidence.deltas?.compiledLocalGapV2,
      compiledGapNarrowing: inputs.pipelineEvidence.deltas?.compiledGapNarrowing,
    },
    ...
    approvedCorpus: {
      ...identity(SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS, inputs.approvedCorpusRaw),
      rows: approvedRows,
    ...
    claimBoundary:
      'This receipt records same-identity A/B and B/A model-judge evidence on two additional course groups per anatomy, economics, and physics domain. Combined coverage across the prior and current packets is a dataset property that must be recomputed by the strict readiness gate. Meeting the row-count threshold does not authorize training. This is not human, independent, classroom, held-out-adapter, device, or production-win evidence.',
  };
}

export function verifyScionCourseGroupBreadthJudgeReceipt({ receipt, ...inputs }) {
  const issues = [];
  if (!same(receipt, buildScionCourseGroupBreadthJudgeReceipt(inputs))) issues.push('receipt-reconstruction');
  ...
  if (
    inputs.abEnvelope.reviewCount !== 91 ||
    inputs.baEnvelope.reviewCount !== 91 ||
    inputs.packetReceipt.selectedCases !== 91
  ) {
  ...
  if (
    ...
    inputs.packetReceipt.courseGroupCount !== 6 ||
    !same(inputs.packetReceipt.domains, ['anatomy', 'economics', 'physics']) ||
    !same(inputs.packetReceipt.domainGroupCounts, { anatomy: 2, economics: 2, physics: 2 }) ||
  ...
  if (
    inputs.pipelineEvidence.protocol !== 'scion-authoring-v2-controlled-kernel-replay-v1' ||
    inputs.pipelineEvidence.status !== 'strict-compiler-gap-materially-narrowed' ||
    inputs.pipelineEvidence.semanticKernelControl?.kernelsPerCampaign !== 34 ||
    inputs.pipelineEvidence.semanticKernelControl?.exactPayloadMatch !== true ||
    !same(inputs.pipelineEvidence.semanticKernelControl?.combinedCourseGroupsByDomain, {
      anatomy: 3,
      economics: 3,
      physics: 3,
    }) ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.path !== inputs.packetReceipt.sourceFiles?.[0]?.source ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.sha256 !== inputs.packetReceipt.sourceFiles?.[0]?.sha256 ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.rows !== 91 ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.localEligible !== 88 ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.referenceEligible !== 85 ||
    inputs.pipelineEvidence.candidateSurfaces?.v2?.bothEligible !== 83 ||
    inputs.pipelineEvidence.deltas?.compiledLocalGapV2 !== 32 ||
    inputs.pipelineEvidence.deltas?.compiledGapNarrowing !== 45
  ) {
  ...
  for (const [label, repair] of [
    ...
    if (
      ...
      repair.scorecardsVisited !== 182 ||
      !Number.isInteger(repair.repairsApplied) ||
      repair.repairsApplied < 0 ||
      repair.repairsApplied > 182 ||
      repair.files?.length !== 10 ||
  ...
  for (const [label, repair] of [
    ...
    if (
      ...
      repair.templatesVisited !== 10 ||
      repair.decisionsVisited !== 91 ||
      !Number.isInteger(repair.repairsApplied) ||
      repair.repairsApplied < 0 ||
      repair.repairsApplied > 10 ||
      repair.files?.length !== 10 ||
  ...
  if (
    ...
    classified !== 91 ||
    inputs.ingestionReport.reviewedCases !== 91 ||
    inputs.ingestionReport.approved + inputs.ingestionReport.quarantined !== 91
  ) {
  ...
  if (analysis.agreementRate !== Number((analysis.crossOrderAgreement / 91).toFixed(6))) issues.push('agreement-rate');
  if (Object.values(analysis.byDomain || {}).reduce((sum, value) => sum + value.total, 0) !== 91) {
...
async function readInputs() {
  const files = [
    ...
    PRIOR_CORPUS,
    PRIOR_CAMPAIGN,
    SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS,
  ];
...
export async function writeScionCourseGroupBreadthJudgeReceipt() {
  const inputs = await readInputs().catch(async (error) => {
    if (error.code !== 'ENOENT' || !String(error.path).endsWith(SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS))
      throw error;
    await fs.writeFile(SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS, '');
    return readInputs();
  });
  ...
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'scion-course-group-breadth-ingestion-'));
  try {
    ...
    await fs.writeFile(SCION_COURSE_GROUP_BREADTH_APPROVED_CORPUS, approvedCorpusRaw);
    const receiptInputs = { ...inputs, ingestionReport, approvedCorpusRaw };
    const receipt = buildScionCourseGroupBreadthJudgeReceipt(receiptInputs);
    const verification = verifyScionCourseGroupBreadthJudgeReceipt({ receipt, ...receiptInputs });
    if (!verification.valid)
      throw new Error(`Course-group breadth judge campaign invalid: ${verification.issues.join(', ')}`);
    await fs.writeFile(SCION_COURSE_GROUP_BREADTH_JUDGE_RECEIPT, canonical(receipt));
...
export async function auditScionCourseGroupBreadthJudgeReceipt({ verifyLocalKeys = false } = {}) {
  const inputs = await readInputs();
  const receipt = JSON.parse(await fs.readFile(SCION_COURSE_GROUP_BREADTH_JUDGE_RECEIPT, 'utf8'));
  let ingestionReport = {
  ...
  if (verifyLocalKeys) {
    const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'scion-course-group-breadth-audit-'));
    try {
  ...
  const verification = verifyScionCourseGroupBreadthJudgeReceipt({
    receipt,
    ...inputs,
    ingestionReport,
    approvedCorpusRaw,
...
async function main() {
  if (process.argv.includes('--write')) {
    const { receipt } = await writeScionCourseGroupBreadthJudgeReceipt();
    console.log(`Scion course-group breadth judge campaign: ${receipt.status}`);
    ...
    console.log(`Receipt: ${SCION_COURSE_GROUP_BREADTH_JUDGE_RECEIPT}`);
    console.log('Plaintext written: false');
    return;
  }
  const result = await auditScionCourseGroupBreadthJudgeReceipt({
