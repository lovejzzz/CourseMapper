...
} from './lib/scionLessonKernelExpansionBatchV3.mjs';
...
const DEFAULT_OUTPUT = 'evaluation/scion-adapters/lesson-kernel-expansion-batch-v0.16.62.json';
const DEFAULT_EXCLUSIONS = Object.freeze([
  ...
  'evaluation/scion-adapters/evidence/semantic-expansion-v0.16.61/capture/local.json',
]);
const DEFAULT_GENERATED_AT = '2026-07-19T20:50:00.000Z';
...
export async function buildScionLessonKernelExpansionBatchV01662(args, tracked) {
  const [campaignRaw, selectorImplementationSha256, ...exclusionSources] = await Promise.all([
    fs.readFile(args.campaign, 'utf8'),
    sha256File('scripts/lib/scionLessonKernelExpansionBatchV3.mjs'),
    ...args.exclusions.map(readExclusionSource),
  ]);
  const campaign = JSON.parse(campaignRaw);
  const batch = buildScionLessonKernelExpansionBatchV3({
    campaign,
...
async function main() {
  ...
  if (args.help) {
    console.log(
      'Usage: node scripts/scionLessonKernelExpansionBatchV01662.mjs [--write|--audit] [--batch-size N] [--exclude-report file]',
    );
    return;
  }
  ...
  const validation = validateScionLessonKernelExpansionBatchV3(batch, campaign);
