...
export function scoreScionClassroomQuestion(question = {}) {
  ...
  const evidenceKeys = Object.keys(question.evidence || {}).sort().join(',');
  ...
  const dimensions = [
    dimension('genuine-uncertainty', question.trigger === 'deterministic-retry-exhaustion' && question.evidence?.terminalReason === 'retry-budget-exhausted' ? 20 : 0, 20, [
      'Requires an observable retry-exhaustion trigger and terminal receipt.',
    ]),
    dimension('bounded-decision', alternatives.length >= 2 && alternatives.length <= 8 && clean(question.decision?.request).endsWith('?') ? 20 : 0, 20, [
    ...
    dimension('falsifiability', criteria.length >= 4 && criteria.some((value) => /cross-discipline/i.test(value)) && criteria.some((value) => /source-ledger/i.test(value)) ? 20 : 0, 20, [
      'Requires transfer, source-integrity, regression, and cost success criteria.',
    ]),
    dimension('privacy-and-restraint', forbidden.length >= 4 && question.trainingEligible === false && leakagePaths(question.decision || {}).length === 0 ? 20 : 0, 20, [
...
export function buildScionClassroomPolicyCard({ learning, question, teacherCandidate, generatedAt = new Date().toISOString() } = {}) {
  ...
  if (!validateScionRoundtableStudentQuestion(question).valid) throw new Error('A classroom policy card requires the bound student question');
  ...
  if (
    ...
    scionLessonKernelSha256(learning.recommendedPolicy) !== scionLessonKernelSha256(teacherCandidate.recommendedPolicy) ||
...
export function buildScionClassroomPreregistration({
  ...
  for (const entry of cases) {
    ...
    const domain = byDomain.get(entry.domainGroupSha256) || { split: entry.split, repairable: 0, negative: 0, caseIds: [] };
    ...
    if (priorSplit && priorSplit !== entry.split) throw new Error(`Source group crosses classroom splits: ${entry.caseId}`);
  ...
  if (domains.filter((entry) => entry.split === 'practice').length !== 2 || domains.filter((entry) => entry.split === 'sealed-transfer').length !== 4) {
  ...
  const manifest = {
    ...
    claimBoundary: 'This preregistration freezes evaluation design and gates; it does not claim measured model learning.',
  };
...
export function buildScionClassroomExam({
  ...
  if ((split === 'practice' && !['baseline', 'advised'].includes(phase)) || (split === 'sealed-transfer' && phase !== 'delayed')) {
  ...
  if (committedForSplit.length !== cases.length) throw new Error(`Classroom packet does not contain the complete ${split} split`);
  ...
  const keyCases = [];
  for (const entry of cases) {
    if (!/^[a-z0-9-]{6,64}$/.test(entry.caseId || '') || seen.has(entry.caseId)) throw new Error('Invalid or duplicate classroom case id');
    ...
    if (
      ...
      scionLessonKernelSha256(entry) !==
        scionLessonKernelSha256(Object.fromEntries(Object.entries(privateEntry).filter(([key]) => key !== 'commitmentNonce'))) ||
    ...
    if (!Array.isArray(entry.issueCodes) || entry.issueCodes.length === 0) throw new Error(`Missing issue codes: ${entry.caseId}`);
  ...
  packetCases.sort((a, b) => {
    const aOrder = scionLessonKernelSha256({ seed: preregistration.design.randomizationSeedSha256, phase, caseId: a.caseId });
    const bOrder = scionLessonKernelSha256({ seed: preregistration.design.randomizationSeedSha256, phase, caseId: b.caseId });
  ...
  const packet = {
    ...
    cases: packetCases,
    forbiddenContext: ['course identity', 'raw source claims', 'answers or labels', 'provider routes or evaluation scores'],
  };
...
export function scoreScionClassroomAttempt({ packet, answerKey, attempt } = {}) {
  ...
  if (answerKey?.packetSha256 !== packet?.identity?.sha256 || !identityValid(answerKey)) issues.push('invalid-answer-key');
  ...
  if (!Number.isInteger(attempt?.providerCalls) || attempt.providerCalls < 0) issues.push('invalid-provider-call-count');
  if (
  ...
  const variantScores = Object.fromEntries(
    [...VARIANTS].map((variant) => {
      ...
      return [variant, selected.length ? Math.round(selected.reduce((sum, row) => sum + row.score, 0) / selected.length) : null];
    }),
  );
  const result = {
    schemaVersion: 1,
...
function pairedClassroomEvidence(baseline = {}, immediate = {}) {
  ...
  const variance =
    pairs.length > 1
      ? pairs.reduce((sum, pair) => sum + (pair.delta - meanDelta) ** 2, 0) / (pairs.length - 1)
      : null;
  const lowerBound95 =
    variance === null ? null : meanDelta - 1.96 * Math.sqrt(variance / pairs.length);
...
export function buildScionClassroomReviewReceipt({ verifierRef, baseline, immediate, delayed, blinded, pairedOrder } = {}) {
  ...
  const receipt = {
    ...
    claimBoundary: 'This receipt attests review procedure only; outcome statistics are derived from frozen result rows.',
  };
...
export function assessScionClassroomPromotion({
  ...
  const expectedSealed = new Set(
    (preregistration?.caseCommitments || []).filter((entry) => entry.split === 'sealed-transfer').map((entry) => entry.caseId),
  );
  const exactSet = (values, expected) => values?.length === expected.size && values.every((value) => expected.has(value));
  if (
  ...
  if (results.some((result) => result?.protocol !== SCION_CLASSROOM_RESULT_PROTOCOL || !identityValid(result))) issues.push('invalid-result');
  ...
  if (unique(results.map((result) => result?.sessionRef)).length !== 3 || results.some((result) => !SHA256_RE.test(result?.sessionRef || ''))) {
  ...
  if (baseline?.policyAccess !== 'none' || immediate?.policyAccess !== 'diagnostic-card' || delayed?.policyAccess !== 'diagnostic-card') {
  ...
  if (
    baselineCases.size !== immediateCases.size ||
    [...baselineCases].some((caseId) => !immediateCases.has(caseId))
  ) {
  ...
  if ((immediate?.variantScores?.boundary ?? 0) < 100 || (immediate?.variantScores?.negative ?? 0) < 100) issues.push('unsafe-immediate-quarantine');
  const sealedNegativeRows = (delayed?.rows || []).filter((row) => row.split === 'sealed-transfer' && row.variant === 'negative');
  if (
  ...
  if (
    [...(immediate?.rows || []), ...(delayed?.rows || [])].some((row) => row.signalOrigin !== 'verifier-derived')
  ) {
  ...
  const providerCalls = results.reduce((sum, result) => sum + (Number.isInteger(result?.providerCalls) ? result.providerCalls : Number.NaN), 0);
  if (!Number.isInteger(providerCalls) || providerCalls < 0 || providerCalls > (thresholds.maximumProviderCalls ?? -1)) {
