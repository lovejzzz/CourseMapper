...
import { applicationRelease } from './scripts/applicationRelease.mjs';
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        codeSplitting: {
          // Every rule below names an intentional leaf module. Capturing that
          // module's whole dependency graph would silently merge evidence and
          // compiler seams back together, so keep the boundary exact.
          ...
          groups: [
            {
              name(id) {
                ...
                if (
                  /src\/lib\/(?:courseMapContinuation|generatedCourseMapHandoff|materializedLessonScope)\.js$/.test(id)
                ) {
                  return 'courseMapContinuation';
                }
                if (/src\/lib\/workspaceSaveStatus\.js$/.test(id)) return 'workspaceSaveStatus';
                if (/src\/lib\/autosaveAttemptState\.js$/.test(id)) return 'autosaveAttemptState';
                ...
                if (/src\/lib\/objectiveConstructInstruction\.js$/.test(id)) return 'compilerFrames';
                if (/src\/lib\/plannedPracticeArtifacts\.js$/.test(id)) return 'plannedPracticeArtifacts';
                if (/src\/lib\/assignmentExtentPriority\.js$/.test(id)) return 'assignmentExtentPriority';
                if (/src\/lib\/linguisticAnalysisEvidence\.js$/.test(id)) return 'linguisticAnalysisEvidence';
                // Verified Coherent Draft policy is a course-agnostic, compile-only
                // contract layer. Keep it independently cacheable so semantic
                // admission and evidence policy can evolve without reparsing the
                // compiler controller or entering the landing route.
                if (/src\/lib\/verifiedDraftVisualQuizFrames\.js$/.test(id)) {
                  return 'verifiedDraftVisualEvidence';
                }
                if (/src\/lib\/operationQualifiedQuizItems\.js$/.test(id)) return 'operationQualifiedQuizItems';
                if (/src\/lib\/compilerSlideDepth\.js$/.test(id)) return 'compilerSlideDepth';
                if (/src\/lib\/verifiedDraftCompilerContracts\.js$/.test(id)) {
                  return 'verifiedDraftCompilerContracts';
                }
                if (/src\/lib\/researchCitationAdmission\.js$/.test(id)) {
                  return 'researchCitationAdmission';
                }
                if (/src\/lib\/authenticEvidenceStudyPractice\.js$/.test(id)) {
                  return 'authenticEvidenceStudyPractice';
                }
                if (/src\/lib\/sourceBoundStudyWorkedExample\.js$/.test(id)) {
                  return 'sourceBoundStudyWorkedExample';
                }
                if (/src\/lib\/assessmentEvidencePresentation\.js$/.test(id)) {
                  return 'assessmentEvidencePresentation';
                }
                if (/src\/lib\/sourceClaimRoles\.js$/.test(id)) return 'verifiedDraftVisualEvidence';
                if (/src\/lib\/statisticalOperationArtifactDetails\.js$/.test(id)) {
                  return 'statisticalOperationArtifactDetails';
                }
                if (/src\/lib\/artifactGenreDecode\.js$/.test(id)) {
                  return 'artifactGenreDecode';
                }
                if (/src\/lib\/discussionProtocol\.js$/.test(id)) {
                  return 'discussionProtocol';
                }
                if (/src\/lib\/instructionalIntentGraph\.js$/.test(id)) return 'instructionalIntentGraph';
                if (/src\/lib\/instructionalPlanGenerationAdmission\.js$/.test(id)) {
                  return 'instructionalPlanGenerationAdmission';
                }
                if (/src\/lib\/generalEvidenceReasoningIntent\.js$/.test(id)) return 'authenticEvidenceStudyPractice';
                if (/src\/lib\/blueprintSemanticContract\.js$/.test(id)) return 'blueprintSemanticContract';
                if (/src\/lib\/slideTimingPlan\.js$/.test(id)) return 'slideTimingPlan';
                ...
                if (/src\/lib\/quality\/deepQualitySubstanceDetails\.js$/.test(id))
                  return 'deepQualitySubstanceDetails';
                if (/src\/lib\/quality\/deepQualityFormatDetails\.js$/.test(id)) return 'deepQualityFormatDetails';
                if (/src\/lib\/quality\/sourceLedgerCitationSupport\.js$/.test(id))
