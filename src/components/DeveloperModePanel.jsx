...
function buildProposedSnapshot(baseSnapshot, drafts, dirtySections) {
  ...
  SECTIONS.filter(section => section.id !== 'raw').forEach((section) => {
...
export default function DeveloperModePanel({ isOpen, snapshot, onApply, onSaveTemplate, onClose }) {
  ...
  const stats = sectionStats(proposed || baseSnapshot, activeSection);
  const matchCount = countMatches(activeDraft, query);
          ...
          <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-4 py-3 dark:border-slate-800">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <button
                ...
                disabled={!dirtySections.has(activeSection)}
