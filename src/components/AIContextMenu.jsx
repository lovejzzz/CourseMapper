...
const AI_ACTIONS = [
  {
    ...
    prompt: (ctx) =>
      `Please improve the following content in ${ctx.location}. Make it more specific, actionable, and pedagogically sound. Current content:\n\n"${ctx.value}"`,
  },
  {
    id: 'expand',
    label: 'Expand',
    ...
    prompt: (ctx) =>
      `Please expand the following content in ${ctx.location} with more detail, examples, or specifics. Keep the pedagogical intent but make it richer. Current content:\n\n"${ctx.value}"`,
  },
  {
    id: 'simplify',
    label: 'Simplify',
    ...
    prompt: (ctx) =>
      `Please simplify the following content in ${ctx.location}. Make it more concise and clear while keeping the key pedagogical points. Current content:\n\n"${ctx.value}"`,
  },
  {
    id: 'rewrite',
    label: 'Rewrite',
    ...
    prompt: (ctx) =>
      `Please completely rewrite the following content in ${ctx.location}. Create a fresh version that achieves the same learning goal but with different wording and approach. Current content:\n\n"${ctx.value}"`,
  },
];
...
export default function AIContextMenu({ position, target, onAction, onClose }) {
  ...
  return createPortal(
    <div ref={menuRef} className="fixed z-[9999] animate-spring-in" style={{ left: x, top: y }}>
      <div
        role="menu"
        aria-label="AI editing actions"
        className="bg-white/95 backdrop-blur-xl rounded-xl border border-slate-200/60 shadow-xl py-1.5 min-w-[200px] overflow-hidden"
      >
        {/* Header */}
