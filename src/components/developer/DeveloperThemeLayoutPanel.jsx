...
export default function DeveloperThemeLayoutPanel({ currentConfig = {}, onUpdateConfigPatch }) {
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <section className="rounded-xl border border-slate-200/70 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
          ...
          <select
            ...
            onChange={(e) =>
              updateConfigPatch(
                { slideTheme: e.target.value === '' ? null : Number(e.target.value) },
                'Slide theme updated.',
              )
            }
          ...
          <p className="mt-3 text-[11px] leading-5 text-slate-500 dark:text-slate-400">
            This becomes the default visual style for generated slide deliverables and future templates saved from this
            IDE.
          </p>
        </section>
          ...
          <div className="mt-3 space-y-2">
            {features.length > 0 ? (
              features.map((feature, index) => (
                <div
                  key={`${feature}-${index}`}
                  className="flex items-center gap-2 rounded-lg border border-slate-200/70 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-950"
                >
                  <span className="flex h-6 w-6 items-center justify-center rounded-md bg-white text-[10px] font-bold text-slate-500 dark:bg-slate-900 dark:text-slate-300">
                    {index + 1}
                  </span>
                  <span className="min-w-0 flex-1 truncate text-[12px] font-semibold text-slate-700 dark:text-slate-200">
                    {titleFromDeveloperId(feature)}
                  </span>
                  <button
              ...
              <p className="rounded-lg border border-dashed border-slate-200 px-3 py-4 text-[12px] text-slate-400 dark:border-slate-700">
                No deliverable tabs selected.
              </p>
            )}
          </div>
        </section>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {columns.length > 0 ? (
            columns.map((column, index) => (
              ...
              >
                <svg
                  className="h-4 w-4 shrink-0 text-slate-300 transition-colors group-hover:text-indigo-400 dark:text-slate-600"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                ...
                <span className="min-w-0">
                  <span className="block truncate text-[12px] font-semibold text-slate-700 dark:text-slate-200">
                    {column.title || titleFromDeveloperId(column.key)}
                  </span>
            ...
            <p className="rounded-lg border border-dashed border-slate-200 px-3 py-4 text-[12px] text-slate-400 dark:border-slate-700">
              No columns are available in this project.
            </p>
          )}
        </div>
      </section>
