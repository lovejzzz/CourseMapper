...
import { getDeveloperSnapshotFindings, isPlainObject } from '../../lib/developerIdeDiagnostics.js';
import { formatBytes, getDeveloperRuntimeDiagnostics } from '../../lib/developerRuntimeDiagnostics.js';
...
function buildDiagnostics(snapshot, dirtySections) {
  ...
  if (lessons.length === 0)
    addIssue({ level: 'error', path: 'courseMap.lessons', message: 'Course map has no lessons.' });
  ...
  features
    ...
    .forEach((feature) => {
      ...
      if (!output)
        addIssue({
          level: 'info',
          path: `deliverables.${feature}`,
          message: `${titleFromDeveloperId(feature)} is selected but has not been generated yet.`,
        });
      if (output?.status === 'error')
        addIssue({
          level: 'error',
          path: `deliverables.${feature}`,
          message: `${titleFromDeveloperId(feature)} has a generation error.`,
        });
      if (output?.status === 'generating')
        addIssue({
          level: 'info',
          path: `deliverables.${feature}`,
          message: `${titleFromDeveloperId(feature)} is still generating.`,
        });
    });
  if (columns.length === 0)
    addIssue({ level: 'warning', path: 'columns', message: 'No course map columns are configured.' });
  ...
  if (!snapshot.modelId && !snapshot.modelName)
    addIssue({ level: 'warning', path: 'modelId', message: 'No AI model is selected.' });
  if (dirtySections.size > 0)
    addIssue({ level: 'info', path: 'drafts', message: 'Developer edits are pending and have not been applied.' });
...
export default function DeveloperDiagnosticsPanel({
  ...
  return (
    <div data-testid="developer-diagnostics-panel" className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map(([label, value]) => (
          <div
            key={label}
            className="rounded-xl border border-slate-200/70 bg-white p-4 dark:border-slate-700 dark:bg-slate-900"
          >
      ...
      <section
        data-testid="developer-health-check"
        className="mt-4 rounded-xl border border-slate-200/70 bg-white p-4 dark:border-slate-700 dark:bg-slate-900"
      >
          ...
          {diagnostics.issues.length > 0 ? (
            diagnostics.issues.map((issue, index) => (
              <div
                ...
                data-testid="developer-diagnostic-finding"
                data-level={issue.level}
                data-path={issue.path || ''}
                className="flex gap-3 rounded-lg border border-slate-200/70 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-950"
              >
                <span
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {issue.path && (
                    <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                      <button
                        type="button"
                        data-testid="developer-diagnostic-path"
                        data-path={issue.path}
                        onClick={() => onDiagnosticPathClick?.(issue.path)}
                        className="min-w-0 break-all rounded px-1 py-0.5 text-left font-mono text-[10px] text-indigo-500 hover:bg-indigo-50 dark:text-indigo-300 dark:hover:bg-indigo-500/10"
                      ...
                      <button
                        type="button"
                        data-testid="developer-diagnostic-copy-path"
                        data-path={issue.path}
                        onClick={() => onDiagnosticPathCopy?.(issue.path)}
                        className="tactile rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-bold text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                        aria-label={`Copy JSON path ${issue.path}`}
                        title={`Copy ${issue.path}`}
                      >
                        Copy
                      </button>
                    </div>
      ...
      <section className="mt-4 rounded-xl border border-slate-200/70 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="mt-3 grid gap-2 text-[12px] text-slate-600 dark:text-slate-300 sm:grid-cols-2">
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Provider:</span>{' '}
            {runtimeDiagnostics.providerLabel}
          </p>
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Model:</span> {runtimeDiagnostics.modelLabel}
          </p>
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Active tab:</span>{' '}
            {titleFromDeveloperId(snapshot.activeTab)}
          </p>
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Pending edits:</span> {dirtySections.size}
          </p>
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Key policy:</span>{' '}
            {runtimeDiagnostics.apiKeyPolicy}
          </p>
          <p>
            <span className="font-bold text-slate-800 dark:text-slate-100">Snapshot size:</span>{' '}
            {formatBytes(runtimeDiagnostics.counts.snapshotBytes)}
          </p>
        </div>
      </section>
          ...
          <div>
            ...
            <h3 className="mt-1 text-sm font-bold text-slate-800 dark:text-slate-100">
              {runtimeDiagnostics.risks.length === 0
                ? 'No runtime risks found'
                : `${runtimeDiagnostics.risks.length} operational signals`}
            </h3>
          </div>
        </div>
          ...
          <div className="mt-3 space-y-2">
            {runtimeDiagnostics.risks.map((risk, index) => (
              <div
                ...
                data-testid="developer-runtime-risk"
                ...
                <div className="min-w-0">
                  ...
                  {risk.path && (
                    <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                      <button
                        type="button"
                        data-testid="developer-runtime-risk-path"
                        data-path={risk.path}
                      ...
                      <button
                        type="button"
                        data-testid="developer-runtime-risk-copy-path"
                        data-path={risk.path}
                        onClick={() => onDiagnosticPathCopy?.(risk.path)}
                        className="tactile rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-bold text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                        aria-label={`Copy JSON path ${risk.path}`}
                        title={`Copy ${risk.path}`}
                      >
                        Copy
                      </button>
                    </div>
