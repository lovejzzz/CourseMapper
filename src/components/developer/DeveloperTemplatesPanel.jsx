...
} from '../../lib/developerTemplateExchange.js';
...
export default function DeveloperTemplatesPanel({
  ...
  function handleExportTemplates() {
    ...
    try {
      ...
      onStatus?.('success', `Exported ${developerTemplates.length} developer template${developerTemplates.length === 1 ? '' : 's'}.`);
    } catch (err) {
              ...
              <div key={template.id} className={`rounded-xl border p-3 ${template.id === activeDeveloperTemplateId ? 'border-indigo-300 bg-indigo-50/60 dark:border-indigo-500/50 dark:bg-indigo-500/10' : 'border-slate-200/70 bg-slate-50 dark:border-slate-700 dark:bg-slate-950'}`}>
                ...
                {secretFindings.length > 0 && (
                  <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-[11px] font-semibold text-red-700 dark:border-red-500/40 dark:bg-red-500/10 dark:text-red-300">
                    Secret-like data found at {secretFindings[0].path}. Delete or replace this template before staging/exporting.
                  </div>
                )}
                  ...
                  {templateDiff.length > 0 ? (
                    <ul className="mt-2 space-y-1">
                      {templateDiff.map(change => (
                        <li key={`${template.id}-${change.type}-${change.path}`} className="truncate text-[10px] text-slate-500 dark:text-slate-400">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
                  {TEMPLATE_STAGE_MODES.map(mode => (
                    ...
                    >
                      ...
                      <span className={mode.id === 'all' ? 'text-indigo-100' : 'text-slate-400'}>{modeDiffCounts[mode.id]}</span>
                    </button>
                  ))}
                </div>
              </div>
