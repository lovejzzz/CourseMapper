 ...
 *
 * The package evidence summary, rendered in the workspace header
 ...
 *     any P0 landed; click opens the full
...
// Same evidence result shape ExportSidePanel's report modal consumes.
...
export default function WorkspaceQualityChip({ packageQualityPass, onOpenReport }) {
    ...
    return (
      <button
        ...
        title={`Export is paused for ${refinementText}. ${
          readinessScore !== null
            ? `Deterministic package evidence is ${readinessScore}/${readinessMax} earned${unobservedPoints !== null ? ` with ${unobservedPoints} unobserved` : ''}; automated conformance is ${quality.score}/100 and is not a teaching-quality grade`
            : `Automated conformance is ${quality.score}/100 and is not a teaching-quality grade`
      ...
      >
        <span>
          {readinessScore !== null ? `Evidence ${readinessScore}/${readinessMax}` : `Conformance ${quality.score}/100`}
        </span>
        {textureScore !== null && (
  ...
  // The honest evidence score leads. The legacy letter grade remains in the
  // machine receipt for compatibility, but is never presented as educational
  // quality: automated conformance only detects encoded defects.
  return (
    <button
      type="button"
      data-testid="workspace-quality-chip"
      ...
      title={`${
        readinessScore !== null
          ? `Deterministic package evidence ${readinessScore}/${readinessMax} earned${unobservedPoints !== null ? ` · ${unobservedPoints} unobserved` : ''} · automated conformance ${quality.score}/100 (not a teaching-quality grade)`
          : `Automated conformance ${quality.score}/100 (not a teaching-quality grade)`
      } · ${issues} issue${issues === 1 ? '' : 's'}${
        textureScore !== null ? ` · Texture ${textureScore}/100 — style and repetition evidence` : ''
    ...
    >
      {readinessScore !== null ? (
        ...
        <>
          <span>Conformance {quality.score}/100</span>
          {textureScore !== null && (
            <span data-testid="workspace-texture-meter" className="font-semibold text-slate-500 dark:text-slate-400">
              · Texture {textureScore}
            </span>
