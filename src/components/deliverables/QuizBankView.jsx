...
// ─── Quiz Bank ───
export default function QuizBankView({ data, isStreaming, onEdit, regeneratingIndex, onRegenerateLesson, onSaveToBank, activeTier, freshLessonIndices, proposals, onAcceptProposal, onDismissProposal, onRegenerateProposal }) {
  ...
  return (
    <div className="space-y-3 p-4">
      {quizzes.map((baseQuiz, i) => {
        ...
        const quiz = (currentTier !== 'standard' && baseQuiz.tiers?.[currentTier]) ? { ...baseQuiz, ...baseQuiz.tiers[currentTier] } : baseQuiz;
        const subtitle = [
