...
const COURSE_MAP_TARGETS = ['lessonPlans', 'slideDecks', 'studyGuides', 'assignments', 'rubrics', 'quizBank', 'discussions', 'syllabus'];
...
export default function DependencyMap({ isOpen, onClose, deliverables }) {
  ...
  const downstream = focused
    ? (focused === 'courseMap' ? COURSE_MAP_TARGETS : (DELIVERABLE_OUTBOUND_MAP[focused] || []))
    : [];
      ...
      <div className="fixed inset-0 z-[9998] bg-black/20 backdrop-blur-sm flex items-center justify-center" onClick={onClose}>
        ...
        <svg viewBox="0 0 360 400" className="w-full" style={{ maxHeight: '360px' }}>
          <defs>
            ...
            <marker id="arrow-up" viewBox="0 0 10 6" refX="10" refY="3" markerWidth="8" markerHeight="6" orient="auto-start-reverse">
              <path d="M 0 0 L 10 3 L 0 6 z" fill="#6366f1" />
            </marker>
            <marker id="arrow-down" viewBox="0 0 10 6" refX="10" refY="3" markerWidth="8" markerHeight="6" orient="auto-start-reverse">
          ...
          {edges.map(({ from, to }, i) => {
            ...
            if (isUpstream) { stroke = '#f59e0b'; strokeWidth = 2.5; marker = 'url(#arrow-down)'; }
            else if (isDownstream) { stroke = '#6366f1'; strokeWidth = 2.5; marker = 'url(#arrow-up)'; }
          ...
          {visibleNodes.map(id => {
            ...
            return (
              <g
                key={id}
                onClick={(e) => { e.stopPropagation(); setFocused(focused === id ? null : id); }}
              ...
              >
                ...
                {/* Upstream ring */}
                {isUp && (
                  <rect x={pos.x - 2} y={pos.y - 2} width={NODE_W + 4} height={NODE_H + 4}
                    rx={11} fill="none" stroke="#6366f1" strokeWidth={1.5} strokeDasharray="4 2" />
                )}
                {/* Downstream ring */}
                {isDown && (
                  <rect x={pos.x - 2} y={pos.y - 2} width={NODE_W + 4} height={NODE_H + 4}
                    rx={11} fill="none" stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="4 2" />
                )}
                {/* Node body */}
                <rect x={pos.x} y={pos.y} width={NODE_W} height={NODE_H}
                  rx={9} fill={colors.fill} stroke={colors.stroke} strokeWidth={1.5} />
                {/* Label */}
                <text x={pos.x + NODE_W / 2} y={pos.y + NODE_H / 2 + 1}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize="8" fontWeight="600" fill={colors.text}>
                  {feat.label.length > 14 ? feat.label.slice(0, 12) + '…' : feat.label}
                </text>
                {/* Status dot */}
                <circle cx={pos.x + NODE_W - 6} cy={pos.y + 6} r={3.5}
                  fill={status} stroke="white" strokeWidth={1} />
              </g>
            );
          })}
        </svg>
        ...
        <div className="mt-4 flex items-center gap-4 text-[10px] text-slate-400">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-400 inline-block" /> Generated</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" /> Stale</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-300 inline-block" /> Not generated</span>
        </div>
        ...
        {focused && (
          <div className="mt-3 p-3 bg-slate-50/80 rounded-xl border border-slate-200/40 animate-spring-in">
            <p className="text-[11px] font-bold text-slate-600 mb-2">
              {getFeature(focused)?.label || focused}
            </p>
            <div className="flex flex-col gap-1.5">
              {upstream.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-[10px] text-indigo-500 font-semibold whitespace-nowrap mt-0.5">← Feeds in:</span>
                  <div className="flex flex-wrap gap-1">
                    {upstream.map(id => {
                      const f = getFeature(id);
                      const c = COLOR_MAP[f?.color || 'indigo'];
                      return <span key={id} className={`${c.badge} text-[9px] font-semibold px-1.5 py-0.5 rounded-full`}>{f?.label || id}</span>;
                    })}
                  </div>
                </div>
              )}
              {downstream.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-[10px] text-amber-500 font-semibold whitespace-nowrap mt-0.5">→ Affects:</span>
                  <div className="flex flex-wrap gap-1">
                    {downstream.map(id => {
                      const f = getFeature(id);
                      const c = COLOR_MAP[f?.color || 'indigo'];
