...
export default function ChatPanel({
  ...
  return (
    <div
      ...
      data-print="hide"
    >
      {/* ── Header ── v0.20.07: one title; status appears only when it says
          something (working, needs setup), not "Available" on every visit. */}
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-slate-200/70 flex-shrink-0">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            <h2 className="text-sm font-semibold text-slate-800 truncate">Assistant</h2>
            {headerAgentStatus.label !== 'Available' &&
            ...
            {showsAgentIdentity && !compactReady && workspaceModelStatus.blocked && (
              <button
                type="button"
      ...
      {/* v0.20.07: the working-set card repeated the header's lesson count and
          the export drawer's check status. Show it only when it has something
          to act on: materials waiting for sync, or a generation error. */}
      {showsAgentIdentity &&
        !compactReady &&
        !getPackageTrustStatus({ packageQualityPass }).canDownload &&
        !ribbonModel?.running &&
        (pendingSyncFeatureIds?.length > 0 || Boolean(error)) && (
