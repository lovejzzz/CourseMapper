      ...
      <button
        ...
        title={
          hasSyncError
            ...
            : hasTools
              ? `${count} agent-created macro${count === 1 ? '' : 's'}`
              : 'No macros yet'
        }
      >
        {/* wand/sparkle icon */}
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">
                  Tools the agent built via{' '}
                  <code className="px-1 py-0.5 rounded bg-slate-100 text-slate-600 text-[10px]">create_tool</code>.
                  Session-scoped, synced to your account.
                </p>
              </div>
              {onImport && (
                <button
              ...
              <div className="mt-2 space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[10px] text-slate-500">
                    Paste a macro JSON snippet someone shared, or use{' '}
                    <code className="px-1 py-0.5 rounded bg-slate-100 text-slate-600 text-[10px]">Copy JSON</code> on
                    one of your own.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      setImportText(EXAMPLE_IMPORT_JSON);
                      setImportError(null);
                    }}
            ...
            {hasSyncError && (
              <div className="mt-2 p-2 rounded-md bg-amber-50/80 border border-amber-200/60">
                ...
                <p className="text-[10px] text-amber-600 mt-0.5">
                  <code className="font-mono">{syncError.name}</code> ({syncError.op}) didn't reach the cloud:{' '}
                  {syncError.message}. Local copy still works — any future macro save will retry.
                </p>
              </div>
            )}
          </div>
                        ...
                        <div className="flex items-center gap-0.5 flex-shrink-0">
                          <button
                            ...
                            className={`transition-colors p-1 ${
                              copiedName === tool.name ? 'text-emerald-500' : 'text-slate-400 hover:text-indigo-500'
                            }`}
