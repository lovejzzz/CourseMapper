...
function SeverityDot({ severity }) {
  return (
    <span
      className={`inline-block w-1.5 h-1.5 rounded-full ${SEVERITY_COLORS[severity]?.dot || 'bg-slate-400'} flex-shrink-0 mt-1.5`}
    />
  );
}
  ...
  return (
    <div className="flex items-start gap-2 py-1.5 border-b border-slate-100/50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[12px] text-slate-700 leading-snug">{finding.message}</p>
...
export default function ValidationCard({ report, onFixClick }) {
  ...
  const headerIcon = errorCount > 0 ? 'text-red-500' : warningCount > 0 ? 'text-amber-500' : 'text-blue-400';
