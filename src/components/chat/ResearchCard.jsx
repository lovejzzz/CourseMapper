    ...
    return (
      <div className="flex gap-2 py-1.5 border-b border-slate-100/50 last:border-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.author && <p className="text-[11px] text-slate-500 truncate">{item.author}</p>}
    ...
    return (
      <div className="flex gap-2 py-1.5 border-b border-slate-100/50 last:border-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.authors && <p className="text-[11px] text-slate-500 truncate">{item.authors}</p>}
          <div className="flex items-center gap-2 mt-0.5">
            {item.year && <span className="text-[10px] text-slate-400 font-medium">{item.year}</span>}
            {item.publisher && <span className="text-[10px] text-slate-400">{item.publisher}</span>}
            {item.isbn && <span className="text-[10px] text-indigo-500 font-medium">ISBN: {item.isbn}</span>}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex gap-2 py-1.5 border-b border-slate-100/50 last:border-0">
      <div className="flex-1 min-w-0">
        ...
        {!isWiki && item.authors && <p className="text-[11px] text-slate-500 truncate">{item.authors}</p>}
        {isWiki && item.snippet && <p className="text-[11px] text-slate-500 line-clamp-2 mt-0.5">{item.snippet}</p>}
        <div className="flex items-center gap-2 mt-0.5">
          {item.year && <span className="text-[10px] text-slate-400 font-medium">{item.year}</span>}
          {!isWiki && item.citationCount > 0 && (
            <span className="text-[10px] text-indigo-500 font-medium">{item.citationCount} citations</span>
          )}
          {item.doi && (
            <a
...
export default function ResearchCard({ research, status }) {
  ...
  const totalCount = (research.results || []).reduce((sum, g) => sum + (g.items?.length || 0), 0);
    ...
    return (
      <div className="mx-2 my-1 px-4 py-3 rounded-xl bg-violet-50/60 border border-violet-200/30 animate-spring-in">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[13px] font-medium text-violet-700">Researching: &ldquo;{research.query}&rdquo;</p>
            {research.reason && <p className="text-[11px] text-violet-500 mt-0.5">{research.reason}</p>}
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-2 my-1 px-4 py-3 rounded-xl bg-red-50/60 border border-red-200/30 animate-spring-in">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-[13px] font-medium text-red-700">Search failed for &ldquo;{research.query}&rdquo;</p>
            {research.error && <p className="text-[11px] text-red-500 mt-0.5">{research.error}</p>}
          </div>
        </div>
      </div>
    );
    ...
    <div className="mx-2 my-1 rounded-xl bg-white/60 border border-slate-200/30 shadow-glass animate-spring-in overflow-hidden">
      ...
      {expanded && (
        <div className="px-4 pb-3 space-y-2 border-t border-slate-100/50">
          ...
          {totalCount === 0 && <p className="text-[12px] text-slate-400 py-2">No results found.</p>}
        </div>
      )}
    </div>
  );
