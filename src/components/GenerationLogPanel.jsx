...
export default function GenerationLogPanel({ entries, defaultCollapsed = false }) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (e.type === 'success' && i > 0 && (entries[i - 1].type === 'warning' || entries[i - 1].type === 'switch')) doneIndices.add(i - 1);
  }
        ...
        {!collapsed && (
          <div className="divide-y divide-slate-100/60">
            {entries.map((entry, i) => {
              ...
              return (
                <div key={i} className={`flex items-start gap-2 px-3 py-1.5 ${typeStyles[entry.type] || typeStyles.info} ${isDone ? 'opacity-50' : ''}`}>
