...
export default function EditProposalPanel({ proposal, featureId, onInsert, onRegenerate, onDismiss }) {
  ...
  return (
    <div
      className={`mb-3 rounded-squircle-xs overflow-hidden border transition-all duration-300 ${
        isStreaming ? 'border-indigo-200/60 bg-indigo-50/40 animate-pulse-subtle' : 'border-amber-200/70 bg-amber-50/40'
      }`}
    >
      {/* ── Header ── */}
      <div
        ...
        {isStreaming && !proposedData ? (
          ...
          isEditing ? (
            <div className="w-full">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-amber-600 mb-1 block">
                Edit Raw Data (JSON)
              </span>
              <textarea
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-semibold border transition-all ${
              isEditing
                ...
                : 'text-slate-500 border-slate-200/60 hover:text-slate-700 hover:bg-slate-100/60'
            }`}
  ...
  function MiniList({ items, max = 2 }) {
    ...
    return (
      <ul className="mt-1 space-y-0.5">
        ...
        {rest > 0 && <li className={`text-[10px] ${mutedColor} italic ml-3`}>+ {rest} more…</li>}
      </ul>
    );
  }
      ...
      return (
        <div>
          ...
          {plan?.homework && (
            <Field
              label="Homework"
              value={typeof plan.homework === 'string' ? plan.homework : plan.homework?.description}
            />
          )}
        </div>
      );
