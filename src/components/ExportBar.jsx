...
export default function ExportBar({ onExport, onImport }) {
  ...
  const btnBase = 'tactile flex items-center gap-2 px-4 py-2.5 rounded-pill text-xs font-semibold transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed';
  ...
  return (
    <div className="mt-4 ml-10 animate-spring-in">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {onImport && (
          <>
            ...
            <input ref={importRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleImportChange} className="hidden" />
            <button onClick={() => importRef.current?.click()} className={btnLight} title="Import course map from .xlsx or .csv">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex items-center gap-2 mt-2 flex-wrap">
        ...
        <button onClick={() => handleExport('gsheets')} disabled={!!exporting}
        ...
        <button onClick={() => handleExport('gdocs')} disabled={!!exporting}
