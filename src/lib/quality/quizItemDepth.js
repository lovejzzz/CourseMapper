...
const REASONING_RE =
  /\b(?:based on (?:this|these|the) evidence|evaluate(?: the claim)?|why is (?:this|the\s+\w+) invalid|what should|what concept (?:best )?(?:justifies|explains|applies)|what is (?:the )?(?:(?:most likely|likely|most effective|best|primary) )?(?:outcome|impact|risk|benefit|cause|response|principle|heuristic|purpose|flaw|issue|way)|which (?:(?:ethical|usability|accessibility|research|design)\s+)?(?:principle|heuristic) (?:is|best)|which (?:(?:next|best|first|primary|most likely|likely)\s+)?(?:response|interpretation|conclusion|action|step|change|claim|finding|inference|method|evidence|approach|choice|use|move|outcome|impact|risk|benefit|cause|principle|heuristic|purpose)|how should|what does this|best (?:reflects|explains|supports|addresses)|what (?:problem|issue|risk|impact)|what(?:\s+\w+){0,4}\s+(?:pitch|value|classification|category|diagnosis|pattern)|is (?:this|the) claim|most (?:useful|relevant|clearly|evident)|reveals?|supports?|indicates?|suggests?)\b/i;
