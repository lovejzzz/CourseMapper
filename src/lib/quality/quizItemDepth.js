...
const ACTOR_RE =
  /\b(?:user|participant|researcher|team|designer|patient|student|customer|developer|colleague|manager|stakeholder|employee|shopper|reader|learner|client|musician|composer|engineer|analyst|historian|clinician|instructor|writer|artist|scientist|person|worker|executive|chief executive|ceo|board member|board|employer|applicant|nurse|consumer|owner|company|corporation|organization|firm|factory|manufacturer|supplier|vendor|non-governmental organization|ngo|regulator|agency|hospital|plant)s?\b/i;
const ACTION_RE =
...
export function isAppliedQuizStem(stem) {
  ...
  if (wordCount < 12 || !/[.?!:]['’”")\]]?\s*$/.test(text) || !REASONING_RE.test(text)) return false;
