...
export async function generateXlsx(courseMap, customColumns) {
  ...
  workbook.title = `${courseMap.courseName || 'Course'} Course Map`;
  workbook.subject = 'Course Map';
  workbook.description = `Course map generated by CourseMapper for ${courseMap.courseName || 'a course'}`;
    ...
    for (const section of sections) {
      ...
      for (const col of COLUMNS) {
        ...
        if (col.key === 'evaluateDesign') {
          ...
          rowData[col.key] = toStr(section[col.key]);
        }
      }
...
export async function buildXlsxBuffer(courseMap, customColumns) {
  ...
  const worksheet = workbook.addWorksheet('Course Map');
  ...
  for (const lesson of courseMap.lessons) {
    ...
    const endRow = currentRow - 1;
    if (endRow > startRow) worksheet.mergeCells(startRow, 1, endRow, 1);
  }
