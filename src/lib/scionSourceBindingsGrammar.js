...
// Shape and source aliases only. Exact spans and semantic roles are assessed separately.
export function sourceBindingsGrammar(operation, inputs) {
  const spec = Object.hasOwn(TEACHING_OPERATION_SPECS, operation) ? TEACHING_OPERATION_SPECS[operation] : null;
  if (!spec) throw new Error('Unknown teaching operation.');
  ...
  return [
    ...
    `bindings ::= ${object(Object.entries(spec.bindings).map(([key, type]) => [key, type === 'record' ? 'record' : 'phrase']))}`,
    `phrase ::= "null" | ${object([
      ['source', 'source'],
      ['quote', 'string'],
      ['occurrence', 'count'],
