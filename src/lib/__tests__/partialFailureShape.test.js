...
function buildChangeSummary({ toolName, toolArgs, result }) {
  const changes = [];
  const failedItems = [];
  const originalInputs = toolName === 'edit_course_map'
    ? (toolArgs?.patches || [])
    : (toolArgs?.actions || []);
  ...
  const message = failed === 0
    ...
    : applied > 0 ? `${applied} applied · ${failed} failed` : `${failed} change${failed !== 1 ? 's' : ''} failed`;
...
describe('changeSummary shape for edit tools', () => {
  it('pure success — all details.success=true', () => {
    const summary = buildChangeSummary({
      ...
      toolArgs: { actions: [
        { type: 'addItem', featureId: 'quizBank', lessonIndex: 0, item: { q: 'x', ty: 'multiple_choice' } },
      ]},
      result: {
        applied: 1, failed: 0,
    ...
    const summary = buildChangeSummary({
      ...
      result: {
        ...
        details: [
          ...
          { action: 'addItem', featureId: 'quizBank', lessonIndex: 9, success: false, message: 'Lesson index 9 out of range (0-2)' },
        ],
      },
    });
