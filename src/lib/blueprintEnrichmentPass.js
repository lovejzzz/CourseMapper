  ...
  const systemPrompt = [
    ...
    ...(activityLessonIds.length > 0
      ? [
          ...
          'The scenario establishes ONLY the initial conditions. Never reveal, paraphrase, or foreshadow the evolving update or its required decision in the scenario; that information belongs only in up and is released later. Bad sc: "Participants revise the response after synchronized evidence updates." Good sc: "Two delegations dispute a patrol collision while a civilian convoy approaches the corridor."',
          'For each update, information states the new fact or condition. For requiredDecision, choose exactly one observable post-update action permitted by the schema enum; the compiler grounds artifact references in the visible product title.',
          'For ar.ti, choose exactly one inspectable product form permitted by the schema enum. The compiler adds the course-specific subject to its visible title.',
