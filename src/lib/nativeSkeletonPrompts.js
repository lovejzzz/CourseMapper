...
4. RECURRING ASSESSMENTS: when the source states a recurring cadence ("weekly autograded quizzes", "weekly labs", "weekly reading responses"), expand it into one assessments[] entry PER SESSION it applies to — title each "<cadence artifact>: <that session's topic>" (e.g. "Autograded quiz: while loops") with the cadence's kind. When the source states TWO OR MORE cadences ("weekly autograded quizzes and hands-on coding labs"), each cadence gets its OWN entry per session — NEVER glue two artifacts into one title (WRONG: "Autograded quiz: while loops 2. Hands-on coding lab: while loops"; RIGHT: two entries, "Autograded quiz: while loops" and "Hands-on coding lab: while loops"). Expanding a stated cadence is transcription of the assessment PLAN, not invention; one-off named titles stay verbatim under rule 2. The full assessments[] array must cover the whole plan: most courses carry at least one entry per teaching session, so if your array has fewer entries than sessions, re-read the source for a stated cadence before returning.
