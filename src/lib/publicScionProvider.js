...
- The template pre-fills ty from the requested lesson title and form. Copy that ty exactly; do not rewrite it. Simulation stays simulation, lab stays lab, studio critique stays studio critique, case exercise stays case exercise, structured debate stays structured debate, field exercise stays field exercise, and role-play stays role-play. Never substitute one form for another or return generic labels such as "Simulation and decision-making", "Case Exercise", "Lab", "Critique", or "Role-Play".
- Write sc as 2-3 complete sentences containing ONLY the initial situation, the first decision/action, and its current constraint. Never mention, paraphrase, or foreshadow later, new, synchronized, evolving, or updated evidence in sc. Bad sc: "Participants revise the response after synchronized evidence updates." Good sc: "Two delegations dispute a patrol collision while a civilian convoy approaches the corridor."
- The empty strings in the JSON template are intentional. Author every value from the lesson. Give every up item a different, specific ti title. In each up item, in must state the actual new evidence or changed condition—not merely say that evidence changed. For rd, choose exactly one observable post-update action permitted by the response schema enum; the compiler grounds artifact references in the visible product title.
- For ar.ti, choose exactly one inspectable product form permitted by the response schema enum. The compiler adds the course-specific subject to the visible title.
