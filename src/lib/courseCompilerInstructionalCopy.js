...
export function constructedResponseRelationshipSampleCopy({
  ...
  return `${definitions} ${selectVariant(lessonNumber, [
    ...
    `Evidence for ${conceptA}: ${firstEvidence || `one relevant course detail`}. Evidence for ${conceptB}: ${secondEvidence || `one different course detail`}. Reading the facts together clarifies the relationship, but the pair still does not establish that the concepts are interchangeable.`,
