...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.67 Presents a Clean, Downloadable Physics Package',
  highlights: [
    'The attached V0.16.66 Physics ZIP and console log are reproduced as a real compiler failure, not dismissed as model variance: the package graded 74/C because a Lesson 5 fallback rubric leaked into Lesson 3, “Midterm Examination II” was misclassified as a graded artifact, and one cumulative exam repeated the same definition stem twelve times.',
    'Lesson-aware export slicing now trusts explicit lesson identity before array position, every compiler and fallback rubric carries its lesson number, and native in-class checks no longer invent rubric obligations from generic words such as “analysis.”',
    'The assessment registry now recognizes exam and examination consistently. “Midterm Examination II (25%)” routes to the Quiz & Exam Bank as a real exam instead of becoming a blocking no-brief handoff in Assignment Briefs.',
    'Cumulative definition questions keep the same admitted concept, keyed answer, and evidence demand while rotating six visible stems and use statements, eliminating the twelve-fold rendered-text warning without weakening assessment content.',
    'Prerequisite primers render once per lesson and never print “Electric current: Electric current…” or “Gauss’s law: Gauss’s law…”. The exact enabled/disabled DOCX regression proves the old echo and the clean rendered result.',
    'A fresh exact Physics browser run completes 15/15 lessons, 15/15 knowledge kernels, 9/9 material families, 38/38 export checks, and Grade 99/A in 59.9 seconds with three model calls, zero blockers, zero warnings, zero P0/P1/P2 findings, and a working 2.32 MB ZIP containing 123 graded course files.',
    'The release passes 432 unit test files and 5,274 tests, focused causal regressions, formatting, lint, production build, locked bundle budgets, independent ZIP regrading, and final DOM inspection with no blocker, warning, draft-ZIP, failure, or error language.',
    'Gemma weights remain unchanged and the research adapter remains inactive. The measured lift is in the model-neutral Scion compiler, grader, and export boundary, so compatible paid-model routes inherit these fixes while browser download, WebGPU inference, and local caching remain Scion-specific.',
  ],
  landingHighlights: [
    'Wrong-lesson rubric content can no longer leak through array position.',
    '“Midterm Examination” is recognized and exported as a real exam.',
    'Cumulative exams vary their visible question framing without changing the key.',
    'Cited prerequisite definitions render once, without X:X echoes.',
    'The exact 15-week Physics run reaches 99/A with zero findings.',
    'The 2.32 MB package downloads and independently regrades cleanly.',
    'The full suite passes 5,274 tests and every release gate.',
    'Gemma weights are unchanged and the research adapter remains inactive.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.67.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01667_PHYSICS_ZERO_BLOCKER.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Reproduce the blocker from the files the user received',
      icon: 'PROOF',
      color: 'violet',
      items: [
        'The original 2.32 MB Physics archive contained 123 graded course files, passed physical export checks, and still graded 74/C with one P0 blocker. The audit followed the rendered DOCX text and manifest registry rather than relying on the workspace banner.',
        'Lesson 3’s rubric file opened with Lesson 5 current-and-resistance criteria. Midterm Examination II was registered as a graded artifact but compiled into an assignment no-brief shell. The cumulative exam repeated one definition question frame twelve times.',
        'Those were three independent product defects: lesson slicing, exam classification, and rendered exam wording. No model retry could reliably repair any of them.',
      ],
    },
    {
      label: 'Make lesson identity explicit at every rubric boundary',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'Lesson-aware array filtering resolves explicit lessonNumber, ln, sourceLessonNumber, lesson title, week, due week, and related-lesson fields before it ever considers positional fallback.',
        'Compiler-owned and fallback rubrics now carry lessonNumber. Native in-class checks and bounded evidence checks are recognized as formative shapes instead of generating an unrelated rubric because their prose contains “analysis.”',
        'Package-level regressions prove a labeled Lesson 5 rubric cannot enter Lesson 3 and that the same item still reaches Lesson 5.',
      ],
    },
    {
      label: 'Treat examination as exam everywhere',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The canonical assessment classifier accepts exam and examination while preserving review-guide exclusions. The full graph-to-blueprint path now compiles Midterm Examination II as a real exam paper.',
        'Its manifest record is kind exam and points to Quiz & Exam Bank/Lesson 15 - Course Synthesis - Quiz & Exam Bank.docx. Assignment Briefs retains only an honest lesson handoff, not a promised graded artifact.',
      ],
    },
    {
      label: 'Remove mechanical repetition at the rendered boundary',
      icon: 'LIVE',
      color: 'violet',
      items: [
        'Cumulative definition items rotate six stem and intended-use frames while keeping concept, authored definition, answer key, and distractor rationale unchanged.',
        'The before arm triggers the same twelve-repeat export audit; the enabled arm stays below the threshold and retains at least six distinct multiple-choice stems.',
        'DOCX prerequisite rendering bolds the leading term inside its definition instead of prepending the term again. Duplicate dependency edges are collapsed at the document boundary.',
      ],
    },
    {
      label: 'Prove the exact course is green and downloadable',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The rebuilt production bundle completes the exact 15-week calculus-based Physics brief in 59.9 seconds with three browser-local model calls, 15/15 lessons, 15/15 kernels, and 9/9 material families.',
        'The finish digest records ready, zero blockers, zero warnings, Grade 99/A, zero P0/P1/P2, and 38/38 export checks. The final DOM contains Ready to export, Verified · Grade A, and Download ZIP with none of the blocked-state language.',
        'The downloaded 2.32 MB ZIP independently extracts to 123 graded files plus manifest and reports. A fresh offline grade returns the same 99/A and zero findings.',
      ],
    },
    {
      label: 'Close every local release gate without widening budgets',
      icon: 'PROOF',
      color: 'blue',
      items: [
        'The complete suite passes 432 files and 5,274 tests. The focused six-file causal set passes 108 tests, including real DOCX extraction and package slicing.',
        'Format, lint, production build, and locked bundle budgets pass. Exam copy was moved into the existing compile-only polish boundary so the main compiler remains below its 830 KiB ratchet without raising a ceiling.',
      ],
    },
    {
      ...
      color: 'emerald',
      items: [
        'The pinned public Gemma 4 E2B weights are unchanged and the research adapter remains inactive. This release changes classification, deterministic compilation, rendering, grading behavior, and tests.',
        'Lesson identity, assessment routing, repetition control, readiness, grading, and export are model-neutral; compatible paid-provider routes benefit from the same fixes. WebGPU execution, model download, caching, and browser-runtime recovery remain Scion-specific.',
...
export const HISTORICAL_RELEASE_CHANGELOGS = [
  ...
  },
  {
    ...
    highlights: [
      ...
      'Archive export no longer inherits an unrelated publish-review lock: verified drafts remain downloadable with their review notes, and the real Chromium export suite passes all 25 ZIP, DOCX, CSV, PPTX, scope, repair, and blocking scenarios.',
  ...
  },
  {
    ...
    highlights: [
      ...
      'Publish readiness and archive exportability are separate promises: review-blocked content stays blocked for publishing while a physically verified package exposes Download draft ZIP with its notes intact.',
