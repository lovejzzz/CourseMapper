...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Opens Both Readings Together',
  highlights: [
    'Post-judgment ingestion now requires exactly two distinct sealed envelopes and two distinct keys. A single order, duplicate path, duplicate envelope identity, reused key identity, swapped key, or malformed key fails before the organizer can derive a preference.',
    'Both AES-256-GCM envelopes are authenticated and decrypted only in memory. Their packet, prompt, judge, order, session, source context, artifact, scorecard, decision, and plaintext hashes must all validate before corpus or report output is touched; neither completed review pass is written to disk.',
    'The dual-order bridge accepts exactly one A/B and one B/A reading from distinct fresh sessions. Stable ties, insufficient evidence, low winner scores, non-positive margins, missing defects, changed bytes, and side disagreement remain quarantined instead of becoming synthetic training wins.',
    'Only stable score-qualified agreements become unblinded chosen/rejected training rows. The derived corpus binds both pass hashes, all four scorecard hashes, source and packet identities, exact training-pair bytes, minimum winner scores, margin, and concrete losing-side defects as single-model Codex evidence.',
    'The real B/A judgment is still missing, so v0.16.20 ingests no outcome and creates no preference, dataset row, adapter weight, held-out win, device result, or paid-reference parity claim. Public Scion remains the pinned browser-local Gemma base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'Both sealed readings must arrive together.',
    'Each envelope and key must have a distinct identity.',
    'Completed review plaintext never touches disk.',
    'Ties, order effects, and weak wins stay quarantined.',
    'The bridge is ready; the second judgment is still missing.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.20.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Open both sealed readings together',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The sealed ingestion command requires two distinct envelope paths and two distinct key paths.',
        'Canonical key decoding, key hashes, ciphertext hashes, GCM authentication, and plaintext hashes validate before analysis.',
        'Envelope metadata must match the decrypted packet, judge, order, session, prompt, and review count exactly.',
        'A bad or missing second input fails before existing corpus or organizer-report bytes are changed.',
      ],
    },
    {
      label: 'Derive preferences without pass plaintext',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Both decrypted batches stay in memory while the existing reverse-order validator checks every source, artifact, scorecard, and decision.',
        'A/B and B/A must use the same exact judge identity but distinct fresh session IDs and agree on the anonymous winner.',
        'Ties, insufficient evidence, low-quality winners, non-positive margins, and position-sensitive outcomes remain quarantined.',
        'Only derived chosen/rejected rows and a non-outcome organizer report may be written; completed pass plaintext is never emitted.',
        'The command reports counts, corpus path, report path, combined disclosure, and plaintextWritten=false.',
        'No real B/A pass exists yet, so approved preferences, quality rows, adapters, wins, and paid-reference parity remain zero.',
        'The strict layered release evaluator remains compiler-contract-only; secure ingestion is not classroom or independent evidence.',
      ],
    },
  ],
};
