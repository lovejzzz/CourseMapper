...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.50 Reads More Carefully and Exports What It Proves',
  highlights: [
  ...
  landingHighlights: [
    'Source-strict V4 catches 78/78 frozen losses with zero preferred-artifact regressions.',
    'Only two of 192 retained local source atoms newly enter retry; both are reviewed semantic failures.',
    'Custom deliverables export the learning plan without leaking compiler metadata.',
    'Reload restores the exact terminal grade, and Scion Agent replies render as clean language.',
    'Role-aware grading removes false structure findings while preserving the real source-trust note.',
    'The real browser-local audit finished at 99/A and verified the exact twenty-two-file ZIP.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.50.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Turn every judged loss into a bounded semantic rule',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Source-strict V4 adds nine narrow, source-bound checks learned from previously stable losses. It rejects a known truth presented as a misconception, a correction that answers with an unrelated true predicate, a correction that drops the technical identifier under dispute, and examples whose timing contradicts the cited rule.',
        'Additional guards preserve implicit-versus-explicit contrast, distinguish a research team’s learning goal from a participant learning objective, require an interactive prototype definition to include behavior as well as appearance, and retain a source-backed naming identity in the correction.',
        'The exact frozen replay now detects 78/78 losing artifacts while preserving 78/78 preferred counterparts. This is deterministic interception evidence, not a claim that the compiler generated the preferred prose.',
        'Across the ninety-one-pair two-arm comparison, no eligibility changes. Across 192 retained local source atoms, only two newly enter bounded retry, and reviewers had already identified both as semantic failures.',
      ],
    },
    {
      label: 'Compile custom work without exposing the compiler',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'A user-facing identity classifier runs before incidental prompt wording, so “Trip plan for study” no longer falls into a generic feedback-form shape merely because its longer description mentions review.',
        'The study-trip-plan family projects learning purpose, before-visit checklist, field evidence tasks, evidence-capture options, return-to-course work, and logistics to confirm. Its document is compact enough to use in the field.',
        'A shared recursive metadata boundary removes sourceGrounding, compilerTrustReceipt, blueprintQualityReceipt, localReviewNote, and related internal fields from both workspace rendering and export serialization.',
        'The exact final custom DOCX contains the learner plan and no internal review marker. That privacy boundary applies to future custom families through one shared policy.',
      ],
    },
    {
      label: 'Remember terminal truth and let Scion speak cleanly',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Full local and portable project snapshots persist terminal quality evidence only when the pass is complete, digest-bound, and actually ready or blocked. Running, idle, and unverified failure state cannot masquerade as saved proof.',
        'Reloading the audited package restores 99/A, texture 95, one finding, the completed digest, and 100% Ready to export instead of dropping the Living Course Compiler to an indefinite 66% waiting state.',
        'The built-in Agent calls the same local Scion runtime for advisory conversation. A real one-sentence course-summary request returned a relevant course-specific answer in about eleven seconds.',
        'Nested response parsing now selects the semantic object from a mixed punctuation-plus-object array. The repaired UI shows the sentence, not `[\")\", {\"chatReply\": ...}]`.',
      ],
    },
    {
      label: 'Fix the ruler without hiding the remaining evidence',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The manifest structure grader now counts a custom artifact only in its own feature family. One custom file no longer creates six false “three declared, two present” findings across lesson plans, decks, quizzes, study guides, discussions, and assignments.',
        'Registry-linked assignment briefs defer weight totals to the course assessment registry; quiz answer-key handoffs do not pretend to be analytic rubrics; source-strict study guides can remain at two supported terms rather than inventing a third.',
        'A course with zero genome-linked lessons writes `judgment: not evaluated (0 genome-linked lessons)` into its evidence instead of silently omitting the judgment layer.',
        'The same package moved from 89/B and eight findings to 99/A and one finding without changing learner prose. The remaining citation note stays visible because the supplied reading’s bibliography trust and license were not verified.',
        'Because the grader implementation changed, the adapter firewall refused the old transitive hash. V4 preserves the same five held-out course identities while freezing grader v1.10.16 and its complete eleven-file implementation receipt before any new candidate is evaluated.',
      ],
    },
    {
      label: 'Inspect every handoff and verify the file users receive',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The real browser-local Scion run followed the 3.35 GB model download and every observed Model, Map, Enrich, Compile, Verify, and Grade transition to a terminal 624-second result.',
        'Phone at 390×844, tablet at 768×1024, and desktop passes covered landing, previous-session resume, content, Agent, finish, grade, and export. The tablet export card now receives a centered readable measure instead of the obsolete 200px side rail.',
        'Nonblocking notes use a calm blue information treatment, routine review spot-checks are counted separately from real decisions, and the package panel remains the single Download ZIP owner.',
        'The final 349,627-byte archive contains twenty-two files, passes ZIP integrity, embeds 99/A with texture 95 and one P2 note, and is bound to SHA-256 f0e18c0c7c14d8613c8d9f87a090200538eca68c48b2289060c61034de4d3290.',
        'No adapter is active, no Gemma weight changed, and no universal, human, instructor, classroom, or paid-reference parity claim is made. The shared compiler improvements can help other providers without making every provider output equally good.',
      ],
    },
  ],
};
