...
export const CURRENT_RELEASE = {
  ...
  highlights: [
    ...
    'The training launcher no longer invokes MLX-VLM’s unseeded, validation-free CLI directly. A narrow Scion runner seeds NumPy shuffling and MLX LoRA initialization, injects the real validation split, uses a physical batch of one with two-step accumulation and activation checkpointing for Metal memory, records every ORPO parameter explicitly, and derives the adapter ID from the canonical run identity.',
    'Training plans and completion receipts bind the clean Git commit and tree, exact base revision, dataset and toolchain identities, seed, command, adapter files, and retained local log digest. Two real same-commit runs in separate output roots produced identical validation metrics and byte-identical 105.46 MB weights. Manifest schema v3 requires that chain for learned adapters and carries it through GGUF conversion.',
    'This release proves deterministic mechanics with permanently non-promotable smoke weights; it does not complete the fresh B/A judgment, create approved preferences, train quality weights, activate an adapter on the public site, or claim a model win. Hosted Scion remains base-only plus compiler.',
  ],
  landingHighlights: [
    'The exact dataset, source bytes, and split identity are recorded.',
    'NumPy shuffling and MLX LoRA initialization use one declared seed.',
    'Python, packages, and trainer source bytes are revision-pinned.',
    'MLX training receipts survive separate browser GGUF conversion.',
    'No quality weights, judgment, or model-quality win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.31.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Run identity before weights',
      icon: 'AI',
      color: 'emerald',
      items: [
    ...
    {
      label: 'Seeded training through browser conversion',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'MLX-VLM 0.6.3 does not expose a seed flag even though it uses NumPy permutation and MLX random initialization, and its CLI hard-codes validation to None. The Scion wrapper sets both generators and injects the manifest-bound validation split before the trainer runs. The launcher uses physical batch one, two-step gradient accumulation, and activation checkpointing to preserve the effective batch while bounding Metal memory, and supplies every parameter explicitly rather than inheriting changing library defaults.',
        'A completion receipt binds the plan, adapter configuration, weight bytes, completion time, and a digest of the locally retained training log. Two real runs from one clean commit into separate external roots reproduced the same plan identity, metrics, configuration bytes, and 105,459,677-byte weight SHA-256; their timestamp-bearing receipts and raw logs correctly differ. Raw logs and weights stay outside Git.',
        'Adapter manifest schema v3 requires direct training receipts for research and candidate MLX packages. A separately converted browser GGUF copies the plan, result, and source-adapter manifest, while its conversion receipt binds the verified MLX source and converted GGUF bytes.',
        'Legacy schema-v2 smoke and rejected packages remain admissible only as mechanical history. They cannot be relabeled research, candidate, or promoted, and a changed receipt, source manifest, output weight, dataset split, toolchain, symlink, dirty tree, or arbitrary adapter ID fails closed.',
        'This is a mechanics reproducibility result, not a quality result. Its 76 structural rows include zero approved single-model-judge preferences, so the smoke weights are permanently non-promotable and inactive. The fresh reverse-order judgment, stable approved preferences, research weights, five-domain adapter win, paid-reference parity, and three remaining device profiles are still missing.',
      ],
    },
  ],
};
