...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.60 Teaches From Its Measured Gaps',
  highlights: [
  ...
  landingHighlights: [
    'Source-only repair turns measured defects into stronger lesson kernels.',
    'The compiler distinguishes negated distractors from genuinely supported answers.',
    'Five diagnosed domain gaps become five clean paired-order preferences.',
    'A balanced fourteen-case wave covers every domain and failure family.',
    'Four-way reference capture is faster and remains atomically resumable.',
    'Thirteen of fourteen expansion cases earn training qualification.',
    'The source-ledger ruler reaches 20 of 100 required full lessons.',
    'Gemma weights remain unchanged and no quality adapter is active.',
    'Course names, phone previews, lesson clocks, and every export path are replayed.',
    'The failed CI receipt is rebound and the next held-out ruler is frozen.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.60.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01660_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Repair only from supplied evidence',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'Each teacher session sees only its immutable source packet, anonymous diagnoses, prompt, and output schema in an ephemeral read-only cleanroom.',
        'Per-case schemas pin the exact lesson id, original artifact hash, and every numbered source fact. Compiler admission runs again before any candidate can reach judgment.',
        'The first pass repaired five semantic gaps; a narrow second pass removed one unsupported named geology example. All five ultimately qualified with zero winner critical defects.',
      ],
    },
    {
      label: 'Make the compiler precise as well as strict',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'A correction such as “the lower layer is younger reverses the relationship” is no longer misread as affirmative support for the rejected option.',
        'Non-exact lexical support must preserve semantic-token order, preventing scattered words about gross investment, capital stock, and change from inventing a second relation.',
        'The explicit two-answer regression remains red when one explanation genuinely endorses two alternatives.',
      ],
    },
    {
      label: 'Scale evidence without lowering the bar',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'The defect-directed selector chooses fourteen unseen lessons with two per domain, fourteen distinct course groups and source kernels, and all nine failure families.',
        'Four-way reference capture overlaps isolated Codex sessions while a serialized atomic write queue keeps the checkpoint resumable. The single shared local runtime remains intentionally serial.',
        'Seven cases qualify directly. Six stable-but-ineligible winners receive targeted repair; all six later qualify. One order-unstable pair is excluded without repair spend.',
      ],
    },
    {
      label: 'Grow a task-matched ruler',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'Five initial repaired rows plus thirteen expansion rows join the two V0.16.59 rows: twenty unique full-lesson preferences across all seven training domains.',
        'Every accepted row preserves its exact source ledger, passes current compiler admission, wins both anonymous presentation orders, clears the score floor, and carries zero winner critical defects.',
        'The receipts bind selection, raw captures, teacher packets and results, compiler reports, order-specific packets and reviews, training rows, implementation hashes, and honest claim boundaries.',
      ],
    },
    {
      label: 'Keep the promotion boundary closed',
      icon: 'CHECK',
      color: 'amber',
      items: [
        'The twenty rows are only 20% of the minimum corpus gate. They are same-identity model-teacher and model-judge evidence from separate sessions, not human or independent validation.',
        'No adapter has been trained on these rows, no candidate has faced the frozen V10 held-out benchmark, and no download, memory, or runtime burden changed for website users.',
        'Hosted Scion remains the pinned public Gemma 4 E2B base plus the model-neutral compiler. Compatible paid routes benefit from the compiler precision fix too.',
      ],
    },
    {
      label: 'Make polished frames tell the truth',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Setup preview keeps meaningful title connectors: “Spanish for Healthcare Professionals” and “Writing with AI” no longer collapse to “Spanish” or “Writing.”',
        'The deep grader accepts min, mins, minute, and minutes in exported session outlines. It no longer turns visible, valid timing rows into a false zero-minute P0.',
        'The repaired desktop and 390×844 flows pass full-screen preview, current-tab formats, selected-material ZIP contents, package scope, and project backup downloads.',
      ],
    },
    {
      label: 'Close the verification chain',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'GitHub run 29694477954 passed format, lint, and the release ledger, then exposed one stale source-compiler replay receipt before later checks could run.',
        'Rebuilding from retained response bytes keeps the aggregate result at 121 admitted atoms, 71 retry seats, 10 source-grounded key repairs, and zero response mutations while removing repaired artifacts from model-training eligibility.',
        'Grader 1.10.20 and its twelve-file implementation receipt are frozen as held-out ruler V10 before any adapter comparison; no V9 score or promotion decision is inherited.',
      ],
    },
  ],
};
