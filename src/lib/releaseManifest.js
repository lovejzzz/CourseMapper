...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.77 Compiles Experiential Learning End to End',
  highlights: [
    'Lessons that explicitly request a simulation, laboratory investigation, studio critique, case exercise, structured debate, field exercise, or role-play now receive one canonical experiential-activity blueprint inside the existing lesson-authoring call—not a second model pass per material.',
    'A lesson-local selector preserves the exact matching activity clause from the instructor brief, while the template seeds only a topic-and-form label from that lesson. This prevents a simulation from silently becoming a case exercise without hard-coding any scenario; the strict contract rejects generic, incomplete, mismatched, and mistimed payloads.',
    'The compiler normalizes the model-authored phase weights once to the exact course session length. Lesson plans, slide frames, the student activity packet, the in-app preview, DOCX output, and ZIP export all consume that same clock and retain the same admitted activity content.',
    'The compiler owns reusable mechanics—accessible participation modes, activity logs, timing normalization, and safe evidence handling—but contains no fixed lesson actors, facts, discipline story, or hard-coded activity scenario. If activity admission fails, it compiles an honest standard lesson and never labels a hollow substitute as a complete activity.',
    'Cross-domain proof covers an international-relations negotiation, an engineering lab, a UX studio critique, a counseling role-play, and a transit-policy case at 50, 60, 75, 90, and 120 minutes. Every accepted blueprint preserves its authored atoms across compiler surfaces and every clock closes exactly.',
    'The deep-quality grader now treats experiential activities as runnable contracts: a title alone cannot pass without roles and constraints, inspectable evidence, evolving phases, required action, a named artifact, structured debrief, boundary, and visible clock.',
    'The production compiler now enforces a stricter learner-facing contract after admission: initial-only scenarios, genuinely new synchronized updates, a closed course-grounded product vocabulary, distinct role constraints, and inspectable requirements for the initial evidence-bound decision, update-responsive revision, and unresolved uncertainty plus next evidence check. It replaces generic assignment shells and duplicate role constraints deterministically without inventing disciplinary facts or adding model calls.',
    'The local evaluation router now recognizes the production experiential prompt instead of dropping activity authoring into the legacy route. The final five-lesson International Crisis Bargaining browser run completed in 103.667 seconds with 12 provider requests, ten task calls, and eight pipeline calls, 5/5 admitted kernels, 9/9 compiled material families, 99/A with Texture 96, zero scored findings, blockers, warnings, retries, failed calls, or warning/error console entries, and 38/38 passing export checks.',
    'Frame-by-frame proof covers 26 states from landing through model readiness, every compiler interval, package readiness, and download. The 746,980-byte ZIP contains 45 files, passes compressed-data testing, and is bound to SHA-256 ffa86abc7460ed2ef1cc5eaaaebb7f8f1372389378067f54d5a146a3ea7a0eac. Rendered export inspection also removed a blank assignment tail page and an orphaned UDL page: the final assignment uses four pages, the lesson plan uses three balanced pages, and the four-slide deck passes overflow testing.',
    'Gemma weights remain unchanged and the research adapter remains inactive. The canonical activity IR, admission, compiler projections, grading, and export path are model-neutral and benefit compatible paid providers; browser download, WebGPU inference, local cache, and public-model routing remain Scion-specific. The complete release gate passes 450 unit-test files and 5,571 tests, 151/151 Chromium E2E tests, the 40/40 compiler contract, build, bundle, release, live-probe, ZIP, and rendered-artifact checks.',
  ],
  landingHighlights: [
    'One course-specific activity blueprint drives every material.',
    'Only explicitly requested experiential lessons invoke the contract.',
    'Five unrelated activity domains pass the same strict admission gate.',
    'Every activity clock fits the actual class length exactly.',
    'Lesson plans, slides, packets, preview, DOCX, and ZIP stay aligned.',
    'Generic or incomplete activity text fails closed.',
    'Compatible paid models inherit the same compiler capability.',
    'The adapter stays inactive and Gemma weights stay unchanged.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.77.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01677_EXPERIENTIAL_ACTIVITY_ACCEPTANCE.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Author the activity once',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'Explicit experiential language selects qualifying lessons before lesson authoring begins.',
        'The existing lesson-kernel response receives one compact top-level activity blueprint for each qualifying lesson.',
        'Ordinary lesson prompts remain byte-identical, preserving prompt-cache behavior and historical evaluation receipts.',
        'No per-deliverable model call, hidden rewrite loop, or fixed activity story is added.',
      ],
    },
    {
      label: 'Admit only runnable content',
      icon: 'PROOF',
      color: 'blue',
      items: [
        'The canonical IR requires a specific situation, roles and constraints, inspectable evidence, updates, actions, artifact, debrief, boundary, and timing.',
        "Only the matching activity clause from the instructor's source brief reaches that lesson; unrelated course instructions stay outside the kernel.",
        'A source-derived activity label preserves the exact requested form on the first call; the model still authors every substantive activity atom.',
        'Schema and semantic admission reject missing, placeholder, meta, duplicate, out-of-scope, overfilled, ungrounded, mode-mismatched, and mistyped content.',
        'The public Scion admission gate and the paid-provider kernel parser enforce the same semantic contract.',
        'A missing or rejected blueprint produces an explicit standard-lesson fallback rather than invented domain content.',
      ],
    },
    {
      label: 'Compile one coherent experience',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'One largest-remainder normalization fits the authored phase weights to the actual session duration.',
        'Lesson-plan segments and slide frames retain the same phase names and minutes instead of re-normalizing independently.',
        'The student packet carries roles, evidence, phases, log fields, artifact requirements, debrief, and boundary as one assignment surface.',
        'The compiler repairs duplicate role constraints and generic assignment shells into role-specific, activity-native language without adding domain facts.',
        'Artifact requirements preserve the initial evidence-bound decision, the update-responsive revision, and one unresolved uncertainty with its next evidence check.',
        'DOCX, ZIP, preview, and deep grader all read the canonical activity packet.',
      ],
    },
    {
      label: 'Prove generality',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'International relations, engineering, UX, counseling, and business-policy fixtures use unrelated scenarios and artifacts.',
        'The same contract accepts stakeholder roles, functional lab roles, critique roles, and bounded clinical roles without forcing one teaching pattern.',
        'Session clocks of 50, 60, 75, 90, and 120 minutes close exactly across lesson-plan and slide projections.',
        'Hollow title-only substitutes fail the score-bearing deep-quality check in every domain.',
      ],
    },
    {
      label: 'Make the browser surface usable',
      icon: 'PROOF',
      color: 'blue',
      items: [
        'The assignment view lazy-loads a responsive activity briefing rather than inflating the landing route.',
        'Evidence, roles, role-only information, updates, timing, activity log, artifact, and debrief use distinct readable hierarchy.',
        'The preview is editable through the same path-based editing boundary as the rest of the assignment.',
        'One combined assignment packet prevents a second activity document from appearing for the same lesson.',
        'A 26-frame Chromium filmstrip verifies the visible journey, and rendered DOCX/PPTX inspection verifies the exported learner-facing hierarchy.',
      ],
    },
    {
      ...
      items: [
        ...
        'The model authors course-specific meaning; the compiler adds mechanics, normalization, accessibility support, and projection only.',
        'Compatible providers inherit the same canonical IR and compiler while Scion alone owns the public browser-local runtime.',
        'This proves a reusable activity architecture and encoded artifacts—not universal factual correctness, instructor approval, classroom outcomes, adapter superiority, or paid-reference parity.',
      ],
    },
  ],
};
