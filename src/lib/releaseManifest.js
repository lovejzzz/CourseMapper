...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.59 Keeps the Instructor’s Facts in Charge',
  highlights: [
  ...
  landingHighlights: [
    'Exact instructor facts survive unsafe model-authored teaching atoms.',
    'Equivalent, ambiguous, multi-supported, and overclaimed choices are quarantined.',
    'Fallback questions use the supplied fact list without inventing source material.',
    'A cached strict-source lesson completes the full compiler path in 22 seconds.',
    'The local Agent returns concise prose instead of internal tool envelopes.',
    'Two full lesson-kernel preferences pass the current training gate.',
    'The paid reference still wins all seven fresh domain comparisons.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.59.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01659_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Keep the source ledger authoritative',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'An explicit three-to-five-fact instructor-only brief becomes a numbered, ordered source contract for the browser model and canonical compiler.',
        'If every generated key term and multiple-choice atom fails admission, the exact supplied facts remain available as knowledge rather than being discarded with unsafe model text.',
        'The compiler refuses unsupported scope markers and generates bounded fact-list questions that never imply a worked example, solution, or source case exists when it does not.',
      ],
    },
    {
      label: 'Make both admission boundaries agree',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'The early Scion gate and canonical compiler now share checks for near duplicates, equivalent equations, inverse comparisons, unsupported scope, and explanations that support multiple options.',
        'Ordinal answer-position leakage catches phrases such as “third and fourth options,” not just a single ordinal reference.',
        'These checks are conservative rejection rules: they may quarantine unsupported text but never synthesize a fact to force admission.',
      ],
    },
    {
      label: 'Spend local inference where it can help',
      icon: 'RECOVER',
      color: 'blue',
      items: [
        'A difficult three-lesson source-only run spent 441 seconds across local retries and outer recovery yet admitted zero of three kernels. Fresh seven-domain evidence also admitted zero of seven base artifacts.',
        'Exact-ledger builds now stop after the bounded local attempt set, skip outer enrichment recovery, preserve compiler-owned facts, and expose review notes for rejected pedagogy.',
        'A cached one-lesson replay completed in 22 seconds with one exact knowledge set, nine of nine material families, an honest 89/B grade, and two visible review notes.',
      ],
    },
    {
      label: 'Keep the Agent connected and legible',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'Public Scion Agent chat receives only the dynamic workspace context plus a compact read-only contract, rather than the complete mutation-tool protocol.',
        'Replies are limited to 240 output tokens and explicitly forbid JSON, function-call envelopes, hidden analysis, or false claims of workspace edits.',
        'In the live replay, an exact fact lookup produced clean grounded prose in about 70 seconds; the previous longer attempt continued into a malformed internal tool envelope after more than two minutes.',
        'Desktop and 390×844 phone frames retain one ZIP owner, distinct Content/Agent/Export modes, internally scrollable tables, calm blue ready-with-notes language, and 44px phone targets for the quality seal and lesson-collapse control.',
      ],
    },
    {
      label: 'Turn the measured gap into training evidence',
      icon: 'PROOF',
      color: 'amber',
      items: [
        'Seven fresh domain pairs were judged anonymously in both presentation orders. The paid reference won all seven; local base admitted 0/7 and reference admitted 2/7 under the final current compiler.',
        'Computer Science and User Experience Design become the first two score-qualified full lesson-kernel preference rows; the final gate correctly removes a previously qualified Physics row with an unsupported exclusivity distractor.',
        'The frozen semantic replay still detects 78/78 stable losing artifacts with zero preferred regressions. No adapter is active, no weight changed, and no base or paid-reference victory is claimed.',
      ],
    },
  ],
};
