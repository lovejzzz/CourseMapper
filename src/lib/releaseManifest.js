...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.49 Turns a Live Build into an Auditable Course',
  highlights: [
    'The Living Course Compiler now represents the complete build instead of only model download: Model, Map, Enrich, Compile, Verify, and Grade advance from real state, stay monotonic, narrate recovery, and reserve 100% for a terminal ready or review outcome.',
    'The workspace now uses one coherent visual language: successful packages are green instead of warning-yellow, the duplicate header ZIP action is gone, slide visuals and concept maps are visible in context, and the built-in Agent can execute a direct Scion slide improvement without exposing pseudo-tool JSON.',
    'A source-trust boundary rejects attractive false friends before compilation and preserves complete sourceRef coverage. The audited music package uses two exact Open Music Theory interval resources with recorded provider, license, concepts, activities, assessments, rubric criteria, and factual claims.',
    'Music-theory compilation now produces disciplinary work across every selected surface: interval classification and inversion lessons have distinct slides, practice, discussion facilitation, FAQs, assessment frames, study terms, rubrics, course-map actions, and observable evidence rather than renamed generic templates.',
    'Manual Scion lesson regeneration now stays on the deterministic compiler path. A reload during regeneration normalizes stale streaming state, restores saved content as ready, clears the abandoned regeneration marker, and preserves the exact enriched-lesson provenance used by final grading.',
    'Texture grading now ignores only export chrome such as document titles, page footers, and question metadata while still measuring learner-facing prose. After content variation and that measurement repair, the live two-lesson package reached 100/A, texture 98, and zero findings.',
    'The exact exported package passed an independent extracted-file regrade at 100/A with zero P0, P1, or P2 findings; all twenty-one ZIP entries passed integrity and expected-folder checks. No adapter is active, no Gemma weight changed, and no paid-reference, instructor, or classroom parity is claimed.',
    'Judge-informed source-strict V3 detects sixty-eight of seventy-eight frozen stable losses—four more than V0.16.48—with zero preferred-artifact regressions. It records two additional issue incidences but adds zero retry burden across 192 retained local seats and zero retry seats across either arm of the ninety-one-pair frozen surface.',
  ],
  landingHighlights: [
    'The Living Course Compiler follows the whole build and reserves 100% for a verified terminal state.',
    'Refresh-safe Scion regeneration restores completed content instead of trapping the workspace in a false live state.',
    'Trusted interval sources remain traceable through every learner-facing artifact and the final manifest.',
    'Music lessons now differ in the thinking, practice, evidence, and facilitation they ask of learners.',
    'One final ZIP action, calm success semantics, native visuals, and direct Agent improvements reduce workspace friction.',
    'The audited two-lesson package independently regraded at 100/A with texture 98 and zero findings.',
    'Source-strict V3 catches 68/78 frozen losses without adding retry burden on the retained evaluation surfaces.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.49.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Let one honest progress surface own the entire build',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The ribbon begins with real model-byte progress, then transfers ownership to mapping, lesson enrichment, material compilation, deterministic verification, and grading. Requested lessons, token counts, and elapsed time cannot impersonate completed artifacts.',
        'Four live cards expose Course map, Knowledge, Materials, and Checks. Enrichment coverage is counted from exact lesson IDs; a handled fallback remains visible, and the terminal color distinguishes Ready from Ready with review notes.',
        'The duplicate header Download ZIP action is removed. Export remains in the package panel, next to the scope, readiness outcome, grade, and notes that determine whether downloading is responsible.',
        'Ready with notes no longer borrows the warning-yellow used for failures. Success, review, active work, and blocking failure now have separate color and language roles in both light and dark workspace states.',
      ],
    },
    {
      label: 'Make interruption and direct revision feel native',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'A real refresh during lesson-level regeneration exposed a stale persisted streaming flag. Restore now promotes saved streaming content to done, returns empty interrupted content to idle, and clears the abandoned regeneratingIndex instead of displaying “Writing content live” forever.',
        'Manual Scion regeneration uses the compiler directly and records the restored enrichment overlay. Regrading therefore sees the same exact enriched lesson IDs as the content surface instead of lowering a complete package for missing runtime-only provenance.',
        'The Agent is connected to Scion for advisory chat and bounded direct actions. “Improve slides” now changes the current deck through the compiler, and provider text that resembles a tool envelope cannot leak raw regenerate_slide_decks syntax into the conversation.',
        'Slide decks render native visual frames and concept maps inside the workspace. The frame-by-frame audit checked loading, regeneration, refresh recovery, ready state, dark mode, every deliverable tab, finalization, and the single export action.',
      ],
    },
    {
      label: 'Compile the subject instead of decorating a template',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'Interval classification and interval inversion no longer share a renamed discussion prompt. Each lesson now receives a distinct problem, facilitation move, response stem, criterion, accessibility protocol, and FAQ explanation tied to its own disciplinary reasoning.',
        'Lesson plans rotate catch-up structures instead of repeating one exact paragraph. Autograded quizzes retain the answer-key and no-partial-credit contract while varying how that contract is explained to instructors and learners.',
        'Slide frames teach notation, semitone evidence, compound-interval reduction, inversion sums, and complementary quality changes. Course maps, study guides, assignment briefs, discussion prompts, rubrics, quiz banks, and FAQs use the same concepts without collapsing into copied prose.',
        'Rubric rows name observable evidence such as interval number, quality, counted span, semitone check, inversion pair, and correction rationale. Learners can see what a successful analysis contains rather than being scored on generic “professional decisions.”',
      ],
    },
    {
      label: 'Keep citations attached to the concepts they support',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Source discovery now treats title and topic resemblance as a candidate, not proof. Known multivariate-classification and civil-service-classification false friends are quarantined from a music-interval course before they can supply terms or claims.',
        'The verified package uses the exact Open Music Theory Intervals chapter and its interval worksheet. Provider, URL, license, source reference, and concept linkage remain inspectable in the source ledger and exported package manifest.',
        'The final source graph covers four of four outcomes, four of four activities, two of two examples, two of two assessments, six of six rubric criteria, and four of four factual claims with sourceRef links.',
        'The trust registry recognizes the selected Open Music Theory provider and license without weakening the source-quality gate for unknown or merely similar pages.',
        'The frozen semantic replay now detects sixty-eight of seventy-eight stable losing artifacts, four more than V0.16.48, while every preferred counterpart still passes. The new issue signals overlap artifacts already rejected on the retained runtime surfaces, so measured retry burden remains unchanged across 192 local seats and both ninety-one-row comparison arms.',
      ],
    },
    {
      label: 'Grade learner-facing quality, then verify the exact ZIP',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The texture metric used to interpret structural lines such as “Q1: (Apply, Easy)” and repeated export footers as prose openers. It now strips only that metadata and continues to score the actual question stem, explanation, and instructional text.',
        'Content variation and the corrected measurement produced a live score of 100/A with texture 98, opener variety 93, sameness 99, tail quality 99, and zero findings across the selected two-lesson course.',
        'The exact downloaded ZIP was extracted outside the browser and graded again from its files. The independent result remained 100/A with zero P0, P1, or P2 findings across nineteen graded files.',
        'All twenty-one archive entries passed ZIP integrity, expected-folder, note-length, and slide-density checks. This proves this bounded package and workflow; it is not a claim that every prompt, domain, or model output will score identically.',
        'The browser audit used a local provider-compatible transport fixture for repeatable UI timing while exercising the production compiler, persistence, finalizer, grader, and exporter. It does not claim a new Gemma inference benchmark.',
        'No quality adapter is active. Scion V0.16.49 remains the immutable public Gemma 4 E2B base plus the model-neutral compiler, with optional adapter work gated on a held-out base-model win.',
      ],
    },
  ],
};
