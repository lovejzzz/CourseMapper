...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.56 Makes Every Lesson Earn Admission',
  highlights: [
    ...
    'Lesson-kernel admission now rejects unsupported numeric, malformed-LaTeX, spelled-out, and relative quantities; copied scenario scaffolds; duplicate or fragmentary facts; and increase/decrease relationships that reverse the supplied source.',
    'When local retries are exhausted, Scion retains the lowest-risk complete model-authored attempt instead of blindly returning a worse final retry, while preserving selected-attempt and unresolved-issue evidence.',
    'The 148-case, seven-domain lesson-kernel campaign rebuilds under a clean prompt policy that excludes evaluator-only quality-focus text from production model objectives.',
    'In a clean three-case Physics, Economics, and UX pilot, GPT-5.4-mini wins all three paired-order comparisons, but zero pairs become training rows: two winners retain critical judge defects and one fails compiler admission.',
    'A hash-bound evidence receipt packages campaign identities, raw base/reference captures, current-compiler replays, isolated reversed-order judgments, classifications, and the empty preference output.',
    'Two production-bundle browser builds verified the complete Scion workflow. The repaired one-lesson build completed in 61 seconds, kept the full Physics title from its first frame, and retained zero document-level overflow at a 390×844 viewport.',
    'Source-bound quiz recovery now asks objective-specific application, analysis, evaluation, and creation questions. The grader reports those recovered seats as a P1 knowledge limitation instead of implying that Scion produced a verified disciplinary answer key.',
    'No hosted adapter is active and no Gemma weight changed. V0.16.56 improves shared admission, compilation, recovery, and evaluation integrity without claiming aggregate model quality, paid parity, or a trained-model win.',
  ],
  landingHighlights: [
    'All 78 frozen judged losses are caught with zero preferred regressions.',
    'Unsupported quantities and reversed source relationships now fail closed.',
    'Exhausted retries keep the best complete attempt, not merely the last one.',
    'Evaluator notes can no longer leak into the 148-case production prompt.',
    'The clean pilot exposes the paid-reference gap without manufacturing data.',
    'Every capture, replay, judgment, and zero-row result is hash-bound.',
    'A real 61-second browser build keeps its full title and fits a 390×844 phone.',
    'Quiz recovery is objective-specific and visibly graded as a knowledge limitation.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.56.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01656_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Bind the current semantic ruler',
      icon: 'PROOF',
      color: 'blue',
      items: [
        'The V0.16.56 wrapper replays the immutable 78-pair judged atom corpus through the exact current provider, answer-alignment, key-term, and scenario implementation.',
        'All 78 stable losing artifacts enter repair or retry and all 78 preferred counterparts remain admitted. The receipt records the implementation hashes, source-corpus identity, issue families, and zero-regression result.',
        'This is a bounded frozen-corpus result. It does not establish unseen precision, aggregate course quality, or adapter improvement.',
      ],
    },
    {
      label: 'Reject source drift without rewriting it',
      icon: 'SOURCE',
      color: 'violet',
      items: [
        'Quantity grounding covers ordinary numbers and units, malformed JSON-decoded LaTeX quantities, spelled-out quantities, and relative claims such as doubled or halved.',
        'A narrow comparative-direction check matches the same input and output concepts against the best source relationship, rejecting an opposite sign without confusing plate area with plate separation.',
        'Facts must be distinct complete sentences. Exact and paraphrased placeholder scenarios such as source detail one/source detail two are refused before compilation.',
        'The compiler asks the model to re-author from the supplied source; it never silently replaces a factual claim with compiler-authored course content.',
      ],
    },
    {
      label: 'Keep the strongest honest retry',
      icon: 'RECOVER',
      color: 'emerald',
      items: [
        'Each complete local attempt receives an admission-risk score from critical and other unresolved issue families.',
        'If no attempt is admitted, the browser path and capture harness retain the lowest-risk complete model-authored response rather than overwriting it with the final retry.',
        'The result records selectedAttempt and every retained issue. Selection changes orchestration only; it does not splice, rewrite, or falsely admit model text.',
      ],
    },
    {
      label: 'Clean the production-matched campaign',
      icon: 'CAMPAIGN',
      color: 'blue',
      items: [
        'The campaign still spans 148 cases, 25 course groups, 72 source kernels, seven domains, and production-compatible licenses.',
        'Prompt policy V2 excludes evaluator-only quality-focus text from objectives. The campaign identity binds that exclusion and requires a fresh deterministic rebuild against the current production prompt compiler.',
        'Historical V0.16.54 artifacts remain internally verifiable but cannot masquerade as a current clean rebuild because their prompt policy included evaluator metadata.',
      ],
    },
    {
      label: 'Measure the gap without manufacturing a win',
      icon: 'PROOF',
      color: 'amber',
      items: [
        'The clean base capture admits 0/3 cases after six retries in 112,979 ms; the paid-reference capture admits 2/3 after five retries in 50,006 ms.',
        'One isolated A/B session and a distinct B/A session agree that GPT-5.4-mini wins Physics, Economics, and UX. Zero preferences enter training because two winners retain critical judge defects and one fails compiler admission.',
        'The hash-bound evidence directory preserves raw captures, current replays, judge packets and reviews, paired classifications, and the intentionally empty preference file.',
        'Three cases and one identified model judge are diagnostic evidence, not statistical significance, human validation, aggregate quality, or paid-reference parity.',
        'No adapter is active and no Gemma weight changed. Shared admission and compilation benefit paid-model routes too; browser-local inference and retry orchestration remain Scion-specific.',
      ],
    },
    {
      label: 'Repair the live first frame',
      icon: 'LIVE',
      color: 'blue',
      items: [
    ...
    {
      label: 'Expose missing knowledge instead of disguising it',
      icon: 'CHECK',
      color: 'amber',
      items: [
        'When a lesson-kernel item does not clear admission, quiz recovery now derives explicit apply, analyze, evaluate, and create tasks from the exact lesson objective and assigned source boundary instead of repeating generic course-title prompts.',
        'The quality report counts every source-bound recovery seat and adds a P1 finding that asks the instructor to attach or verify a subject source before regeneration. Recovery can preserve a useful assessment task, but it does not invent a trusted answer key.',
        'The repaired live package showed four such recovery seats alongside two admitted model-authored questions; its 89/B grade kept that limitation visible rather than turning deterministic structure into a false model-quality claim.',
      ],
    },
  ],
};
