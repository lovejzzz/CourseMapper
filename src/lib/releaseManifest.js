...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Many Roots',
  highlights: [
    'Every neutral review candidate now receives one stable course-group identity bound to its exact canonical course input. Explicit readable group IDs cannot conceal changed prompts, and every variant of the same input shares the same group hash across model outputs.',
    'Blind packet selection now balances domain, course group, and atom kind. The course-group hash is part of the public case digest, reviewer submission, organizer key, packet digest, campaign receipt, and approved training row; forged labels or changed group hashes fail closed.',
    'Campaign readiness now measures independent course depth instead of atom volume. The existing 309 candidates and 160 selected cases resolve to only four course groups—one per domain—so the receipt truthfully reports reviewable incomplete coverage until every domain has at least three distinct groups and the domain target is met.',
    'No trained adapter or instructor preference is claimed. Completed reviews remain zero, research training remains blocked, and hosted Scion continues to use the pinned browser-local Gemma base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'Course diversity is measured, not inferred from row count.',
    'Every review is bound to its exact course group.',
    'The current campaign fails its own depth requirement honestly.',
    'Hosted Scion remains base-only while evidence grows.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.10.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Scion course-group evidence',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Canonical identity: each matched pair derives its group from an explicit manifest ID plus the exact shared course-input digest, or from that digest alone. A reused readable label with two different inputs excludes every affected pair.',
        'Shared variants: both retained UX model comparisons resolve to the same `ux-design-studio` course group because their canonical inputs are identical; model count no longer inflates course count.',
        'Balanced sampling: round-robin selection buckets by domain, course-group digest, and atom kind before taking another example from a bucket.',
        'Public integrity: anonymized cases expose only the course-group digest. That digest participates in the case hash and is copied into the offline review template.',
        'Organizer integrity: protocol v3 folds the private A/B mapping, exact source row, source-file digest, and course-group identity into a separate organizer digest, then folds that digest into the public packet hash. Changing either side of the blind handoff invalidates ingestion.',
        'Ingestion integrity: missing, forged, or mismatched course-group hashes are rejected before preference admission. Approved rows carry both the stable group ID and hash into the curated corpus.',
        'Coverage truth: receipts report total course groups, groups per domain, a three-groups-per-domain target, group-coverage status, and combined campaign readiness.',
        'Existing evidence: 309 clean atoms still produce 160 balanced review cases, but they represent four unique course inputs—Computer Science, Geology, Music Theory, and UX each have one.',
        'Honest status: the packet is `reviewable-incomplete-coverage`, not campaign-ready. Reviews remain useful, but cannot support a balanced research or production dataset claim by themselves.',
        'Current boundary: zero completed instructor reviews, no research dataset, no new adapter, no learned-quality win, and no public promotion are claimed.',
      ],
    },
  ],
};
