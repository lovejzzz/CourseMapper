...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.58 Makes Evidence Correct the Course',
  highlights: [
    'The source-lineage compiler corrects four retained multiple-choice answer indexes only when each item citation and the instructor-supplied source jointly support one different option. Current local issue instances fall from 78 to 74 with zero non-answer mutations and zero paid-reference mutations.',
    'Scion now detects incomplete and overlong answer options, treats missing facts, key terms, scenarios, and MC seats as critical attempt-selection failures, and keeps a complete lower-risk draft instead of an attractive but unusable JSON shell.',
    'The browser-local base no longer certifies its own quiz answers. Draft cold-solving is disabled; a focused repair ships only when strict cited-source evidence uniquely confirms its key, and compiler-constructed repairs never become adapter preference rows.',
    'Retry merging preserves citation integrity by moving facts and their MC items as one unit while allowing independently cleaner key terms to survive. A whole-response audit must improve with no newly introduced defect before any merge is retained.',
    'A real cached-base Macroeconomics build reached Map, Enrich, Compile, Verify, and Grade in 114 seconds, produced all five selected materials at 89/B and texture 88, and connected both deterministic Agent tools and a free-form local Scion explanation to the finished package.',
    'Frame-by-frame desktop and 390×844 testing found and removed a one-frame drag-trash flash on ordinary clicks, kept one ZIP owner and zero phone-width page overflow, and explained provisional numbered lesson slots before the course map exists.',
    'No hosted adapter is active and no Gemma weight changed. These source-admission, retry-selection, merge, compiler, and UX improvements are shared with compatible paid-model routes; browser-local execution remains Scion-specific.',
  ],
  landingHighlights: [
    'Cited source evidence safely corrects four retained answer keys.',
    'Incomplete option fragments and missing lesson cores now enter retry.',
    'A complete low-risk draft beats an unusable partial shell during recovery.',
    'Scion never treats the same local model as its own independent verifier.',
    'Facts and cited quiz items stay atomic across retry merging.',
    'A real 114-second build reached 89/B and answered through the local Agent.',
    'Ordinary tab clicks stay calm, and phone layout keeps zero page overflow.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.58.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01658_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Let cited evidence move only the key',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'A cited multiple-choice key can move only when the generated fact remains anchored to the supplied lesson source and strict answer support identifies one different option.',
        'The repair changes only the declared answer index. Question, options, explanation, and cited fact indexes remain byte-equivalent, and every repair carries source-lineage evidence.',
        'Four previously retained conflicts are corrected. Under the current detector, local issues move from 78 to 74; paid-reference artifacts remain byte-identical and no constructed repair enters training.',
      ],
    },
    {
      label: 'Prefer complete evidence over clean-looking fragments',
      icon: 'RECOVER',
      color: 'blue',
      items: [
        'Option admission now exposes underlength, overlength, and function-word endings instead of letting a constrained decoder fragment look complete.',
        'Missing facts, key terms, scenarios, and MC seats carry critical recovery weight. On a fresh exact Economics capture, attempt selection retained the complete three-issue artifact instead of the eight-issue one-fact shell.',
        'Retry merging keeps facts and position-indexed MC citations together; only independent key terms may cross attempts, and only when the full artifact improves without a new issue.',
      ],
    },
    {
      label: 'Stop same-model self-certification',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'Production Scion skips draft cold-solving because another call to the same base is not independent factual verification.',
        'Admission requests exactly one compact repair at a time, excludes the broken verbose options from the retry input, and requires strict deterministic support from the cited lesson facts before replacing a seat.',
        'Same-model repair evidence is marked training-ineligible. An adapter candidate still needs genuine judged preferences and held-out proof rather than self-agreement.',
      ],
    },
    {
      label: 'Test the complete experience frame by frame',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'The cached-base one-lesson Macroeconomics run showed model readiness, map streaming, retries, recovery, compilation, verification, and grading before reaching 100% at 114 seconds.',
        'The finished package produced five selected materials at 89/B with texture 88. A deterministic Agent audit completed in one second, and a free-form package-specific Scion explanation completed in about 46 seconds without editing the workspace.',
        'A normal tab click no longer paints the drag ghost or drop target for one frame. Real drag-to-delete still works, the single Export panel owns ZIP download, and the phone document remains 385 pixels wide for a 390×844 viewport.',
        'Before a map exists, Specific lessons now tells users that numbered slots are provisional and that Scion assigns topics during map generation.',
      ],
    },
    {
      ...
      color: 'amber',
      items: [
        'V0.16.58 improves the source-lineage repair boundary, semantic admission, retry selection, repair routing, compiler UX, and evaluation observability. It does not modify the pinned Gemma weights or activate an adapter.',
        'The stronger current detector also exposes eight historical local and six historical paid-reference option defects not counted by the frozen V0.16.57 profile. Those are newly visible defects, not manufactured regressions.',
        'Shared source admission, recovery ranking, merge safety, compilation, grading, and UX benefit compatible paid-model routes. Browser WebGPU inference, compact local retry, and base caching remain Scion-specific.',
      ],
    },
  ],
};
