...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Rootstock & Graft',
  highlights: [
    'Scion now has an implemented adapter architecture, not a renamed-model story: an immutable public Gemma 4 E2B Instruct base, a separately hash-bound LoRA delta, and the versioned Scion compiler. The local runtime refuses bare or mutated adapter directories, publishes exact base and adapter identity, and has completed one real base-plus-adapter schema inference. That historical delta is mechanics-only smoke evidence and can never be promoted.',
    'Learning stays fail-closed. The group-safe dataset builder loaded 471 raw preference events and admitted zero: every row lacked the independent evidence or explicit split identity required for production training. Candidate manifests remain impossible below 3,000 verified pairs across five domains, and promotion additionally requires five exact matched courses plus hash-verified factual, instructor, device, and production attestations.',
    'The Qwen-versus-Gemma gap became a compiler improvement. Domain-neutral applied-reasoning detection, admission-aware repair, lesson-batched topic repair, and final option-label normalization cut exact-Qwen Scion calls from 108 to 91. The real 12-lesson Business Ethics rerun reached 99/A with zero P0/P1/P2 and 38/38 export checks; its 1.247× burden versus the 73-call Gemma control is inside the 1.25× ceiling.',
    'The claim boundary is explicit: v0.16.6 trains and promotes no new weights, and the hosted keyless route remains anonymous-model plus Scion compiler. Compiler improvements benefit every provider that uses the shared pipeline; a future Gemma adapter would benefit only its pinned compatible base. Production browser dynamic-LoRA support and qualified instructor-reviewed training data remain open milestones.',
  ],
  landingHighlights: [
    'Gemma is pinned as Scion’s future local rootstock.',
    'Small adapter packages are identity- and hash-bound.',
    'Unsafe raw preferences cannot enter training.',
    'A real Qwen course now passes the compiler ceiling.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.6.json',
    roadmap: 'docs/SCION_NEXT_LEVEL_PLAN.md',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'Base contract: Scion pins `google/gemma-4-E2B-it` to immutable revision `9dbdf8a839e4e9e0eb56ed80cc8886661d3817cf`. The Q4 GGUF fallback is separately pinned to its exact repository, revision, filename, byte count, and SHA-256 expectation instead of trusting a mutable model label.',
        'Package contract: `scripts/scionAdapterPackage.mjs` binds every regular adapter file by safe relative path, byte count, and streaming SHA-256. Traversal, symlinks, duplicate paths, unexpected files, byte changes, hash changes, and base-revision mismatches fail verification.',
        'Runtime truth: the OpenAI-compatible local shim accepts only a verified manifest-backed adapter directory, forwards the exact base revision to the worker, and reports adapter state, ID, manifest hash, active status, and load error through both health and model discovery.',
        'Mechanical smoke: a historical 52.8 MB adapter loaded on the exact Gemma base in 5.576 seconds and returned `{"status":"ok","purpose":"adapter-smoke"}` under a constrained schema. Its manifest is permanently `smoke` and `promotable: false`; its old training provenance cannot satisfy a quality claim.',
        'Dataset proof: the v0.16.6 build loaded 471 raw rows, admitted zero, quarantined all 471, created no train/valid/test rows, and found zero group overlap. The compact evidence binds the generated manifest plus all three empty split hashes without retaining quarantined prompts or responses.',
        'Training boundary: candidate and promoted manifests are invalid unless their bound dataset is ready with at least 3,000 verified pairs spanning five known domains. The ORPO launcher resolves the 40-character Gemma revision, writes outputs outside Git, and refuses production training when the gate is closed.',
        'Promotion boundary: the adapter audit requires five domain-matched 12-lesson courses, exact active adapter identity, 99/A with zero P0/P1, no per-domain call regression over 1.05×, at least 20% median call reduction, and hash-matched factual, blind-instructor, device, and production evidence. It verifies the referenced files\u2019 real hashes instead of trusting self-reported digests.',
        'Compiler diagnosis: the old applied-depth rule recognized only 6/45 Qwen MC items and triggered 33 rewrite actions. The domain-neutral detector recognizes concrete case-to-framework and case-to-judgment reasoning while retaining recall, minimum-length, completion, actor/action, and evidence guards.',
        'Repair redesign: Scion now targets two applied seats per lesson, skips stem rewriting when immutable options or explanations already fail admission, batches residual topic repair once per lesson, verifies the batch with two cold solves, and normalizes answer-option labels before projection.',
        'Real browser proof: exact Qwen revision `0e7ffd5c629ef7719d4cbc04069232580bfa9d9c` completed `round-2026-07-13T05-09-35-402Z` in 548 seconds with 104/104 requests, 101 extracted files, 99/A, zero P0/P1/P2, texture 95, and 38/38 export checks with no warnings.',
        'Measured improvement: Qwen used 91 Scion pass calls versus 108 in its previous exact Business Ethics run, a 15.7% reduction. The matched exact-Gemma compiler control used 73, so the candidate/control ratio is 1.247× and passes the frozen 1.25× ceiling. One intermediate doubled-label defect was fixed and excluded from the final proof.',
        'Claim boundary: no v0.16.6 adapter is trained, promoted, shipped, or implied by the public Scion label. The hosted route still uses Pollinations\u2019 anonymous text endpoint plus the Scion compiler. Shared compiler improvements benefit Local, GPT, Claude, Gemini, and other routed providers; a future adapter delta is base-specific.',
      ],
    },
  ],
};
