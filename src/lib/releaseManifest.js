...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion’s Frozen Quality Test Is Now a Training Firewall',
  highlights: [
  ...
  landingHighlights: [
    'Training now fails closed against the exact frozen benchmark.',
    'Held-out domains and relabeled held-out courses are quarantined.',
    'Dataset identity binds the exam before optimization begins.',
    'The current corpus is 75 usable pairs and zero valid preferences.',
    'No adapter, weight, quality, or production win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.33.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'The test set becomes untouchable',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The dataset builder now reads the canonical held-out manifest before it reads training eligibility. It verifies the five frozen Crucible fixtures, exact public Gemma base contract, grader identity, and disjointness policy. The benchmark must be a regular file; a missing file, symlink, malformed manifest, changed fixture, or wrong policy fails before any split is emitted.',
        'Rows are screened twice: by normalized domain and by normalized course ID. That second key matters because relabeling astro-101 as computer science must not make an Astronomy exam course eligible for training. Excluded rows stay in the quarantine ledger with explicit domain or course-group reasons and never reach deduplication or split assignment.',
        'Dataset schema v4 publishes a holdout-firewall receipt with the exact benchmark hash, frozen time, five excluded domains, group counts, admitted overlap counts, and excluded row counts. Dataset identity v2 binds that receipt along with source bytes, domain registry, admitted data, split identities, evidence distribution, and output hashes.',
      ],
    },
    {
      label: 'One boundary from corpus to evaluation',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Every training plan now reopens the benchmark, validates it again, checks its recorded SHA-256, recomputes domain and group separation, and requires both domain-plus-course hashes and course-ID-only hashes. Changing a source, split, benchmark, dataset identity, or boundary proof after curation blocks the run.',
        'The paired adapter evaluator adopts the same course-ID-only proof. A future adapter cannot reach the five-domain promotion run with a held-out course hidden under a different training-domain label, even if its older domain-plus-course hash appears distinct.',
        'The tracked readiness receipt is rebuilt from the 418 current stored pairs and exact source files. Seventy-five pass structural contract evidence, spanning four domains and five groups; zero have admissible stable same-identity Codex preference evidence. Research still needs 100 qualified preferences across four domains and three groups per domain; production still needs 3,000 pairs and five balanced domains.',
        'This release deliberately trains nothing. It prevents a future apparent win from being caused by test leakage, but it does not make Gemma better, create a research adapter, beat base, match GPT-5.4-mini, add device evidence, or activate weights in production. The next quality step remains a new identity-matched two-order Codex campaign.',
      ],
    },
  ],
};
