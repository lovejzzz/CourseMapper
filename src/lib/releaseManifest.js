...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Frozen Ground',
  highlights: [
    'The five-domain adapter benchmark is now frozen before candidate evaluation: World Languages, World Literature, Psychology, Nutrition, and Astronomy, with 12–15 lessons each. Every prompt and prompt-only source packet is hash-bound, and a training dataset must prove both domain-level and hashed course-group separation or the benchmark is ineligible.',
    'Crucible can now run exact paired Scion experiments. Both adapter and base-only arms must use the complete frozen course set, the same clean compiler commit and tree, the same compiler configuration and grader bytes, and the exact QAT parent. The Local model health contract also records adapter scale, so a friendly model label cannot stand in for active runtime identity.',
    'A canonical evidence producer now derives promotion JSON from real course.json, project, report, digest, console, package manifest, and ZIP artifacts. It hashes every retained input, refuses duplicate or missing courses, validates adapter/base state, and emits paired receipts; the promotion audit rejects hand-shaped records that do not carry this producer and artifact identity.',
    'No adapter is promoted by this release. The 101-pair exact-QAT smoke remains permanently non-promotable and the independently qualified production corpus remains at zero. Hosted Scion stays base-only until a newly trained candidate wins this frozen benchmark, lowers median compiler calls by at least 20%, and passes factual, device, export, production-canary, and blind-instructor gates.',
  ],
  landingHighlights: [
    'Five unseen course domains are frozen before training.',
    'Adapter and base runs share one compiler and grader identity.',
    'Promotion evidence is derived from real package artifacts.',
    'Hosted Scion remains honestly base-only.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.8.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Scion paired evidence',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Frozen ruler: `evaluation/scion-adapters/held-out-course-benchmark-v1.json` fixes five prompt-only 12–15 lesson courses across five domains. Course inputs, source packets, the exact QAT base contract, and the grader file are SHA-256 bound.',
        'Leakage proof: curated dataset manifests now publish only SHA-256 course-group identities, not raw course names. Evaluation recomputes the frozen groups and blocks old or incomplete manifests that cannot prove separation.',
        'Run preflight: Crucible refuses a partial course selection, dirty tracked compiler source, relevant untracked source, a different base revision, an inactive or mismatched candidate adapter, or a base arm that still reports an adapter.',
        'Shared protocol: every course receives a stable pair ID plus identical benchmark, prompt, source, compiler commit/tree/config, grader, and base-contract digests. The variant is the only intended model-state difference.',
        'Runtime telemetry: the Local health and model endpoints now expose the manifest adapter scale, and Crucible retains it beside the exact source model, source revision, adapter ID, and adapter-manifest digest.',
        'Artifact-derived evidence: `capture:scion:adapter:pairs` reads both complete Crucible rounds, verifies all five domains, and hashes the course input, saved project, report, digest, console, exported package manifest, and ZIP before emitting candidate/base evidence and a receipt.',
        'Promotion boundary: the promotion audit requires the canonical producer, artifact receipts, frozen benchmark and grader hashes, one pair per domain, exact adapter/base state, no quality regression, a 1.05× per-domain call ceiling, and at least 20% median call reduction.',
        'Claim boundary: this release improves the evaluation machinery; it does not make the smoke adapter stronger, add independently reviewed preferences, or promote trained Scion weights. Public Scion remains the pinned browser-local Gemma base plus the model-neutral compiler.',
      ],
    },
  ],
};
