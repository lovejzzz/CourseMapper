...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion: Evidence to Decision',
  highlights: [
    'Scion is now explained end to end: it is Course Mapper\u2019s constrained authoring system, not a newly trained foundation model. The hosted Scion Draft route requests Pollinations\u2019 legacy `openai-fast` alias, but the anonymous provider selects and may change the actual backing model; the fixed value is the Course Mapper contract, compiler, grader, and export path.',
    'One shared evidence-to-decision contract now governs authoring, admission, deterministic projection, and evaluation. A lesson scenario passes only with concrete context, an actionable decision or problem, an inspectable evidence packet, a real tension or constraint, and specific materials; failures return named issue codes instead of an unexplained boolean.',
    'Rejected or missing scenarios recover without fabricating facts or spending another provider call: the compiler may reuse only an admitted key-term example, its misconception/correction pair, and an admitted fact, and it saves `derived-kernel-fallback` provenance. One-sided multiple-choice explanations receive a bounded contrast frame that names an authored competing option without inventing a new subject claim.',
    'The paired quiz diagnostic is now a release gate, not a flattering comparison table. Missing lessons count in the scenario denominator; strict mode exits nonzero; and explicit bars cover 12-lesson scope, applied reasoning, supported inference, contrastive rationales, scenario coverage/readiness/materials, and cue-free claim-evidence-boundary short answers. One pair remains directional evidence, never proof of general model superiority.',
    'Public Scion resilience now matches the route\u2019s real failure modes: incomplete lesson envelopes retry slowly, admission failures rotate across missing lessons, and the free route reserves four recovery calls while paid providers keep the two-call budget. The final 12-lesson browser run reached 99/A with zero P0/P1/P2 findings, 12/12 enrichment, and every strict quiz bar green at $0.',
    'Rendered output closed the last gap the score missed: visual QA found a blank fifth page in a long quiz answer key, traced it to a final empty spacer paragraph, and removed it. The regenerated Lesson 12 quiz now ends cleanly on page 4; representative Lesson 1, 8, and 12 quiz documents render without clipping or overlap.',
  ],
  landingHighlights: [
    'Scion explained: a constrained system, not a newly trained model.',
    'One evidence-to-decision contract from prompt to grader.',
    'Grounded zero-call scenario recovery with visible provenance.',
    'Strict paired evaluation with denominator-correct release bars.',
    'Fresh public run: 99/A, 12/12, zero findings, $0.',
    'Rendered quiz QA removes the blank trailing page.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.2.json',
    roadmap: 'docs/V0.16.2_EVIDENCE_TO_DECISION_ROADMAP.md',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'Final browser evidence (`round-2026-07-10T16-48-15-154Z`): User Experience Design Studio, 12 lessons, 99/A, zero P0/P1/P2, readiness ready, export 38/38, enrichment 12/12, $0.',
        'Strict paired diagnostic: applied MC 67.6%, supported inference 100%, contrastive rationales 100%, scenario coverage/readiness/materials 100%, cue-free and claim-evidence-boundary short answers 100%. No measured reference advantage cleared the 10-point learning threshold.',
        'Rendered quiz inspection covered Lessons 1, 8, and 12. The blank Lesson 12 trailing page found during the first render was fixed and re-rendered as a clean four-page document.',
        'Claim boundary remains unchanged: this one local pair is operational evidence. Independent instructor review and hash-retained production canaries are still required for independent-validation or production-proof claims.',
      ],
    },
  ],
};
