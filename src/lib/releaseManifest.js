...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Measures What the Compiler Fixes—and What It Cannot',
  highlights: [
  ...
  landingHighlights: [
    'The same compiler is replayed on local and paid-reference outputs.',
    'Local admission rises 132/192 → 168/192.',
    'Reference admission rises 177/192 → 182/192.',
    'The measured contract gap shrinks by 68.89%.',
    'The remaining gap is key-term output—not an MC contract gap.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.26.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'One ruler across both model arms',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The audit materializes both frozen campaign manifests and reconstructs all twelve course groups from their source Genome files before reading any result.',
        'Every local and reference project must pass the original source-capture verifier, including prompt-set, course-input, source-packet, model-response, compiler-recovery, admitted-output, and project-graph hashes.',
        'The replay applies exactly the production MC recovery: retain an existing complete sentence before an unfinished tail, then realign a key only when the affirmative explanation supports another option by the decisive lexical margin.',
        'Historical raw admission is independently recomputed and must equal each retained call receipt. One already-retained recovery key term fills one missing local seat; no response or source project is changed.',
        'The tracked v0.16.26 receipt binds all 24 evidence projects and the compiler, admission, source-capture, graph, provider, and stream-integration implementations.',
      ],
    },
    {
      label: 'Compiler leverage is real; model quality is still separate',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The model-neutral compiler benefits both retained arms: +36 atoms for local Gemma and +5 for GPT-5.4-mini. The weaker model benefits more because it creates more repairable contract failures.',
        'Raw reference advantage falls from 45/192 atoms to 14/192 after compilation, so deterministic recovery closes 68.8889% of this measured contract-admission gap.',
        'Both arms finish at 86/96 admitted MC items. That is MC contract-admission parity only; the audit does not decide whether the questions are equally correct, useful, or teachable.',
        'The remaining cross-arm difference is entirely local key-term output. Twelve correction fields duplicate definitions, one source index is invalid, and one expected key-term seat was never produced.',
        'That residual is a model or future-adapter target because the compiler cannot safely invent a misconception correction or a missing source-grounded term.',
        'No result is relabeled as a quality win. Fresh B/A judgment, a real quality adapter, five held-out domain wins, paid-reference quality comparison, and three device profiles remain unfinished.',
      ],
    },
  ],
};
