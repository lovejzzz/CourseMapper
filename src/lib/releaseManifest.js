...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Domain Truth',
  highlights: [
    'The second exact-provenance domain refused a false win. In Business Ethics, Gemma appeared to score 98/A but shipped music-theory material and now regrades 74/C with a P0; Qwen scored 89/B with six reading-identity P1s. Neither package qualifies, so Qwen is not promoted and Gemma does not receive a second passing domain.',
    'Both models exposed the same native-skeleton seam: each returned all 12 sessions and then stopped inside the top-level assessments array. Scion now closes only a prefix ending after a fully complete array object, retains the complete sessions, synthesizes one weighted assessment per session, records the recovery, and still fails closed when an object itself is unfinished. Both saved responses replay to 12 sessions, 12 assessments, and exactly 100% weight.',
    'The local quiz critic is model-neutral again. A hard-coded Music Theory label and music-specific repair language are gone; topic vocabulary no longer discards music as generic noise. A fresh exact-Gemma v0.16.5 browser run completed all 87 inner generations, passed 38/38 export checks, and regrades 99/A with zero findings after narrow Business Ethics citation calibration. Strong multi-signal contamination checks now run even for generic-profile courses.',
    'Evaluation truth improved with the compiler. Local-model health separates started, completed, failed, and in-flight inner generations; each HTTP body-log row carries its own inner-call metrics; Local-provider Crucible runs receive the intended extended generation cap; and imported evidence paths remain portable when runs execute in an external cache worktree.',
  ],
  landingHighlights: [
    'Business Ethics exposed a false 98/A Gemma result.',
    'Neither exact model produced a qualifying second domain.',
    'Truncated 12-session skeletons now recover safely.',
    'Wrong-discipline leakage now blocks the package.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.5.json',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'Matched-domain proof: exact `google/gemma-4-e2b-it` run `round-2026-07-13T01-10-00-422Z` and exact `mlx-community/Qwen3.5-4B-4bit` run `round-2026-07-13T02-13-40-591Z` used the same 12-lesson Business Ethics brief, real Local provider, voice-off mode, browser export, current compiler, and grader. Both extracted 101 files at $0; neither package passed the promotion contract.',
        'False-green proof: Gemma\u2019s pre-fix report said 98/A with two P2s, but the real exported package contains counterpoint and fugue material inside Business Ethics lesson materials. Replaying the package through the v0.16.5 discipline gate produces 74/C with one P0. The old grade is retained in evidence instead of rewritten.',
        'Qwen gap proof: Qwen completed one attempt in 551 seconds and graded 89/B with six P1s and two P2s. Three named readings disappeared from both their lesson-plan materials and syllabus schedule; no foreign music-theory content was found in its final package.',
        'Skeleton proof: Gemma stopped at 2,432 response characters and Qwen at 5,546, both after complete session arrays and inside the top-level assessments array. The narrow recovery replays both saved responses into 12 sessions, 12 synthesized assessments, and a 100% weight total; an unfinished assessment object remains unrecoverable.',
        'Harness proof: Gemma\u2019s first attempt finished enrichment just as the driver\u2019s 10-minute workspace wait expired, so Crucible mislabeled a generation timeout as `finalizing-package`. Real Local-provider runs now receive the same 3× step cap already granted to routed shim runs.',
        'Contamination-source proof: the best-of-two quiz prompt contained a literal `Course: Music theory` and later repair prompts said `advanced theory` and `double-check the music theory`. Those model-specific strings are removed, and the lesson-topic stop list no longer treats music/theory as generic noise.',
        'Telemetry proof: a deterministic fake worker holds one request in flight, then completes it. Health moves from started 1 / completed 0 / in-flight 1 to started 1 / completed 1 / in-flight 0, and the body log attributes exactly one inner model call to the HTTP envelope.',
        'Portable-evidence proof: model evidence imported from the detached cache worktree records canonical `verification-output/crucible/<round>` identifiers rather than machine-relative `../../..` paths. The compact domain audit retains exact package and shim-log hashes plus the corrected quality verdicts.',
        'Post-fix browser proof: exact Gemma run `round-2026-07-13T02-50-48-419Z` stayed on typed native authoring after disclosed skeleton recovery, completed 87/87 inner generations with zero failures, and passed 38/38 export checks in 762 seconds. The live report\u2019s only P1s were legitimate `Utilitarianism` and product-safety sources; the citation-calibrated exact package regrades 99/A with zero findings. Generic probe suppression no longer disables universal contamination detection.',
        'Claim boundary: v0.16.5 changes no foundation-model weights and does not promote Qwen, claim a second passing domain, or claim instructor readiness. UX remains the only qualifying matched domain; four passing domains, the browser-device matrix, and independent instructor evidence remain open.',
      ],
    },
  ],
};
