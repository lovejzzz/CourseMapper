...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.51 Keeps the Course in Hand',
  highlights: [
    'An interrupted setup now stages the course brief and intended next step before a lazy route loads. When a stale deployed chunk forces the one-time refresh, EduTool resumes the setup instead of discarding the user’s course; attached file names are remembered only to request reattachment, never the private file bytes.',
    'Course-map Review and Check starters now invoke the deterministic package audit. They no longer send an audit-shaped sentence to the advisory model and accept a malformed acknowledgment as if a review occurred.',
    'Living Course Compiler progress now labels its aggregate percentage as Overall, separating whole-build progress from the model-download percentage. A voice pass at texture 100 skips the provider call because the bounded self-check cannot improve beyond its ceiling.',
    'Warning color is reserved for work that needs attention. Local Scion setup, app-update recovery, and the landing-page AI-draft disclosure use calm information or neutral treatments instead of amber failure language.',
    'Recommended materials, all-versus-specific scope, and every lesson selector expose real pressed state. The quality report is a labeled modal dialog, closes with Escape, and moves initial focus to its close action.',
    'The phone-width setup path was replayed at 390 by 844. Its primary materials CTA stays on one deliberate line, the attachment footer stacks cleanly, the lesson grid remains operable, and no page introduces horizontal overflow.',
    'The repaired flow passed lint, a production build, 4,749 tests, and a local desktop-and-phone browser replay covering brief recovery, materials, generation setup, selection semantics, and responsive frames.',
    'No adapter is active and no Gemma weight changed. These are compiler, orchestration, accessibility, efficiency, and workflow-trust improvements; model-neutral paths can benefit paid providers while the browser-local runtime remains Scion-specific.',
  ],
  landingHighlights: [
    'App updates preserve the course brief and resume the intended setup step.',
    'Agent Review and Check starters run a real deterministic package audit.',
    'Overall build progress is distinct from model download progress.',
    'Scion information is blue or neutral; amber is reserved for attention.',
    'Material and lesson selectors expose correct pressed state to assistive technology.',
    'Desktop and 390×844 phone replays pass without horizontal overflow.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.51.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Keep the brief through an app update',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The live audit reproduced a stale lazy-route chunk during deployment. The update boundary refreshed correctly but silently erased the selected example and course brief.',
        'Continue and quick-start now stage a short-lived setup receipt before the next route loads. It contains the prompt, a bounded action, and attachment names; it never stores attached file contents.',
        'After the one-time refresh, a text-only brief resumes automatically. If files were attached, the landing page restores the brief and names exactly which files must be reattached.',
        'The update boundary now reads as an informational transition—“Updating EduTool”—instead of rendering a red crash card for an expected deployment handoff.',
      ],
    },
    {
      label: 'Make Agent audits perform the work they promise',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The browser audit clicked “Review Lesson 1 … for gaps” and exposed a real contract failure: the starter had no action metadata, so the advisory local model returned a malformed acknowledgment and no audit ran.',
        'Every course-map Review and Check starter now declares the local-audit action and course-map feature identity. ChatPanel routes it through the deterministic package auditor already used by the other deliverables.',
        'The regression test scans every course-map audit starter so a future wording change cannot silently disconnect the action from the tool.',
      ],
    },
    {
      label: 'Explain progress and spend model work carefully',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The Living Course Compiler’s visible percentage now says Overall. A user can distinguish the complete build’s 9% from a simultaneous model download at 58% without reading an accessibility label or guessing which bar owns the number.',
        'The voice pass now checks the deterministic pre-score before it calls a provider. If the chosen surface is already texture 100, it records a transparent skipped receipt and spends zero calls because no bounded improvement is possible.',
        'The completion manifest carries the voice skipped count, keeping optimization visible instead of making the cheaper path look like missing work.',
      ],
    },
    {
      label: 'Use color and semantics consistently',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'Local Scion setup is a blue note, not a yellow warning. The first model download and the need to review AI drafts remain explicit without visually implying that configuration is broken.',
        'The collapsed landing status says AI draft in neutral slate. Amber remains available for real review findings and blocked work.',
        'Recommended materials begin neutral and become accented only when the exact recommended set is selected, so a preset no longer looks applied before the click.',
        'The quality report declares a labeled modal dialog, moves focus to Close, and responds to Escape. Scope and lesson controls expose aria-pressed truth for keyboard and assistive-technology users.',
      ],
    },
    {
      label: 'Replay the setup frame by frame',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The local release candidate was replayed from a populated brief through materials and generation setup at desktop size and 390×844 phone size. Five hashable frame captures ship with the release evidence.',
        'The mobile materials CTA uses responsive padding and nonbreaking label groups, while the attachment footer stacks its action and supported-file guidance instead of squeezing both into one row.',
        'The replay verified zero horizontal overflow, correct pressed states before and after interaction, two selected lessons, informational Scion copy, and setup recovery after a forced reload.',
        'Focused regressions passed 67/67. The complete suite passed 4,749 tests with 162 intentional skips; lint and the production build also passed.',
        'No adapter is active, no Gemma weight changed, and no universal course-quality or paid-reference parity claim is made. V0.16.51 hardens the experience around the V0.16.50 semantic compiler result.',
      ],
    },
  ],
};
