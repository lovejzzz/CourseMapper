...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Founder Research',
  highlights: [
    'Every domain review packet now contains two explicitly separate evidence lanes over the same hash-bound blind cases. The existing working-instructor lane keeps its independence, domain-teaching, and no-conflict attestations; a new founder-research lane lets a solo product founder contribute real judgments without claiming to be an external instructor.',
    'The forty-case reviewer is now practical to finish: it shows one case at a time, reports completed progress, supports back, next, direct case jumps, and flags for later, autosaves locally, and remains a self-contained zero-network page.',
    'Founder exports carry a dedicated protocol and evidence class, product-founder role, declared conflict, non-independent status, and claimEligible false boundary. A separate validator pins those fields, while the production instructor validator and ingestion path reject founder evidence by construction.',
    'Founder review can expose answer-key errors, contradictions, ambiguity, unsupported generalization, and overclaim for compiler repair or a non-promotable research experiment. It cannot enter the approved production corpus, satisfy the two-instructor gate, or promote an adapter.',
    'This release changes evaluation workflow, not Gemma weights or hosted inference. Approved training pairs, independently validated adapter wins, and qualifying device profiles remain zero; public Scion remains the pinned browser-local base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'Founder judgment stays useful without becoming fake independence.',
    'Forty cases become a focused one-at-a-time workflow.',
    'Progress, jumps, flags, and drafts remain entirely local.',
    'Founder evidence is structurally barred from production ingestion.',
    'No model-weight or adapter-quality gain is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.14.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Two honest evidence lanes',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Each domain folder keeps review.html for qualified independent instructors and adds founder-review.html for the solo-founder research path.',
        'Both pages render the same anonymized cases and neutral source context without exposing model identity, organizer mapping, source rows, or network requests.',
        'Founder JSON is labeled founder-review, non-independent, conflict-declared, and claim-ineligible; it cannot accidentally masquerade as external evidence.',
        'The original instructor schema and two-reviewer corpus-admission rules remain unchanged.',
      ],
    },
    {
      label: 'A review that can actually be completed',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Only the active case is displayed, reducing the original forty-card scroll into one focused comparison.',
        'Completed-count progress and a visual bar update from the actual ratings, choice, and rationale fields.',
        'Back, next, direct case jump, and flag-for-later controls make interruption and return safe; autosave remembers the active case and flags.',
        'Submission reopens the first incomplete case before browser validation, then downloads the correct instructor or founder provenance shape.',
        'The decisive boundary remains visible in the interface and exported data: founder results may guide research, but cannot promote Scion.',
      ],
    },
  ],
};
