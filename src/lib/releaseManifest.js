...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Clean Seed',
  highlights: [
    'Training candidates now require exact course-input identity before their model outputs can be paired. The audit removed 68 World Literature atoms whose two runs used different prompts, restored 45 Music Theory atoms through an explicitly labeled lesson-number fallback for repeated generic titles, and binds every retained row to both source artifacts and one shared input digest.',
    'Blind instructor review is now a frozen, holdout-disjoint protocol instead of a mutable folder of forms. A 160-case campaign balances 40 cases across Computer Science, Geology, Music Theory, and UX; each public case, packet, source file, and organizer mapping is SHA-256 bound. Ingestion rejects changed evidence and atomically merges domain batches without erasing or duplicating earlier approvals.',
    'Adapter learning now has a separate research tier: at least 100 blind-instructor-approved pairs, including at least 20 in each of four domains, plus three isolated course groups per domain can train a non-promotable research adapter. Domain-stratified group splitting keeps every course in exactly one split, and adapter manifests retain the per-domain evidence distribution. Public candidate and promotion requirements remain stricter at 3,000 pairs, five domains, fifteen groups, and the same per-domain instructor floor.',
    'No trained adapter is promoted by this release. The campaign needs 320 independent instructor judgments and currently has zero completed reviews, so research training correctly refuses to start. Hosted Scion remains the pinned browser-local Gemma base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'Mismatched course runs cannot enter Scion training.',
    '160 blind cases are frozen outside the held-out ruler.',
    'Research adapters cannot masquerade as public candidates.',
    'Hosted Scion remains honestly base-only.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.9.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Scion clean-seed corpus',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Input identity: matched-candidate extraction hashes canonical prompt and attachment identity before comparing any lesson. Different course inputs are excluded rather than repaired after the fact; attached-source comparisons require an explicit source-packet digest.',
        'Artifact provenance: every neutral atom records the shared input digest, both saved-project digests, lesson identities, and whether matching used title overlap or the narrow repeated-generic-title fallback.',
        'Corpus correction: World Literature fell from the training campaign because its retained Scion and paid-model projects used different prompts. Music Theory entered because the prompts were byte-identical and its generic reference titles could be paired by matching lesson number.',
        'Frozen campaign: `audit:scion:review-packet` creates 160 anonymized cases, exactly 40 per non-held-out domain, and a receipt that binds the source candidate file plus the five-domain held-out benchmark.',
        'Tamper resistance: case digests cover prompt and both randomized sides; the packet digest covers every complete case; reviewer submissions carry both; organizer ingestion recomputes the packet from its private key before accepting a judgment.',
        'Safe accumulation: approved rows merge atomically by case digest. Re-ingesting the same review is idempotent, later domain batches preserve earlier approvals, and a conflicting row under the same evidence identity fails closed.',
        'Instructor boundary: two distinct working instructors teaching the case domain must agree on one side, clear 4/5 factual and teachability floors, provide rationales, and attest independence and no conflict before a row can enter the reviewed corpus.',
        'Research tier: a non-promotable adapter may train at 100 approved pairs only when each of four domains contributes at least 20 instructor-approved pairs and three isolated course groups. Stable per-domain hashing gives each domain separate train, validation, and test courses with no group leakage.',
        'Distribution proof: dataset and adapter manifests retain per-domain group counts, per-domain instructor-pair counts, split row counts, and split domain counts. A large aggregate from one discipline or an empty domain split cannot masquerade as balanced research evidence.',
        'Promotion boundary: research status survives MLX-to-GGUF conversion and can never satisfy `requirePromoted`. The public candidate gate still requires a ready 3,000-pair, five-domain, fifteen-group dataset plus all frozen quality and external evidence.',
        'Current truth: 309 neutral candidates and a 160-case packet exist, but completed reviews remain zero. No research dataset, new adapter, learned-quality win, or public promotion is claimed.',
      ],
    },
  ],
};
