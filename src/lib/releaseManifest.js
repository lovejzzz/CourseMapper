...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Honest Quality',
  highlights: [
  ...
  landingHighlights: [
    'One evidence-aware ruler replaces unlabeled quality numbers.',
    'Model comparisons reject duplicated trials and arbitrary scores.',
    'Every scorecard is bound to its exact source and output bytes.',
    'Scion promotion requires five-domain quality and lower compiler burden.',
    'No adapter win or speed result is claimed without real evidence.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.13.json',
    roadmap: 'docs/QUALITY_BENCHMARK_V1.md',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Evidence-aware quality contract',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Construct separation: deterministic compiler checks, rubric profiles, validation tier, operational evidence, and comparison effects remain distinct results.',
        'Rubric breadth: nine dimensions, 26 observable criteria, 0/2/4 anchors, explicit interpolation rationales, and 23 specialized deliverable/package evidence plans.',
        'Evidence states: scored, not applicable, not evaluated, and insufficient evidence remain distinct. Missing review fields fail instead of becoming zero or silently leaving the denominator.',
        'Evidence caps: automated 69, model 79, human/disputed 89, independently validated up to 100 only with exact held-out source/export evidence and no findings.',
        'Independent review: two unique domain-matched working instructors, per-review and critical-dimension coverage, ordinal reliability, exact/adjacent agreement, and immutable disagreement evidence.',
        'Held-out governance: validation is locked unless git state, dirty-tree state, and contamination history are declared. Public visibility remains an explicit limitation.',
      ],
    },
    {
      label: 'Comparison and Scion promotion truth',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Arm integrity: the analyzer requires distinct registered candidate/control IDs and rejects an output attributed to the wrong arm.',
        'Trial integrity: predeclared cases, distinct trial indices, unique generation/blinding seeds, stable within-case inputs/settings, and nonduplicated output pairs prevent favorable-row inflation.',
        'Score integrity: the comparison runner reads and hashes each scorecard, then verifies its source, artifact, rubric, profile score, dimensions, evidence class, validation tier, and clean review status.',
        'Operational truth: provider failures remain in the denominator and all-attempt latency/cost/calls remain visible beside success-conditioned quality.',
        'Compiler truth: Scion calls, repair calls, rejected atoms, and recovered atoms receive candidate-minus-control paired intervals instead of disappearing behind a clean final package.',
        'Promotion integration: the model bakeoff accepts only measured-for-scope v1 comparisons with five positive held-out case intervals, complete qualified blind review, and a strictly lower compiler-call interval.',
        'Legacy boundary: existing summary-only blind comparisons stay visible but can no longer promote a candidate. The deterministic 99/A package defect grade remains regression evidence only.',
        'Product boundary: Scion V0.16.13 remains the pinned public browser-local Gemma base plus the versioned, model-neutral compiler; quality adapter and real device-speed evidence remain absent.',
      ],
    },
  ],
};
