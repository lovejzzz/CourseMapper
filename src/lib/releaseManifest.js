...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Codex Judge',
  highlights: [
    'honest-quality-benchmark-v1 now has an explicit single-model-judge primary lane instead of silently treating every Codex judgment as advisory. The legacy qualified-human lane remains separate and unchanged; neither lane can impersonate the other.',
    'Every Codex comparison preregisters one exact model, runtime or session revision, and prompt hash. Candidate and control scores must come first from byte-verified, source- and artifact-bound scorecards carrying the same judge identity before a pairwise winner can count.',
    'Each frozen artifact pair is judged in both A/B and B/A presentation orders. The analyzer counts one stable trial outcome only when both orders agree after unblinding, retains missing or position-sensitive passes, and computes preference uncertainty over stable trials rather than double-counting one judge as two votes.',
    'Scion promotion now binds the five frozen held-out domains to at least ten distinct trials each, two reversed-order Codex passes per trial, fifty stable outcomes, and one hundred recorded passes. Positive per-domain score intervals, lower compiler burden, factual, export, browser-device, and runtime gates remain mandatory.',
    'This release makes the ruler executable; it does not manufacture a result. No quality adapter is promoted, no paid-reference parity is claimed, no device evidence is invented, and public Scion remains the pinned browser-local Gemma base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'Codex can now be the declared primary judge without becoming fake human evidence.',
    'Scores are hash-bound before a winner is chosen.',
    'A/B and B/A must agree before one trial counts.',
    'Fifty stable outcomes cover all five frozen domains.',
    'No adapter win or model-weight change is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.15.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'One judge, exact provenance',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The comparison preregistration chooses qualified-human or single-model-judge explicitly; absence preserves the legacy human mode.',
        'The model lane requires one exact Codex model, revision, and prompt digest on the preregistration, scorecards, and pairwise passes.',
        'Model scorecards stay capped at model-provisional evidence and can never create human, instructor, classroom, independent, or multi-judge validation.',
        'The checked-in prompt, template, five-domain policy, and audit bind the protocol by SHA-256 and fail closed on drift.',
      ],
    },
    {
      label: 'Score first, reverse sides, count once',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Both anonymous artifacts require byte-verified benchmark scorecards before pairwise preference, and each preference binds both artifact and scorecard hashes.',
        'A trial requires at least two passes containing A/B and B/A; disagreement becomes visible position sensitivity and blocks the primary claim.',
        'Repeated passes from the same judge produce one stable trial outcome, preventing pseudo-replication and inflated confidence.',
        'Scion requires ten trials in each of five frozen domains, fifty stable outcomes, one hundred recorded passes, a Wilson lower bound above 0.5, and lower compiler burden.',
        'The live bake-off still reports no model promoted, which is the truthful result until a real adapter supplies all required evidence.',
      ],
    },
  ],
};
