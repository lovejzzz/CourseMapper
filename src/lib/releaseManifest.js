...
export const CURRENT_RELEASE = {
  ...
  highlights: [
    ...
    'Equal package grades no longer hide unequal compiler work. Qwen required 85 Scion quality-pass calls and 35 rejected actions versus Gemma\u2019s 52 calls and 19 rejections: 1.64× call amplification, above the 1.25× promotion ceiling. The burden audit now accepts committed evidence JSON, so the comparison survives a fresh clone instead of depending on ignored local run folders.',
    'The local route now distinguishes an open HTTP socket from a loaded model. It preloads exact weights while the app starts, reports loading/ready/failed state plus load duration and error, retries failed preloads on demand, and makes concurrent first calls await one worker-ready promise. The measured cold Python/Transformers import was roughly 14 minutes; a warm-cache preload was 4.8 seconds.',
  ],
  landingHighlights: [
    'Gemma and Qwen matched on one exact 99/A course.',
    'Qwen used 1.64× Gemma\u2019s Scion quality calls.',
    'Promotion remains blocked above 1.25× amplification.',
    'Local model readiness now means weights are loaded.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.4.json',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'Matched-course proof: exact `google/gemma-4-e2b-it` run `round-2026-07-13T00-02-54-290Z` and exact `mlx-community/Qwen3.5-4B-4bit` run `round-2026-07-12T15-35-35-677Z` used the same 12-lesson User Experience Design Studio brief, Local provider, voice-off mode, browser export, current compiler, and grader. Both packages reached 99/A, zero P0/P1/P2, readiness ready, 101 extracted files, and $0.',
        'Performance proof: Qwen finished in 382 seconds and Gemma in 384 seconds once the worker was ready. Qwen consumed 50,577 estimated input and 42,414 output tokens; Gemma consumed 37,812 input and 29,592 output tokens. Startup import is reported separately instead of being blended into generation time.',
        'Compiler-burden proof: Qwen required 85 Scion pass calls (7.08/lesson), 35 rejected quality actions, and 12 regenerations. Gemma required 52 calls (4.33/lesson), 19 rejections, and seven regenerations. The resulting 1.64× amplification is a P1 promotion blocker under the existing 1.25× ceiling.',
        'Attribution proof: all 52 current Gemma Scion calls are attributed by schema—23 blind solves, seven MC admission batches, seven applied-MC batches, nine prose-polish calls, and six single-item calls. One malformed Lesson 9 repair is retained as a failed `generation-or-parse` action rather than being erased by the final 99/A grade.',
        'Learning-boundary proof: the exact Gemma run added seven raw repair pairs to the flywheel ledger, moving it from 411 to 418 rows. `audit:scion:corpus` still admits 0/418; same-model verification and runtime repair success do not become independent preference evidence.',
        'Portable-evidence proof: the exact Gemma control is committed under `evaluation/scion-model-evidence/`; `audit:scion:compiler-burden` accepts either raw course directories or committed evidence JSON and selects a shared domain. The 85-versus-52 result now reproduces without the ignored multi-gigabyte Crucible directories.',
        'Readiness proof: `npm run local-model` immediately exposes HTTP state as `modelState: loading`, then reports `modelReady: true`, exact source weights, `modelLoadMs`, and any load error. A warm-cache real Gemma preload reached ready in 4,786 ms with zero generation calls or failures.',
        'Concurrency proof: a deterministic fake worker delays its ready line while two item requests arrive concurrently. Both calls now await the same ready promise and return successfully; the worker cannot receive a request before readiness. Timed-out requests retire the poisoned worker instead of leaving later calls queued behind it.',
        'Claim boundary: v0.16.4 changes no foundation-model weights and does not promote Qwen or claim instructor readiness. It establishes one exact matched control, exposes a concrete Qwen compiler-burden gap, and makes local startup state truthful. Four more matched domains, the browser-device matrix, and independent instructor evidence remain required.',
      ],
    },
  ],
};
