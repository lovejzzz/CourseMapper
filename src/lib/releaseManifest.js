...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Now Proves One Exact Adapter from Training Receipt to Real Browser Rollback',
  highlights: [
    'Browser-device receipts and run IDs now derive their release identity from the exact adapter manifest instead of silently retaining a hard-coded historical version.',
    ...
    'A recomputable lineage audit retains and verifies the small training, result, source-manifest, conversion, browser-trace, runtime, hardware, and console receipts, distinguishes expected GPU fault-injection errors from unexpected failures, and keeps both base and adapter weights outside Git.',
    'The review ledger now deterministically rebuilds to 400 cases across sixteen groups, only 92 of them source-grounded; the corpus still has zero same-identity model-judge preferences, so the stale 128-case workbook must not be scored and no quality win is claimed.',
  ],
  landingHighlights: [
    'One deterministic training run now connects to one exact browser artifact.',
    'The browser delta is 52.7 MB; the 3.35 GB public base remains separate.',
    'Real Chrome proved native activation, exact rollback, and three recovery paths.',
    'Git retains the proof receipts and no model weights.',
    'The result is mechanics-only: Scion still has no measured adapter quality win.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.39.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Close the exact adapter lineage',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Earlier releases proved deterministic training and browser activation on different smoke artifacts. v0.16.39 converts the exact v0.16.31 seeded training output and carries its plan, result, dataset, source manifest, converter revision, and file hashes into one browser package.',
        'The browser GGUF contains 552 F16 LoRA tensors in 276 complete pairs, weighs 52,704,096 bytes, and remains a separately downloadable delta at 1.57% of the pinned 3,349,514,112-byte public base.',
        'Capture copies only four bounded provenance JSON files into evidence. The public base, 105 MB MLX weights, and 52.7 MB GGUF stay outside Git and outside the application bundle.',
      ],
    },
    {
      label: 'One real machine, honest remaining gaps',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Chrome 150 on an Apple M4 Max recovered the exact public base after an interrupted download, loaded it cold in 35.190 seconds and warm in 1.039 seconds, then activated the native Gemma 4 LoRA in 3.411 seconds.',
        'The adapter changed the deterministic output at its declared smoke scale. Deactivation restored the exact base-output and project-data hashes; adapter eviction/redownload, a Chrome GPU-process restart, and three repeated completions all recovered successfully. Peak browser working set was 5,606 MiB.',
        'The capture and standalone audit now derive v0.16.39 from the manifest, verify every retained artifact and privacy-scrubbed trace, classify the three deliberate GPU-restart errors only alongside successful recovery, and fail on any unexpected console or page error.',
        'The neutral review ledger independently rebuilds to the exact tracked 400 cases across sixteen course groups, including 92 source-grounded cases. CI now fails instead of silently rewriting the ledger when source admission or matching changes.',
        'This is a permanently non-promotable smoke adapter with zero same-identity Codex preferences. The historical 128-case workbook is stale under the stronger admission gate; at least eight new admissible source-grounded cases are needed before fresh same-identity A/B and B/A judgment. Three device profiles, a real research adapter, five-domain wins, lower compiler burden, and paid-reference parity remain unproven; hosted Scion stays base-only.',
      ],
    },
  ],
};
