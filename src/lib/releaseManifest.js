...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.66 Finishes Every Lesson and Returns the Package',
  highlights: [
  ...
  landingHighlights: [
    'A one-lesson-short course map keeps its work and finishes the missing lesson.',
    'Internal model aborts and incomplete maps recover without becoming false user cancellations.',
    'All nine material families compile from admitted course knowledge.',
    'The real 123-file ZIP passes all 38 physical export checks.',
    'Lesson plans and briefs stop restamping long titles through every paragraph.',
    'The quality report stays centered and usable at the actual viewport boundary.',
    'The model writes compact facts; the compiler owns repeatable structure.',
    'Full local and browser release gates protect the result.',
    'Adapter claims remain frozen until a real held-out win exists.',
    'Gemma weights are unchanged and the research adapter remains inactive.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.66.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01666_RESEARCH_METHODS_RECOVERY.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Let progress finish the course instead of spending a fixed retry count',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'The attached Research Methods run made three successful course-map calls and advanced from six accepted lessons to eight and then eleven, but the fixed two-continuation cap stopped it at 11/12 and mislabeled the result as unknown and non-retryable.',
        'Public Scion now budgets continuation from the number of missing lessons, accepts only the remaining scope, and stops after two consecutive no-progress responses. Cloud-provider continuation keeps its existing two-attempt policy.',
        'The browser runtime still has one global completion tail, so public native lesson-kernel scheduling remains explicitly serial; paid and remote providers retain their provider-specific concurrency.',
      ],
    },
    {
      label: 'Recover each failure class honestly',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'A user Stop still aborts immediately because the external signal remains authoritative.',
        'An internal AbortError is treated as recoverable runtime failure: Scion disposes the damaged inference instance, reloads the already-cached immutable model, and performs one bounded retry.',
        'An incomplete X-of-Y map is now classified as retryable quality failure rather than an unknown crash. Direct regressions cover runtime aborts, true caller cancellation, progress, no-progress termination, and cloud-policy preservation.',
      ],
    },
    {
      label: 'Prove both fixes on their real incident shapes',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The fresh Genetics candidate produces fifteen distinct lessons, admits 15/15 lesson knowledge kernels, and compiles every selected material family.',
        'The exact Research Methods production brief completes 12/12 lessons, 12/12 knowledge kernels, all nine compiled material families, Grade A, quality 99, texture 93, and zero deterministic findings in 89 seconds.',
        'Its fresh 1.92 MB archive was verified from Downloads, tested for compressed-data integrity, and independently extracted: 99 material files, 38 checked export conditions, zero failures, and zero warnings.',
      ],
    },
    {
      label: 'Treat the quality report as a defect list, not decoration',
      icon: 'LIVE',
      color: 'violet',
      items: [
        'The first successful package still carried 11 P2 findings: five assignment briefs and six lesson plans repeated long lesson titles 20 to 28 times. The release did not relabel those findings or lower the grader threshold.',
        'The reducer now derives a compact topic reference that cannot equal the full three-or-more-word title. It also handles an existing determiner so replacement cannot produce phrases such as “the the lesson focus.”',
        'A compiler-level Genetics regression projects the exact fields the DOCX exporter renders and holds all five previously failing titles below the published mail-merge thresholds while internal source and provenance records retain canonical identity.',
      ],
    },
    {
      label: 'Keep the quality dialog inside the viewport',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The report overlay is mounted under document.body instead of inside a transformed workspace subtree. Positioning is therefore measured against the viewport rather than a scaled panel.',
        'The dialog uses dynamic-viewport height, internal scrolling, and an overflow-safe shell. Browser geometry confirms a centered 512-pixel dialog within the 1,275-pixel viewport.',
        'An SSR-safe fallback and portal-parent regression keep the behavior stable in tests and non-browser rendering.',
      ],
    },
    {
      label: 'Put the compiler on the expensive/cheap boundary',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Scion requests compact factual lesson kernels and freezes admitted atoms; nine deliverable families are deterministic compiler products rather than nine new generations of the same structure.',
        'Incomplete facts, unsafe answer alignment, unsupported citations, missing lab requirements, and shallow constructed responses fail their admission boundary instead of being repaired into invented certainty.',
        'This model-neutral architecture is why compatible paid-provider routes also improve: they feed better source material into the same projection, validation, grading, Agent evidence, and export stages.',
      ],
    },
    {
      label: 'Expand the regression and evaluation net',
      icon: 'PROOF',
      color: 'violet',
      items: [
        'Targeted tests cover the internal-abort recovery twin, serial public scheduling, cancellation propagation, quality-modal portal ownership, and exact Genetics mail-merge cases.',
        'The wider code line also adds Crossref runtime, source-ledger feedback, generation-boundary, exported-text, semantic-admission, citation, lab-asset, quiz-depth, and package exporter tests.',
        'Evaluation receipts bind to the current compiler and frozen grader. The held-out adapter benchmark inherits no previous score and cannot activate an adapter from a compiler-only improvement.',
      ],
    },
    {
      ...
      color: 'emerald',
      items: [
        'The pinned public Gemma 4 E2B weights are unchanged and remain downloaded from their immutable public source. Course Mapper does not host a renamed foundation-model copy.',
        'The approximately 105 MB research adapter remains inactive because reduced training loss and compiler lift do not prove a held-out quality win.',
        'No human, instructor, factual, classroom, or independent-model validation is claimed. The release proves bounded software, compiler, browser, and exported-package behavior.',
      ],
    },
  ],
};
