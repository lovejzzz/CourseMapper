...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Now Intercepts 20 of 46 Measured Losses—With the Source Holding Final Authority',
  highlights: [
    'The exact 46-loss replay now intercepts twenty failures instead of eighteen: eight contradicted keys are repaired and twelve unusable artifacts are rejected, with zero model-authored text mutation.',
    'Live Scion kernels now require one or two fact citations per MC item; two newly caught wrong keys—absolute dating and a UX journey map—move only when the question, those exact claims, and one option align uniquely above conservative support and containment thresholds.',
    'Across 192 retained source atoms, ten answer repairs use source alignment: eight replace weaker explanation-only repairs, two are new, and one unsafe explanation-only repair is blocked because the source confirms the declared key.',
    'The source/explanation conflict boundary deliberately admits 130/192 atoms instead of 131/192 and raises retry burden from 61 to 62; the compiler refuses the incoherent item rather than rewriting its text.',
    'Twenty-six measured losses remain unresolved, corpus readiness remains 118/464 with 46/100 judge preferences, and no Gemma weight, quality adapter, or paid-reference claim changed.',
  ],
  landingHighlights: [
    'All 46 stable losses now run as compiler regressions.',
    'Eight contradicted answer keys are repaired without rewriting text.',
    'Twelve unusable artifacts now retry instead of shipping.',
    'Exact cited source evidence can correct or veto explanation-only key inference.',
    'Twenty-six measured losses and the 54-row judged-evidence shortfall remain.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.46.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Give exact source evidence final authority',
      icon: 'AI',
      color: 'blue',
      items: [
        'Live browser-local kernels now require one or two fact citations per MC item. Both the early local parser and canonical compiler pass only those exact claims into repair; older saved responses without citations retain the previous explanation-only behavior.',
        'The question must identify no more than two top claims, and a different option must have at least three supported content tokens, 60% containment, a two-point margin, and no competing supported alternative.',
        'Negative claims, broad or ambiguous questions, missing source indexes, tied evidence, and overlapping option aliases refuse repair. The source can confirm the declared key as well as contradict it, preventing a misleading explanation from overruling stronger evidence.',
        'The new source path corrects two stable failures: absolute dating is identified as a numerical-age method, and a UX journey map is identified as the artifact that visualizes a user’s steps and experience over time. The exact replay now intercepts 20/46 losses: eight answer-index repairs plus twelve bounded-regeneration rejects.',
        'Only answer-index fields change during repairs; model-authored text mutations remain zero and twenty-six measured losses remain unresolved.',
      ],
    },
    {
      label: 'Replace weaker inference and publish every cost',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'Across the same twelve retained-source projects, ten keys use source alignment: eight replace explanation-only repairs, two are newly repaired, and one former explanation repair is removed because the exact source confirms the model’s declared key.',
        'The stricter authority boundary exposes one source/explanation conflict and refuses it. Current admission moves from 131/192 to 130/192 and retry burden from 61 to 62; the response remains byte-identical and enters bounded regeneration.',
        'The adapter corpus remains 118/464: 72 deterministic contract margins plus the same 46 paired-order single-model judge preferences. The v0.16.45 and older profiles remain independently reproducible instead of being silently rescored.',
        'Source-authoritative admission is model-neutral and benefits source-grounded paid-model output too; browser-local Scion additionally uses bounded local retry. Gemma weights remain unchanged, the adapter remains inactive, and the research gate still requires 54 more stable preferences.',
      ],
    },
  ],
};
