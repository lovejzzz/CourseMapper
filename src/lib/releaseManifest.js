...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.54 Keeps the Brief Intact',
  highlights: [
  ...
  landingHighlights: [
    'The website, local benchmark, and future adapter share one lesson-kernel protocol.',
    'Instructor-only facts stay private, complete, and inside the stated source boundary.',
    'Explicit lesson timing now controls the compiled classroom clock.',
    'Recovery progress no longer looks like the build moved backward.',
    'A real Mandarin build reached 89/B with zero blockers and clean exports.',
    'The 148-case lesson-kernel campaign is ready; adapter promotion is not claimed.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.54.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Serve the protocol the adapter is measured on',
      icon: 'PROTOCOL',
      color: 'emerald',
      items: [
        'Public Scion now requests the compact lesson-kernel schema directly: focused facts, precise key terms, one misconception correction, a source-bounded scenario, and two cited multiple-choice checks. Course-level layout, session plans, and skin fields remain compiler-owned.',
        'Every lesson-kernel request carries the frozen `production-lesson-kernel-prompt-v1` identity. A learned route must match both the lesson-kernel task family and that exact prompt protocol; family-only matches fall back to base-only inference.',
        'The local Crucible OpenAI shim performs the same compact message conversion, JSON repair, bounded three-attempt admission, attempt merge, and option shuffle as the browser provider. Adapter and base comparisons no longer use a benchmark-only rich contract.',
        'The public provider, local server, browser runtime, and paired evidence path retain the prompt-protocol route receipt so later comparisons can prove what the weights actually saw.',
      ],
    },
    {
      label: 'Keep the instructor brief intact',
      icon: 'SOURCE',
      color: 'blue',
      items: [
        'The source brief flows into lesson-kernel generation as private model context without being mislabeled as an assigned reading or leaking into the visible supporting-resources cell.',
        'Explicit phrases such as “use only these instructor-provided facts” stop CurriculumOS linking, open-reading discovery, and the source finder. The run trace records those deliberate skips.',
        'For a one-lesson source-only build, complete labeled fact sentences are stored separately from the model kernel and projected ahead of compressed model facts. The model can shorten a kernel without erasing mā, má, mǎ, or mà from the lesson plan.',
        'Explicit 20–240 minute lesson requests become a typed compiler constraint. Six classroom phases scale proportionally and still sum to the exact requested clock instead of defaulting to 110 minutes.',
      ],
    },
    {
      label: 'Strengthen semantic admission without fabricating sources',
      icon: 'GUARD',
      color: 'violet',
      items: [
        'Mandarin admission reads both course identity and source text. A source-scoped Pinyin-and-tones lesson requires accurate tone-marked Pinyin but does not invent unsupported Hanzi merely to satisfy the broader Mandarin pattern.',
        'Multiple-choice admission now verifies that every cited source fact directly supports the keyed option and the first explanatory sentence. A valid-looking fact index cannot mask a key/evidence mismatch.',
        'Lesson plans teach up to five admitted or instructor-supplied facts instead of exposing only the first fact while downstream grading assumes the full set is present.',
        'Complete-sentence composition replaced character cuts on evidence routines, preventing fragments such as “Students submit one visible.” Quality blocker counts are also deduplicated across readiness, package, and grader receipts.',
      ],
    },
    {
      label: 'Make the Living Compiler legible frame by frame',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'The workspace title uses the prompt-aware course identity from the first model frame, so a long instruction block never becomes the page heading while the map is still loading.',
        'Initial retries remain “attempt 2/3” and “attempt 3/3.” Once outer recovery begins, the same inner counters are prefixed with “Recovery 1/1,” making the reset truthful instead of looking like backward progress.',
        'The overall meter moved monotonically through 15%, 34%, 45%, 47%, 48%, and 100% in the real run while the artifact ledger independently reported map, knowledge, materials, and checks.',
        'A downloadable package now titles the action card “Export package”; Finish package is reserved for unfinished states. Nonblocking notes keep the neutral blue information language rather than warning yellow.',
      ],
    },
    {
      label: 'Prove the release in the real browser—and keep the boundary honest',
      icon: 'PROOF',
      color: 'amber',
      items: [
        'The real cached-base audit used the instructor-only Elementary Mandarin prompt, selected the five-material recommended package, and finished in 117 seconds at 89/B with texture 88, zero P0 findings, and 22/22 clean export checks.',
        'A free-form lesson question ran through the real scion-public Agent route. Valid, malformed, and persisted chatReply envelopes now render as clean advisory prose rather than exposing transport labels.',
        'The visible lesson is exactly 50 minutes, includes all five supplied source statements and repeated mā/má/mǎ/mà practice, and contains neither Wikipedia/Tongyong additions nor the earlier broken checkout prose.',
        'The remaining P1 bibliography note is intentionally retained: instructor-provided facts are valid private evidence for this draft, but they are not silently upgraded into licensed bibliography proof. Two P2 style/readability notes are likewise preserved for review.',
        'The task-matched research campaign contains 148 production-compatible lesson-kernel cases across 25 course groups and seven domains. That is evaluation capacity, not an adapter win. No adapter is active, and promotion still requires an exact frozen base comparison across all required domains with no compiler-burden regression.',
        'Compiler improvements are model-neutral at the shared source, admission, compilation, grading, and export boundaries, so paid providers benefit too. Compact local retries, WebGPU execution, and any future adapter activation remain Scion-specific.',
      ],
    },
  ],
};
