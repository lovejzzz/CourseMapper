...
export const CURRENT_RELEASE = {
  ...
  highlights: [
    ...
    'The training launcher no longer invokes MLX-VLM’s unseeded CLI directly. A narrow Scion runner seeds NumPy shuffling and MLX LoRA initialization before trainer import, uses a physical batch of one with two-step accumulation and activation checkpointing for Metal memory, records every ORPO parameter explicitly, and derives the adapter ID from the canonical run identity.',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    ...
    },
    {
      ...
      color: 'blue',
      items: [
        'MLX-VLM 0.6.3 does not expose a seed flag even though it uses NumPy permutation and MLX random initialization. The Scion wrapper sets both generators before importing the trainer. The launcher uses physical batch one, two-step gradient accumulation, and activation checkpointing to preserve the effective batch while bounding Metal memory, and supplies every parameter explicitly rather than inheriting changing library defaults.',
