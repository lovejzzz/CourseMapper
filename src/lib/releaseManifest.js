...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.48 Makes Every Frame and Fallback More Honest',
  highlights: [
    'Source-strict key-term admission detects sixty-four of seventy-eight measured weak artifacts instead of fifty, including twenty-three of thirty-four bad key terms instead of nine, while preserving all seventy-eight preferred counterparts.',
    'A ninety-one-pair cross-domain burden replay gives eight additional local Scion outputs and two GPT-5.4-mini outputs an honest retry seat; every added seat is a source-grounding key-term failure, so the stricter compiler benefits both model paths without claiming equal gains.',
    'A fresh browser-local Bayesian run completed five of five selected materials in 109 seconds after bounded local-kernel retries. The Living Course Compiler stayed monotonic through observed 16, 38, 41, 46, and 100 percent states and disclosed the handled fallback.',
    'Frame sampling exposed intermittent black Chromium GPU tiles from nested dark-mode backdrop filters. The stable dark workspace path now removes that compositor hazard; six captures at 250 ms intervals were byte-identical.',
    'When local enrichment exhausted its attempts, the previous quiz measured course process on 64% of stems and options. The repaired Bayesian fallback measures priors, likelihood ratios, posterior odds, weak evidence, and product experiments with 0% process-language share and honest Bloom tags.',
    'The built-in Scion Agent now unwraps its local chatReply envelope before rendering, and its usage calls no longer make a completed “Ready in” duration grow after the course build has ended.',
    'The hosted model remains the immutable public Gemma 4 E2B base plus the model-neutral Scion compiler. No adapter is active, no weight changed, and no paid-reference, instructor, or classroom parity claim is made.',
  ],
  landingHighlights: [
    'Source-strict admission catches 64/78 measured losses with zero preferred-artifact regressions.',
    'The Living Course Compiler survived the difficult browser path in 109s with honest retry narration.',
    'Dark-mode transitions no longer flash black Chromium compositor tiles.',
    'A failed local kernel now yields subject-centered Bayesian assessment instead of course-process filler.',
    'Scion Agent replies render cleanly, and the completed build time stays frozen.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.48.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Catch semantic failures before they become training evidence',
      icon: 'AI',
      color: 'blue',
      items: [
    ...
    {
      label: 'Test the living compiler as a moving interface',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The release path was inspected in the real in-app browser from brief entry through package selection, mapping, nested local attempts, deterministic compilation, verification, grading, answer reveal, and Agent chat.',
        'A warm one-lesson Bayesian run finished five selected materials in 109 seconds at 89/B and texture 90. The observed percentage never moved backward and the ribbon named the exact local retry attempt instead of using decorative time-based motion.',
        'Dark-mode glass effects triggered giant black GPU tiles on alternating captures. Disabling nested backdrop filters only in the dark workspace removed the failure while retaining the glass visual language; six stability captures were identical.',
        'The completed build clock is now separate from post-build Agent and image usage. Chatting with Scion can update the cost/usage ledger without rewriting how long the course build took.',
      ],
    },
    {
      label: 'Fail into the subject, not into a template',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'The local lesson kernel failed its bounded contract on the live Bayesian prompt. That failure remains visible in the run evidence; V0.16.48 does not relabel deterministic recovery as successful model enrichment.',
        'The old fallback compiled attractive but empty questions about lessons, evidence moves, weekly artifacts, and a “professional decision.” The Agent measured 64% process language, correctly warning that students could pass without Bayesian knowledge.',
        'The compiler now recognizes Bayesian inference and decision analysis, then uses conservative facts that require no invented source: prior odds × likelihood ratio = posterior odds, diagnostic evidence drives update magnitude, and identical evidence can yield different posteriors from different priors.',
        'All six fallback items are subject-centered, answer keys rotate, rationales diagnose real Bayesian misconceptions, and explicit stem verbs keep Bloom labels aligned to the work students actually perform.',
        'The six disciplinary frames ship as an independently cached, workspace-only compilerFrames chunk. The compiler implementation stays below its existing raw-size ratchet, the new frame chunk has its own 9 KiB raw / 4 KiB gzip ceiling, and neither file enters the landing download.',
      ],
    },
    {
      label: 'Make the built-in Agent feel native',
      icon: 'AI',
      color: 'blue',
      items: [
        'Scion already answered through the browser-local model, but its advisory text-only path could paint the shared JSON response envelope verbatim. The same nested-response normalizer used by tool-calling providers now unwraps local Scion replies before display.',
        'The post-fix browser turn answered a likelihood-ratio question in one clean, workspace-grounded sentence with no raw field name or braces.',
        'Scion remains advisory in the local Agent path and does not pretend it changed the workspace. Native local tool calling remains future work.',
        'No adapter is activated by this release. The pinned public base, compiler rules, browser runtime, evaluation evidence, and optional future adapter remain independently testable layers.',
      ],
    },
  ],
};
