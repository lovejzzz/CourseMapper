...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Comes Home',
  highlights: [
    'Scion is now genuinely browser-local. The hosted product no longer sends Scion prompts to Pollinations or any other generation endpoint: it downloads a revision-pinned 3.35 GB public Gemma 4 QAT GGUF directly from Hugging Face, caches it in browser storage, runs inference on WebGPU, and sends compact output through the Scion compiler. Paid-provider paths remain unchanged.',
    'The exact-QAT adapter path is now real from training output to browser rollback. A deterministic bridge maps MLX LoRA tensors into PEFT orientation, a revision-pinned official llama.cpp converter emits a separate 52.7 MB GGUF, and schema-v2 receipts bind the source manifest, mapping, converter, artifact, and inference scale. A real browser verified native Gemma 4 LoRA activation and exact base restoration; this ten-iteration smoke is permanently non-promotable.',
    'Gemma 4 prompt handling and first-use UX are truthful. Scion owns the exact Gemma turn format because the legacy browser chat-template API returned an empty prompt; the corrected browser output is coherent. A persistent progress banner discloses the 3.35 GB first download, model errors fail visibly, CSP allows only the required weight hosts, and the runtime works without cross-origin isolation or risky global auth headers.',
    'Promotion still fails closed. The exact-QAT smoke changed the deterministic course-authoring canary only at scale 16; scales 1 and 4 showed native activation but no output effect. Its 101 structural-evidence pairs are smoke-only, while the independently qualified production corpus remains at zero. The product reports base-only Scion until a real candidate beats exact base-only Scion across five held-out domains with lower compiler burden, factual and device passes, clean exports, and blind instructor preference.',
  ],
  landingHighlights: [
    'Scion prompts now stay in the browser.',
    'The public Gemma base is immutable and hash-identified.',
    'Exact-QAT adapter conversion, loading, and rollback are proven.',
    'No unqualified adapter is presented as trained Scion.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.7.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Browser-local Scion',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Transport boundary: `provider === "public"` remains only for saved-project compatibility. The stream reader intercepts it before remote request construction and invokes browser-local Scion; the shared request builder throws if any Scion code attempts to construct a generation URL.',
        'Exact base: the browser downloads `google/gemma-4-E2B-it-qat-q4_0-gguf` revision `69536a21d70340464240401ba38223d805f6a709`, file `gemma-4-E2B_q4_0-it.gguf`, 3,349,514,112 bytes, SHA-256 `3646b4c147cd235a44d91df1546d3b7d8e29b547dbe4e1f80856419aa455e6fd`.',
        'Exact parent: Hugging Face identifies the source as `google/gemma-4-E2B-it-qat-q4_0-unquantized` revision `1ca4dd94b623b6e0dd9da00c2239ab84b4f3e5ce`. Production adapter training binds this QAT parent rather than the historical non-QAT checkpoint.',
        'Real public base proof: Chrome downloaded the complete immutable artifact directly from Hugging Face in 68.704 seconds, activated WebGPU, confirmed Gemma 4 metadata, and produced coherent base text without a model backend.',
        'No model backend: Scion inference, token streaming, JSON repair, compact-kernel admission, and compilation execute in the browser. Network access is limited to app assets and the pinned public weight download; paid-provider paths remain direct and unchanged.',
        'Reproducible runtime: the shipped module was built from wllama revision `58903000dbea6acfc0eb9c738d8be50d1052cf23`, llama.cpp revision `5ec717d1256e34558a44dc09adf1e6e16f2e2682`, Emscripten 4.0.20, and the repository patch under `runtime/scion-wllama/`.',
        'Asset identity: the packaged JavaScript digest is `ca1b99d084b649a89400d8a839f3e772a46d1f65f145b80d9aa49bc43ddbc41a`; the WebGPU-JSPI WASM digest is `732bac4661d613461cee0b3132660beab159b926910c8431181ee01608e44229`.',
        'Exact-QAT training smoke: a non-adoptable ten-iteration run used 101 structurally evidenced pairs across five explicit domain groups against `google/gemma-4-E2B-it-qat-q4_0-unquantized` at the pinned revision. The dataset is `smoke-only`; the independently qualified production corpus remains at zero.',
        'Deterministic conversion: `mlx-lora-to-peft-to-gguf-v1` validates the source package and exact base, transposes 276 complete LoRA A/B pairs into PEFT orientation, and invokes the official llama.cpp converter pinned at `5ec717d1256e34558a44dc09adf1e6e16f2e2682`. The output is a 52,704,096-byte F16 GGUF with 552 tensors and native `gemma4`/`lora` metadata.',
        'Receipt-bound package: adapter-manifest schema v2 binds the source adapter ID and manifest digest, dataset digest, mapping digest, llama.cpp revision and converter digest, conversion receipt, GGUF bytes and SHA-256, browser runtime, and explicit inference scale. Partial conversion outputs are removed on failure.',
        'Real exact-QAT browser proof: the local-only canary hash-verified the schema-v2 package and GGUF, activated the adapter natively, probed a strict JSON music-theory item, and rolled back to the exact base output. Scale 1 and scale 4 had no deterministic effect; scale 16 changed output and still restored exact base state.',
        'Adapter truth: verified bytes entering native runtime state are insufficient. Scion reports active only after native metadata says Gemma 4 LoRA and a deterministic canary changes; rollback must clear native state and reproduce the exact cached base canary.',
        'Promotion truth: the historical smoke remains excluded by non-QAT base mismatch. The exact-QAT smoke removes that mismatch but remains excluded by smoke-only data, insufficient training, weak normal-scale effect, and missing quality gates. The hosted product remains base-only until a production candidate passes every frozen gate.',
        'Paired-evidence truth: promotion no longer accepts two favorable records merely because they share a domain label. Every adapter/base course pair must bind one unique pair ID, identical input/source/compiler/config/grader/base hashes, a clean compiler tree, the same 12-lesson course identity, the manifest adapter scale, and an explicit base-only control. Every domain must exist in both arms, package grade and P2 findings cannot regress, and duplicate domains, reused pair IDs, or any mismatch fail closed.',
        'Prompt correction: the legacy browser chat-template bridge returned an empty Gemma 4 prompt. Scion now validates roles, protects reserved markers, and emits the exact Gemma turn format; real base inference changed from garbage to a coherent formative-assessment answer.',
        'First-use UX: Model Configuration discloses the 3.35 GB cached weight download, and a global progress banner survives screen changes. Unsupported WebGPU/JSPI, download, identity, and runtime failures surface visibly with no silent remote fallback.',
        'Deployment and privacy: a fresh non-isolated origin proved the single-thread JSPI runtime works without COOP/COEP, avoiding global auth-popup risk. CSP adds only the Hugging Face/Xet weight hosts, and README, Privacy, Terms, capability metadata, pricing provenance, and UI copy now agree.',
      ],
    },
  ],
};
