...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.47 Turns a Non-Promotable Adapter Run into a Faster, Safer Compiler',
  highlights: [
    'The strict source-grounded preference corpus now admits 143 of 145 rows across seven domains, thirty-two course groups, eighty-one task groups, and sixty-four source kernels; the frozen five-domain holdout remains disjoint and the research gate has zero blockers.',
    'The final focused campaign scored fifteen new cases twice in isolated A/B and B/A sessions. Eleven preferences stayed stable across order—eight favoring GPT-5.4-mini and three favoring base Scion—with 86.7% outcome agreement.',
    'Strict semantic admission now detects fifty of seventy-eight measured weak artifacts while preserving every preferred artifact in the regression set; two corpus rows are quarantined instead of being relabeled as valid training examples.',
    'Research training now binds only the strict 143-row dataset, the immutable public Gemma 4 E2B revision, the frozen holdout, a clean Git tree, and the pinned MLX toolchain. Research output is permanently non-promotable without a later held-out adapter win.',
    'Two alternative local authoring prompts were rejected honestly: V3 and V4 both lost badly to the active V2 policy, including a severe key-term collapse, so the weaker experiments remain inactive.',
    'The reproducible run produced a research candidate, not a product claim: diagnostic held-out evidence blocks activation, the corpus is not production-license-compatible, and single-model Codex judgment is not human, instructor, classroom, or paid-reference-parity evidence.',
    'A real Chrome release replay reduced the same difficult one-lesson Scion build from 329 seconds to 143 and 147 seconds while preserving 89/B, texture 91, 9/9 material types, 38/38 export checks, and zero runtime errors.',
    'The Living Course Compiler now narrates nested local attempts and advances through observed retry checkpoints instead of freezing at 35% and jumping directly to 50%; captured frames show 41%, 43%, and 49% before compile.',
    'The frozen Nutrition failure exposed two compiler defects: fused numbered assessment titles and prompt-example leakage. Source-checked list recovery now retains the real lab, midterm, and final project while rejecting unsupported invented cadences.',
  ],
  landingHighlights: [
    'The validation-selected research adapter did not earn promotion; hosted Scion stays base-only.',
    'Real Chrome replays cut the same one-lesson build from 329s to 143–147s.',
    'Nested local retries now move and narrate the Living Course Compiler honestly.',
    'A frozen Nutrition failure became a source-bound assessment parser and prompt repair.',
    'The pinned public Gemma base, model-neutral compiler, and strict holdout remain separate claims.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.47.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Close the evidence gaps without lowering the gate',
      icon: 'AI',
      color: 'blue',
      items: [
        'Three source-capture campaigns broadened the corpus by semantic kernel and then by course group instead of merely repeating forty convenient samples. The final focused campaign targeted only the two remaining readiness blockers: economics judge density and a tenth music-theory source kernel.',
        'Every judged case was source-grounded, anonymized, independently scored in both A/B and B/A presentation order, and sealed before organizer mapping was applied. The last fifteen cases yielded eleven stable, score-qualified preferences.',
        'The strict rebuild admits 143 of 145 rows across seven domains and thirty-two course groups. Four domains meet the twenty-row model-judge floor; every domain meets the group, task, and source-kernel diversity contract required for research.',
        'The five held-out domains and five held-out course groups overlap with zero admitted training rows. Two weak chosen artifacts remain visible in quarantine rather than disappearing into a headline count.',
      ],
    },
    {
      label: 'Make research training reproducible and fail closed',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The research launcher now consumes the exact readiness corpus under the strict semantic profile instead of silently rebuilding from older default sources. Its dataset identity binds source bytes, split bytes, license disclosure, semantic admission, and the holdout firewall.',
        'Training still refuses a dirty repository, a drifting MLX module, a different Gemma revision, an altered split, implicit ORPO parameters, or an existing non-empty output directory. The adapter ID derives from the complete canonical training plan.',
        'One-prompt targeted source campaigns now materialize with a one-prompt minimum instead of inheriting the two-prompt floor used by larger groups. Tests cover generation, recovery, and verification under the same rule.',
        'Research-only source snapshots are kept outside the live Curriculum Genome manifest. Historical capture audits can bind their exact immutable snapshot while the production shard selector remains unchanged.',
      ],
    },
    {
      label: 'Keep failed ideas and claim boundaries visible',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'The active V2 authoring policy remains the winner. V3 fell from 77 to 41 raw admitted atoms and V4 fell to 31; V4 also collapsed admitted key terms from 34 to one. Neither experiment is activated.',
        'Strict admission intercepts fifty of seventy-eight measured weak artifacts with zero preferred-artifact regressions. This is a compiler-quality boundary, not proof that every admitted artifact is factually or pedagogically excellent.',
        'The frozen v0.16.22 MC-recovery replay now opts explicitly into its retired lexical-only key-repair rule so the historical receipt still reconstructs. Production remains on the stricter source-or-explicit-cue boundary and does not inherit that compatibility switch.',
        'The implementation-bound v2 benchmark receipt was re-derived after formatting its bound grader modules. The wrapper hash and ten-file dependency count remain unchanged, while the fail-closed tooling test proves that a stale transitive receipt cannot start a promotion run.',
        'Fast verification no longer depends on an ignored local organizer packet. Both paired-order audits deterministically rebuild the 120-case packet from tracked inputs before reconstructing the workbooks, and reconstruction selects the receipt-bound historical task-guide wording so a later decision-token clarification cannot invalidate an already sealed campaign. The current readiness alias now targets v0.16.47; historical v0.16.46 and research-authorization receipts explicitly retain the v1 holdout, while the later implementation-bound v2 ruler remains promotion-only.',
        'The corpus includes noncommercial and share-alike rows that are allowed only in the isolated research lane. Production training remains blocked until those rows are replaced or explicitly cleared.',
        'No adapter is active or promoted in the website. The reproducible research run completed, but its validation-selected candidate did not earn activation in the diagnostic held-out comparison. A complete implementation-bound v2 base-versus-adapter-versus-reference win remains mandatory.',
      ],
    },
    {
      label: 'Make the living compiler faster without hiding the work',
      icon: 'AI',
      color: 'blue',
      items: [
        'The browser-local Gemma release path was replayed end to end in real Chrome with the same one-lesson usability-testing prompt. The baseline completed in 329 seconds; two optimized builds completed in 143 and 147 seconds, a 145-second median and 55.9% reduction.',
        'The one-lesson recovery budget now reserves one outer call instead of four. Each outer call still owns its bounded three-attempt local loop, reducing the maximum enrichment completions from fifteen to six without removing deterministic compilation or final verification.',
        'Both optimized runs retained the same 89/B result, texture 91, all nine generated material types, one export action, no horizontal overflow, and zero browser runtime errors. The finalizer passed all 38 export checks with no warnings.',
        'The Living Course Compiler component and pure selector now form their own 63.1 KiB raw / 19.7 KiB gzip workspace-route chunk with a 65/21 budget. AppFlow falls from 269.4/81.0 to 251.6/75.9 and clears its existing ratchet without raising the limit. An HTML-preload filter and bundle guard keep that workspace-only chunk off landing, whose initial JavaScript falls from 334.5/108.2 to 327.5/105.8 KiB raw/gzip.',
        'This is a measured one-lesson browser result, not a universal throughput claim. Larger courses keep a recovery allowance that scales up to the existing four-call ceiling.',
      ],
    },
    {
      label: 'Let every real retry move the progress story',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        "The ribbon now folds the runtime's inner stream retries and the compiler's outer recovery into one observable checkpoint sequence. It does not use a decorative timer or infer completion from elapsed time.",
        "The captured release path advanced 35 → 38 → 41 → 43 → 46 → 49 → 50. Browser frames show “Retrying local lesson kernel · attempt 3/3” at 41%, “Recovery 1/1 — lesson 1” at 43%, and the recovery's final local attempt at 49%.",
        'Targets still use the existing 500 ms ease-out width transition, and reduced-motion mode still disables both the transition and pulse. Progress remains monotonic and 100% remains reserved for a terminal package state.',
      ],
    },
    {
      label: 'Turn the Nutrition failure into a source-bound compiler repair',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'The frozen Nutrition package failed because two weighted assessment-list items were fused into single titles. A trailing invented response noun made the real midterm look generic, so no midterm exam document or answer key was compiled.',
        "The unsupported “weekly autograded quizzes” and “weekly reading responses” cadences were traced to concrete examples in Scion's own authoring instructions. Those copyable phrases are gone, and the prompt now requires source-only recurring cadence without supplying a genre to imitate.",
        'Native graph assembly splits only sequential weighted lists where every part has an explicit percentage and assessment identity noun. Each recovered part must also match the instructor source by assessment genre; unsupported parts are dropped rather than normalized into the course.',
        'The exact fourteen-session regression now retains three atomic source-backed assessments, classifies the midterm as an exam, and compiles its question bank and answer key. An adversarial ordinary “phase 2” project title stays single.',
      ],
    },
  ],
};
