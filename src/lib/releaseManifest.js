...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.57 Keeps Every Build Alive and Intact',
  highlights: [
    'Browser-local Scion now serializes completion calls, reloads the cached base after a fatal worker failure, retries once, and quarantines repeated runtime death; a real six-lesson build completed all six knowledge kernels without callback storms or fatal console errors.',
    'An explicit semicolon or numbered topic sequence becomes an indexed lesson contract. The compiler also detects duplicate and renamed lesson identities, including CPI aliases, and blocks export when a generated continuation repeats a topic.',
    'The Living Course Compiler names the active lesson and retry attempt, keeps local runtime internals out of instructor-facing copy, and reaches a real terminal state instead of leaving a generic retry or model-download message behind.',
    'A course-map-only workspace now uses the real Agent loop. The missing retry-state bridge is restored, Scion is shown as the model name, and deterministic structural questions use compiler-owned evidence before model prose.',
    'Legacy full-map revision output can no longer delete later lessons while JSON is still partial. Any unrequested lesson-count reduction is discarded and the previous map is restored.',
    'A 37-second live six-week Macroeconomics map followed all six requested focuses in the exact requested order. Agent checks preserved all six lessons and a free-form Scion explanation completed without mutating the workspace.',
    'At 390×844 the workspace has zero document-level horizontal overflow, all 16 visible controls meet a 40×40 minimum, Content/Agent/Export remain distinct, and export actions stay inside the single export panel.',
    'A new 14-case, seven-domain paired-order diagnostic retains every packet, review, replay, and preference file. The paid reference wins all 14 stable comparisons, but all 14 fail strict score qualification and zero rows enter training.',
    'No hosted adapter is active and no Gemma weight changed. Shared prompt, admission, sequencing, revision safety, and compiler fixes can benefit paid-model routes; browser runtime recovery remains Scion-specific.',
  ],
  landingHighlights: [
    'Fatal local workers recover from the cached base instead of poisoning later calls.',
    'Requested topic sequences become exact indexed lesson contracts.',
    'Duplicate and renamed lesson topics block export before they become a package.',
    'Map-only chat now reaches the real Scion Agent and preserves the whole course.',
    'Partial revision JSON cannot silently delete later lessons.',
    'A live six-week map follows all six requested focuses in 37 seconds.',
    'Phone-width Content, Agent, and Export stay usable with zero page overflow.',
    'Fourteen stable reference wins still produce zero training rows under the score gate.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.57.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01657_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Keep the local runtime alive',
      icon: 'RECOVER',
      color: 'emerald',
      items: [
        'Completion calls are serialized so one browser worker never receives overlapping generation jobs from Agent, enrichment, and recovery paths.',
        'A fatal worker signature unloads the broken runtime, reloads the already cached pinned base, and retries the interrupted completion once. Repeated failure enters a clear recovery-required state instead of creating a callback storm.',
        'The real six-lesson package replay completed six of six lesson kernels in the same browser session with no fatal worker, callback-ID, abort-signal, or unreachable errors in the final console audit.',
      ],
    },
    {
      label: 'Make the requested course sequence authoritative',
      icon: 'MAP',
      color: 'blue',
      items: [
        'Scion extracts explicit semicolon-delimited or numbered lesson focuses from the instructor brief and injects one exact indexed focus per lesson into both the initial and continuation prompts.',
        'A shared normalized title identity removes lesson numbering, normalizes punctuation and conjunctions, and expands CPI before comparison. Duplicate identities are both review findings and export-blocking pedagogical errors.',
        'The live Macroeconomics replay produced scarcity, supply and demand, CPI, unemployment, AD/AS, and fiscal-versus-monetary policy in the requested six-lesson order, completing the map-only package in 37 seconds.',
      ],
    },
    {
      label: 'Protect the course from the Agent',
      icon: 'AGENT',
      color: 'violet',
      items: [
        'The map-only workspace no longer sends every message through the legacy full-map revision path. It uses the same Agent loop and Scion provider as richer packages.',
        'The missing retry-state setter that crashed the first live Agent check is now exported and defensively optional. Public-provider failures use calm Scion copy rather than raw callback or function names.',
        'A legacy revision fallback cannot paint partial full-map JSON into the workspace. If a response returns fewer lessons without an explicit removal request, the result is rejected and the previous map is restored.',
        'Exact duplicate-topic questions are answered from compiler-owned title identities. Free-form explanation still reaches the local Scion model, and constrained-decoder possessive punctuation is cleaned before display.',
      ],
    },
    {
      label: 'Make the live compiler readable frame by frame',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'Retry narration names the active lesson and inner attempt. Map streaming, source lookup, knowledge enrichment, compilation, verification, and grading use user-facing Scion language instead of internal backend identifiers.',
        'Course FAQ search now occupies a full row beneath the counters, informational package notes stay blue, and the header no longer competes with a second download action.',
        'At 390×844 the page scroll width remains 385 for a 390-pixel viewport. All 16 visible buttons in the audited frame meet at least 40×40 pixels, and the Content, Agent, and Export switch remains legible.',
      ],
    },
    {
      label: 'Expand the ruler without lowering it',
      icon: 'PROOF',
      color: 'amber',
      items: [
        'A deterministic selector chooses 14 new cases: two from each of seven training domains, 14 distinct course groups and source kernels, and coverage of all nine declared failure families. Held-out domains and the earlier pilot remain excluded.',
        'Exact local and paid-reference captures were replayed through the current compiler, then judged in 28 isolated A/B and B/A sessions. The reference wins all 14 stable pairs; local wins zero and no order is unstable.',
        'All 14 reference winners fail the strict score qualification because source fidelity, critical defects, or score floors remain insufficient. The preference output is intentionally empty, so no adapter training begins.',
        'The evidence package retains and hash-binds selection, captures, replays, packets, reviews, paired results, empty preferences, and implementation files. Sparse-complete workbooks now report complete only after every selected batch is reviewed.',
      ],
    },
    {
      label: 'Keep the model boundary honest',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'V0.16.57 changes orchestration, prompts, admission, validation, Agent routing, revision safety, UI language, and evaluation receipts. It does not alter the pinned Gemma weights and does not activate an adapter.',
        'Shared topic contracts, duplicate blocking, revision safety, compiler grounding, and downstream validation can benefit paid-model routes when they use the same pipeline. Browser worker recovery and cached-base reload are Scion-local.',
        'The 14-case result demonstrates the remaining gap. It is single-model-judge diagnostic evidence, not human preference, instructor validation, aggregate quality, held-out adapter victory, or paid-reference parity.',
      ],
    },
  ],
};
