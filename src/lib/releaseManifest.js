...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Device Truth',
  highlights: [
    'The browser-device gate now freezes four distinct profiles: Chrome on integrated 8 GB hardware, Edge on integrated 16 GB hardware, Chrome or Edge on a discrete 8 GB GPU, and Chrome on Apple Silicon with at least 16 GB unified memory. Apple Silicon no longer substitutes for discrete-GPU coverage.',
    'Every device run binds one stable adapter-package identity covering its model, exact base, training data, files, runtime, conversion, and manifest scale. Only the mutable promotion block is excluded, preventing an impossible manifest-to-evidence hash cycle without weakening package identity.',
    'A passing profile must prove cold and warm base loads, valid base and adapter completions, native output-changing activation at manifest scale, exact rollback, three repeated completions, memory budgets, interrupted-download recovery, storage-pressure recovery, and WebGPU device-loss recovery.',
    'Browser traces, console logs, sanitized hardware probes, and runtime snapshots are byte-counted and SHA-256-verified inside the evidence directory. The promotion audit now reruns the semantic matrix after checking the outer file digest, so a hash-correct JSON file labeled pass is no longer enough.',
    'This release hardens the test; it does not claim a win. No promotable quality adapter exists, the qualifying matrix remains 0/4, independent reviews and approved training pairs remain zero, and hosted Scion continues to run the pinned public base plus model-neutral compiler.',
  ],
  landingHighlights: [
    'Four frozen browser and hardware profiles replace vague device claims.',
    'Exact adapter identity survives promotion evidence updates.',
    'Memory, recovery, activation, and rollback are semantic gates.',
    'Retained browser artifacts are verified byte for byte.',
    'Hosted Scion remains base-only until every quality gate passes.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.12.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Frozen device protocol',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Profile separation: integrated-8 GB, integrated-16 GB, discrete-8 GB, and Apple-Silicon-16 GB are distinct required seats. Apple Silicon cannot satisfy the discrete seat.',
        'Browser coverage: Chrome is required on low-memory integrated and Apple Silicon; Edge is required on 16 GB integrated hardware; the discrete seat accepts Chrome or Edge.',
        'Identity contract: `scion-adapter-package-identity-v1` covers adapter ID/version/format/scale, exact base, training manifest and counts, packaged files, runtime, and deterministic conversion metadata.',
        'Cycle avoidance: the promotion block is the only manifest field excluded from stable package identity because it contains the device-evidence digest. The audit recomputes the remaining identity from the candidate manifest.',
        'Memory contract: each run reports native system memory and browser working-set peak; the discrete run also reports dedicated VRAM and peak GPU use. Budgets are calculated from frozen profile fractions rather than trusted booleans.',
        'Recovery contract: network-abort, cache/quota, and actual WebGPU-device or GPU-process recovery methods are enumerated. Unknown labels or merely reloaded pages cannot silently pass.',
      ],
    },
    {
      label: 'Semantic promotion evidence',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Scenario verification: cold and warm load, base completion, adapter integrity, native activation, adapter completion, exact rollback, repeat completion, and all three recovery trials are individually checked.',
        'Manifest-scale effect: native adapter metadata alone does not pass. The deterministic activation canary must change at the exact scale named by the candidate package.',
        'Artifact integrity: every run retains one browser trace, console log, sanitized hardware probe, and runtime snapshot. Paths must be relative and contained; symlinks, traversal, duplicate identities, changed bytes, and missing files fail.',
        'Digest integrity: every run has a canonical self-digest and the complete matrix has a manifest-declared SHA-256. Rehashing a failed run only proves its bytes; it does not turn failed recovery into a pass.',
        'Promotion integration: `audit:scion:adapter:promotion` parses the device file and reruns the semantic audit after outer-digest verification instead of trusting `type`, `status`, and `sha256` labels.',
        'Historical boundary: the v0.16.7 exact-QAT smoke proved one Chrome/Apple-Silicon activation and rollback path but omitted the other profiles, measured memory, and recovery matrix, so it remains mechanical-only.',
        'Quality boundary: zero qualifying candidate device profiles exist. This release changes no weights, promotes no adapter, and makes no instructor-readiness or paid-model-superiority claim.',
        'Product boundary: Scion V0.16.12 remains the pinned public browser-local Gemma base plus the versioned, model-neutral compiler.',
      ],
    },
  ],
};
