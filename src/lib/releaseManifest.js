...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion’s Compiler Now Intercepts 12 of 46 Measured Losses—Without Guessing or Rewriting Text',
  highlights: [
    'The exact 46-loss replay now intercepts twelve failures instead of nine: six contradicted keys are repaired and six unusable items are rejected, with zero model-authored text mutation.',
    'Three additional wrong keys are realigned only from the first affirmative sentence, conservative word-form agreement, a unique two-token minimum, and a positive score margin.',
    'Adversarial guards refuse literal option labels, generic “correct choice” prose, negative or misconception sentences, ties, and the measured ambiguous UX case rather than guessing.',
    'Across 192 retained source atoms, key repairs rise from 52 to 57 while admission remains 141 and burden remains 51; five weak deterministic-only training pairs are quarantined, reducing the usable corpus from 123 to 118.',
    'Thirty-four measured losses remain unresolved, the corpus still has only 46/100 judge preferences, and no quality adapter is trained or active.',
  ],
  landingHighlights: [
    ...
    'Five weak deterministic training pairs are quarantined instead of counted.',
    'Thirty-four measured losses and the 54-row judged-evidence shortfall remain.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.44.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Use the model’s own affirmative evidence',
      icon: 'AI',
      color: 'blue',
      items: [
        'The v0.16.43 compiler already intercepted nine of the 46 stable paired-order Scion losses. v0.16.44 adds three safely recoverable answer-key contradictions: `open()` returning a file object, Python list access by position, and pyroclastic flow as the named explosive-volcano hazard.',
        'The new path reads only the first affirmative sentence. It conservatively normalizes plural and verb endings, requires at least two overlapping content tokens, requires one unique best option, and requires that option to beat the declared key. The exact sentence, score vector, thresholds, old index, and new index are retained in every repair receipt.',
        'Literal option labels remain the higher-authority path. Generic “correct choice” prose, negative and misconception language, tied scores, weak one-token paraphrases, and the known ambiguous UX task-flow item all refuse repair.',
        'The exact replay now intercepts 12/46 losses: six answer-index repairs plus the existing six regeneration rejects. Only the answer-index field changes; response-text mutations remain zero. Thirty-four judged losses remain explicitly unresolved.',
      ],
    },
    {
      label: 'Measure the broader cost, not just the wins',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The twelve-project retained-source replay performs five more answer-key realignments, raising key-alignment receipts from 52 to 57 and total deterministic repairs from 72 to 77. It still admits 141/192 atoms, leaves burden at 51/192, and mutates zero retained response strings.',
        'Running the stricter gate over all 464 stored training candidates removes five deterministic contract margins that no longer survive current semantic admission. The usable corpus therefore moves from 123 to 118, with 72 deterministic rows plus the unchanged 46 paired-order judge preferences.',
        'The v0.16.43 readiness profile remains byte-reconstructible at 123/464 and keeps its v0.16.43 compiler-replay binding. Historical evidence is preserved instead of being silently rescored under the new rule.',
        'This release improves compiler handling of measured key contradictions; it does not change Gemma weights, add judged preferences, train an adapter, prove a held-out win, or close the paid-reference gap. The adapter remains inactive and the next evidence gate still requires 54 more stable preferences.',
      ],
    },
  ],
};
