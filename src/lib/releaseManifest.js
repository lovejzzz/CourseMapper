...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Adapter Promotion Now Recomputes Every Quality Canary',
  highlights: [
    'Adapter evidence now binds a promotion-independent package identity instead of the SHA-256 of the manifest that contains the evidence SHA-256. This removes the circular hash introduced by v0.16.28 while preserving exact base, adapter, file, runtime, training, and conversion identity.',
    'Factual promotion evidence is no longer hash-only. The audit requires exactly two cold and two source-grounded browser runs, the exact frozen 25-case packet, one request per case, exact runtime and adapter identity, retained raw option text, and independently recomputed answer and per-domain floors.',
    'Production promotion evidence is no longer hash-only. Exactly three predeclared live-browser runs across at least two domains must cross-bind regular non-symlinked ZIP, trace, console-log, and native runtime-receipt bytes; the ZIP manifest, trace gates, app commit, visual QA, adapter identity, and 99/0/0/0 quality floor are recomputed.',
    'All four external adapter gates—factual canaries, single-model judgment, browser-device matrix, and production canaries—now receive semantic verification. Published factual and production templates, standalone audits, and adversarial fixtures reject endpoint relabeling, repeated runs, refreshed false summaries, traversal, missing receipts, and pass-shaped dummies.',
    'This is a stronger and now constructible promotion ruler, not a model-quality result. No fresh B/A judgment, quality adapter, new Gemma weights, five-domain adapter win, paid-reference parity, or complete device matrix is claimed; hosted Scion remains the pinned public base plus compiler.',
  ],
  landingHighlights: [
    'The adapter identity is exact without an impossible circular hash.',
    'Four factual trials are rescored from retained raw answers.',
    'Three production packages must agree with their ZIPs, traces, logs, and runtime receipts.',
    'Every external adapter gate is now semantic, not just hash-shaped.',
    'No new adapter or model-quality win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.29.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Constructible exact identity',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The adapter package identity hashes schema, adapter, base, training, files, runtime, and conversion fields while deliberately excluding mutable promotion attestations.',
        'Adding the factual evidence hash to a candidate manifest therefore does not change the identity recorded inside that factual evidence. The contract audit proves this invariant directly.',
        'Paired Crucible capture, the MLX compatibility runtime, the single-model judge, and the promotion runner now carry the stable package identity. A raw manifest hash remains capture metadata but is no longer the promotion identity.',
        'The v0.16.28 judgment wrapper migrated from manifestSha256 to packageIdentitySha256, so its exact semantic requirements remain enforceable without a fixed-point hash problem.',
      ],
    },
    {
      label: 'Semantic canaries, honest boundary',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Factual evidence must complete exactly four preregistered runs: two cold and two source-grounded. Every 25-answer vector is rebuilt from retained exact option text and rescored against the canonical packet; grounded runs must be perfect in every domain and cold runs must reach 23/25.',
        'Production evidence must contain exactly three recent public-Scion browser runs across at least two domains. Every run needs twelve lessons, 99 quality, zero P0/P1/P2, clean compiler identity, Codex visual QA, and complete provider requests.',
        'The audit opens the retained ZIP, reads PACKAGE_MANIFEST.json, checks its file count, app version, readiness, and quality against the run; it also parses the trace and a separate native runtime receipt and cross-checks every referenced digest.',
        'Subsidiary evidence paths must be relative regular files confined to the campaign directory. Absolute paths, traversal, escaping real paths, symlinks, duplicate run IDs, duplicate run artifacts, runtime relabeling, and summary-only pass objects fail closed.',
        'The general promotion runner now invokes these semantic audits alongside the existing semantic judge and browser-device matrix instead of treating factual and production canaries as hash-bound booleans.',
        'No existing base-only production canary can certify a future adapter because it lacks exact adapter and runtime-receipt identity. That is an intentional migration boundary, not a retroactive model failure.',
      ],
    },
  ],
};
