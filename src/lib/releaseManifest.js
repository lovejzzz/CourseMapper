...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Now Rejects Copied Misconceptions and Shows Its Whole Build',
  highlights: [
    'The model-neutral key-term gate now rejects embedded field labels, internal claim markers, copied definitions/examples/misconceptions, and lesson facts mislabeled as misconceptions before those atoms can compile.',
    'A keyless receipt replays 82 source-bound key-term cases per model across both sealed judge orders: the stronger gate rejects 19 Scion-base cases, including 14 newly caught cases, and every one has defects in both readings; it rejects 0 GPT-5.4-mini cases in this frozen subset.',
    'Scion’s first-use model preparation is now step one of a visible six-stage meter that continues through Map, Enrich, Compile, Verify, and Grade using observable work rather than a decorative timer.',
    'Downloadable review notes moved from amber warning language to a calm blue information state, and the duplicate header Download ZIP action was removed so export has one clear owner.',
    'The embedded Agent now connects to the active browser-local Scion runtime for grounded, streaming advisory answers with no API key; it remains text-only and never pretends it changed the workspace.',
  ],
  landingHighlights: [
    'Copied or factual “misconceptions” now fail before compilation.',
    'One visible meter follows Scion from model setup to final grade.',
    'Review notes look informative instead of alarming.',
    'Download ZIP now appears only in the export panel.',
    'The built-in Agent can answer through local Scion.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.34.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Catch copied teaching atoms before they compile',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Definitions, examples, misconceptions, and corrections must now make distinct instructional moves. The gate catches copied clauses, embedded field labels, and internal claim-number residue instead of allowing them into learner-facing output.',
        'A misconception must be genuinely false. When the response supplies lesson facts, the same admission boundary rejects a misconception that simply relabels one of those known facts. Failed local atoms enter the existing bounded Scion retry with focused feedback; the compiler does not invent a replacement belief.',
        'The reproducible keyless receipt is retrospective single-model Codex evidence, not training data. Across 82 key-term cases per model and two sealed orders, 19 local cases are now blocked, 14 are new catches, every blocked local case has a defect in both readings, and no paid-reference artifact is rejected in the frozen subset. Fifty-nine any-order local defects remain outside this deliberately high-confidence gate.',
      ],
    },
    {
      label: 'A calmer, observable local product',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The download meter now introduces itself as step one of six and shows both model-transfer and overall progress. The workspace continues the same scale from 15% through the five compiler stages, deriving map progress from the stream, enrichment progress from the active lesson, compilation progress from real material counts, and completion from the finish state.',
        'A downloadable package with advisory notes now reads Ready to download with a blue Review notes in Agent treatment. Amber remains reserved for an actual recommendation to review, and red remains blocked. Export itself now has one Download ZIP button in the export panel; the old header event bridge is gone.',
        'Agent chat recognizes keyless Scion as connected and streams answers through the pinned browser-local runtime. This first connection is intentionally advisory because the public base does not expose native tool calls; Scion is told not to claim workspace edits, while existing deterministic local Agent commands remain available.',
        'This release changes the shared compiler and browser product, not Gemma weights. It creates zero preferences and activates no adapter. The frozen evidence proves only a high-confidence admission improvement and no observed GPT-5.4-mini regression on this subset—not factual correctness, general model parity, human validation, or an adapter win.',
      ],
    },
  ],
};
