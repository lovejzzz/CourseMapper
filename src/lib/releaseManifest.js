...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.64 Is Faster, Distinct, and Downloadable',
  highlights: [
  ...
  landingHighlights: [
    'A real 29-minute production failure becomes a 3-minute measured build.',
    'Duplicate course-map continuations are rejected before they reach the package.',
    'Verified drafts stay downloadable even when instructor review is required.',
    'Older finished projects recover their ZIP without regenerating the course.',
    'Compact fact ledgers replace wasteful full-lesson rewrite loops.',
    'The exact Genetics replay produces 15 distinct lesson identities.',
    'A 2.3 MB ZIP downloads and extracts into 126 files.',
    'Landing JavaScript shrinks from 703.0 KiB to 251.7 KiB raw.',
    '5,178 tests and the full browser/export release gate pass.',
    'Gemma weights are unchanged and the adapter remains inactive.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.64.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01664_PRODUCTION_RECOVERY.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Reconstruct the production failure from evidence',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'The attached production log spans 1,756,578 ms and records 64 provider requests, 38 stream retries, four continuation retries, and three duplicate topic groups.',
        'The generated package passed all 38 physical export checks at 89/B with zero P0 findings, proving that the disabled ZIP was a state-contract defect rather than a corrupt archive.',
        'Every normal lesson attempted the rich kernel contract repeatedly, while four course-map continuations returned the same three topics; both waste paths are now explicit regression fixtures.',
      ],
    },
    {
      label: 'Make course-map continuation fail safe',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The production “Existing lessons … Generate ONLY Lessons …” prompt now parses the real boundary instead of silently dropping every prior title.',
        'Continuation calls remain in the course-map task family rather than being mislabeled as repair work, keeping routing, budgets, and telemetry honest.',
        'A normalized identity admission set rejects repeats against the existing map and within the candidate batch, rebases accepted numbering, and tells the next bounded attempt which topics were rejected.',
      ],
    },
    {
      label: 'Return valid work without weakening publish readiness',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'A package that still needs instructor decisions remains visibly blocked and cannot be presented as publish-ready.',
        'When its finished files pass export verification with zero failures, the single export-panel owner exposes Download draft ZIP and carries the blocked readiness plus quality report into the archive.',
        'Receipts from V0.16.61–V0.16.63 recover a verified draft from exportChecked/exportFailed counters when the older finalizer omitted exportStatus.',
      ],
    },
    {
      label: 'Spend model calls on knowledge, not formatting',
      icon: 'LIVE',
      color: 'violet',
      items: [
        'Scion synthesis requests a compact fact ledger, freezes admitted facts, and lets deterministic projection build lesson, quiz, slide, rubric, and export surfaces.',
        'Retry decisions occur after compiler projection, so a usable fact ledger cannot trigger another provider call merely because it omitted compiler-owned formatting.',
        'The full Genetics replay uses 26 transport requests / 19 logical pipeline calls versus the production baseline of 64 provider requests, without introducing a second model or backend.',
      ],
    },
    {
      label: 'Prove the real Genetics recovery',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'A fresh base-only browser run produces 15 distinct lesson identities from Mendelian inheritance and meiosis through epigenetics, modern technologies, labs, and assessment.',
        'The generation digest completes in 186,729 ms; the end-to-end harness finishes in 200 seconds, downloads a 2,393,572-byte ZIP, and extracts 126 files.',
        'The independent grader reports 89/B, zero P0, two P1, and one P2; 14/15 lesson kernels are enriched and the final lesson remains honestly marked as compiler fallback.',
      ],
    },
    {
      label: 'Remove the landing-load regression',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'A narrow artifact-label helper no longer imports the full language finalizer into the rotating-copy chunk.',
        'Initial landing JavaScript falls from 703.0 KiB raw / 215.9 KiB gzip to 251.7 KiB / 79.5 KiB, while compiler, grader, workspace, and exporters stay lazy.',
        'Bundle budgets explicitly retain narrow workspace-only allowances for the verified-draft contract and semantic assessment admission; no landing-budget increase is granted.',
      ],
    },
    {
      label: 'Ship with an evidence-bound claim',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'The local release gate passes 423 test files and 5,178 tests plus format, lint, production build, bundle budgets, browser generation, ZIP download, extraction, and deterministic grading.',
        'The retained receipt binds the production log and screenshot hashes, candidate digest/report/ZIP hashes, measured deltas, course titles, export checks, and remaining quality gap.',
        'Gemma weights are unchanged and the research adapter remains inactive. This is a compiler, orchestration, UX, and export recovery—not factual validation, human approval, or paid-model parity.',
        'Compatible paid routes inherit the shared continuation, semantic admission, compilation, grading, and export behavior; browser model loading and local inference remain Scion-specific.',
      ],
    },
  ],
};
