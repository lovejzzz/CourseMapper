...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion’s Next Judge Is Selectable, Explicit, and Still Blind',
  highlights: [
    'The v0.16.35 workbook is superseded before scoring because its declared Codex revision was not a provider build identity that Codex Desktop exposes or this repository could verify.',
    'The replacement workbook pins a selectable launch profile—gpt-5.5 with xhigh reasoning on Codex Desktop—and explicitly records that the internal provider build revision is unavailable.',
    ...
    'A new tracked A/B-only workbook retains all 128 source-backed cases in eight immutable chunks, while both v0.16.35 and v0.16.36 continue to reconstruct byte-for-byte as separate historical and current artifacts.',
    'This task has prior campaign context and does not judge the packet: the release creates zero decisions, zero preferences, zero training rows, zero weights, and zero measured Scion quality or speed change.',
  ],
  landingHighlights: [
    'An unverifiable judge label was retired before it produced evidence.',
    'The next task must explicitly launch gpt-5.5 with xhigh reasoning.',
    'The receipt admits that Codex’s internal provider build is not exposed.',
    'All 128 anonymous cases remain blind, blank, and reproducible.',
    'No quality gain is claimed before a genuinely fresh judgment exists.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.36.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Replace a fictional revision with a selectable profile',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The v0.16.35 clean room named codex-gpt-5-2026-07-15 as a revision even though Codex Desktop does not expose an internal provider build revision that the workflow can verify. It was never scored and is now retained only as superseded historical evidence.',
        'The replacement launch profile is explicit and user-selectable: model gpt-5.5, reasoning effort xhigh, runtime codex-desktop, and identity token gpt-5.5@xhigh. The token identifies the public launch controls; it is not presented as an internal build hash.',
        'gpt-5.5 is selected for broad cross-domain factual and instructional judgment, while xhigh gives the sole judge the strongest available reasoning profile. The receipt records internalBuildRevisionAvailable as false instead of filling the gap with an invented label.',
      ],
    },
    {
      label: 'Fail closed before a contaminated score exists',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'New first-order construction requires an explicit Codex launch profile. The verifier rejects absent profile data, an unsupported reasoning setting, non-explicit selection, runtime disagreement, a token that differs from model@reasoning, or a relabeled model.',
        'The v0.16.36 clean room still contains only the prompt, instructions, eight A/B review chunks, eight blank decisions, and a manifest. It exposes no B/A payload, organizer mapping, unblinded identity, outcome, aggregate, or completed judgment.',
        'The current task has seen prior campaign aggregates, so it cannot honestly set the fresh-context attestations and does not score a case. Both the superseded and replacement workbooks reconstruct independently; only the replacement is eligible for a new judge task.',
        'The measured quality and speed change is exactly zero. The next valid action is a newly created Codex task explicitly launched as gpt-5.5 with xhigh reasoning and given only the v0.16.36 workbook.',
      ],
    },
  ],
};
