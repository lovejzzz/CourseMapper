...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Passes Its First Recovery Device Profile',
  highlights: [
    'Scion now has a reproducible isolated-Chrome device runner that captures a real public-base download, exact cached SHA-256, native adapter lifecycle, browser working set, recovery scenarios, and four byte-bound artifacts with deterministic trace privacy checks and no model weights in Git.',
    ...
    'The scale-16 smoke adapter hash-verified, activated through native Gemma 4 LoRA, changed the deterministic output, and rolled back to the exact base-output and project-data digests. Native proof now returns its bound runtime metadata to future capture callers.',
    'This is one passing recovery profile out of four for a permanently non-promotable smoke adapter. Integrated 8 GiB, integrated 16 GiB with Edge, and discrete 8 GiB remain missing; no quality adapter, five-domain win, paid-reference parity, or fresh B/A judgment is claimed, and public Scion remains base-only.',
  ],
  landingHighlights: [
    'A real Apple-Silicon Chrome recovery profile now passes.',
    'The 3.35 GB public base was downloaded and hash-verified.',
    'Network abort, cache eviction, and GPU restart recovered.',
    'Adapter activation changed output and rolled back exactly.',
    'The full matrix is 1/4; this is not a quality-model win.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.25.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'A device run, not a simulated receipt',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The capture runner launches the installed Google Chrome in a dedicated profile and serves only the local lab, runtime assets, and separate smoke adapter package.',
        'The base still comes directly from the immutable public Hugging Face URL. The runner finds the exact OPFS file and independently computes its full SHA-256 before accepting the run.',
        'The browser-side lab uses the product runtime and registry for load, completion, bounded adapter installation, activation, deactivation, eviction, and recovery.',
        'The retained run binds Chrome 150, macOS 26.5.1 arm64, Apple M4 Max hardware, 48 GiB memory, all timings, output digests, and a 5,312 MiB peak working set.',
        'Browser trace, console log, sanitized hardware probe, and redacted runtime snapshot are individually byte-counted and SHA-256-bound inside the evidence directory. Trace text is scrubbed of local paths and audited for local absolute paths, cookies, authorization headers, API keys, and secret-bearing URLs before acceptance.',
      ],
    },
    {
      label: 'Recovery is observed; the boundary stays narrow',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'A real download abort was observed after 8,731,096 bytes; the subsequent cold load completed from the public URL and produced the exact pinned 3,349,514,112-byte digest.',
        'The separately cached adapter was evicted, downloaded again through the bounded registry, verified, and left the base usable.',
        'Chrome DevTools restarted the actual GPU process. The old runtime completion failed, then unload, warm reload, and a fresh base completion succeeded.',
        'Native activation changed the canary digest; native clearing restored the original base digest and the synthetic project-data digest stayed identical.',
        'Future runtime proofs expose the native status already bound inside their proof hash, closing the observability gap found during this real run; the v0.16.25 lifecycle receipt rebinds the changed runtime and focused tests.',
        'The semantic verifier accepts the Apple-Silicon run and reports exactly three missing profiles. The evidence audit treats that incomplete matrix as the expected, non-promotable result.',
        'This smoke result says nothing about educational improvement. The fresh reverse-order B/A judgment remains the next separate training dependency.',
      ],
    },
  ],
};
