...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Uses One Verified Adapter Path',
  highlights: [
    'The localhost mechanical browser canary no longer fetches manifests or adapters through unchecked arrayBuffer() calls. It now uses the same bounded registry install, verification, activation, and deactivation path as the product architecture.',
    'Cached adapters retain their original manifest bytes. Reuse re-hashes those bytes, compares the parsed manifest record, rechecks every file reference, byte count, and SHA-256, and skips the adapter download only when the exact cache remains valid.',
    'A different manifest can no longer replace an adapter under an ID that the runtime reports active. The registry rejects that split-brain update after the trusted manifest is checked but before any replacement file is downloaded.',
    'Base-only state now requires positive rollback proof. If native clearing or the exact deterministic base-output check fails, Scion enters recovery-required, reports native state as unknown, and blocks generation and reloading until the runtime is unloaded.',
    'The hash-bound lifecycle receipt and 42 focused tests prove this software contract only. No new real-device recovery run, production adapter, quality win, held-out result, paid-reference parity, or completed B/A judgment is claimed; public Scion remains base-only.',
  ],
  landingHighlights: [
    'The real browser canary now uses the bounded registry.',
    'Exact valid caches avoid downloading the adapter again.',
    'Active adapters cannot be replaced under the same ID.',
    'Unproven rollback blocks inference until runtime recovery.',
    'This remains lifecycle evidence, not an adapter-quality win.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.24.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'One path from download to activation',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The localhost-only browser canary delegates manifest and adapter delivery to installScionBrowserAdapter instead of maintaining a second fetch implementation.',
        'The canary then delegates cache verification, activation, and deactivation to the same registry lifecycle coordinator used by the adapter architecture.',
        'The integrated canary test supplies a whole-response arrayBuffer trap and proves all manifest, GGUF, and conversion-receipt bytes travel through streams.',
        'The cached record now retains the exact original manifest bytes and verifies their digest and parsed identity before trusting stored file references.',
        'An exact valid cache downloads only the small trusted manifest; a different manifest under an active adapter ID fails before replacement-file transfer.',
      ],
    },
    {
      label: 'Never pretend rollback succeeded',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Registry deactivation accepts base-only state only when the runtime returns restored: true after native clearing and exact output restoration.',
        'A failed activation rollback or explicit deactivation writes recovery-required instead of the previous dishonest base-only default.',
        'The browser runtime marks native state unknown and blocks both completion and another load while quarantined.',
        'Unload clears the quarantined process; a fresh pinned-base load and completion must succeed before the runtime reports ready again.',
        'Forty-two focused tests cover bounded canary delivery, cache reuse and tampering, active-ID replacement, coordinated activation/deactivation, quarantine, and reload recovery.',
        'The v0.16.24 receipt binds the exact implementation and test bytes while retaining the 52,707,007-byte non-promotable smoke budget.',
        'No actual device recovery trial or educational-quality result is created; the fresh reverse-order B/A judgment remains the next training dependency.',
      ],
    },
  ],
};
