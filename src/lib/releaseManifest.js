...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Adapter Promotion Now Verifies the Judgment, Not Just the File',
  highlights: [
    'The single-model-judge promotion attestation is no longer hash-only. It must parse and pass a semantic contract bound to honest-quality-benchmark-v1, the canonical rubric and judge prompt, the exact adapter manifest, and the frozen five-course held-out benchmark.',
    'Each comparison now requires exactly ten trials in each frozen domain, all nine rubric dimensions, byte-verified scoring-first scorecards, balanced candidate placement, and one stable A/B plus B/A decision per trial. Source, course-input, settings, judge, compiler, and artifact identity drift fail closed.',
    'Promotion evidence must include both adapter-versus-base and adapter-versus-pinned-GPT-5.4-mini comparisons. It requires a global and per-domain preference lower bound above chance, positive score-delta intervals, lower Scion-call burden versus base, and exact reuse of candidate artifacts across both controls.',
    "Scorecard verification now rejects absolute, traversing, escaping, or symlinked paths before reading bytes. The adapter promotion report exposes each external evidence item's semantic status and issues, and a published template plus standalone audit command makes the contract reproducible.",
    'This release strengthens what future evidence must prove; it does not perform the fresh reverse-order judgment, train or activate a quality adapter, change Gemma weights, complete the device matrix, or claim a base or paid-reference win. Hosted Scion remains the pinned public base plus compiler.',
  ],
  landingHighlights: [
    'A hash-correct placeholder can no longer certify an adapter.',
    'Five domains × ten trials × two presentation orders are mandatory.',
    'Base and pinned paid-reference comparisons must share exact candidate artifacts.',
    'Scorecard paths and bytes now fail closed together.',
    'No new adapter or model-quality win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.28.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Semantic judgment evidence',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The promotion runner now parses the single-model-judge file and recomputes its evidence instead of trusting status plus SHA-256.',
        'The wrapper binds the exact adapter ID, manifest digest, base revision, scale, canonical quality manifest, rubric, judge prompt, and frozen held-out course manifest.',
        'Adapter-versus-base and adapter-versus-paid-reference are both required; missing, duplicate, renamed, floating, or hash-mismatched comparisons block promotion.',
        'Every comparison must use the same clean compiler and exact judge contract, and the candidate output plus scorecard must be reused across both controls.',
        'The standalone contract audit proves that a hashable pass-shaped dummy is rejected rather than promoted.',
      ],
    },
    {
      label: 'Fifty trials, honest boundary',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The exact five frozen courses contribute ten distinct trials each, with bound source and course-input hashes and balanced A/B side placement.',
        'All nine rubric dimensions and exact rubric digest are mandatory on both outputs. Scorecard files must be byte-verified, content-consistent, regular, non-symlinked, and confined to the comparison directory.',
        'Each trial needs exactly two scoring-first preferences, one A/B and one B/A, with one stable unblinded outcome. Position sensitivity or missing passes block the evidence.',
        'The aggregate and every domain need positive quality-delta intervals and preference evidence above chance; adapter-versus-base also needs a strictly lower Scion-call interval.',
        'Focused adversarial tests reject placeholder evidence, missing reverse order, source substitution, incomplete dimensions, side imbalance, a floating paid alias, and non-reused candidate output.',
        'These are evaluator and promotion-truth improvements only. The actual fresh B/A judgment remains intentionally untouched, so no quality adapter or broader model result changes.',
      ],
    },
  ],
};
