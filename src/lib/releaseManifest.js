...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Prepares a Clean Second Reading',
  highlights: [
    'A new clean-room handoff reconstructs the exact 128 source-backed cases and emits only the B/A review template, blank decisions skeleton, frozen atom judge prompt, instructions, and manifest. The A/B template, completed pass, transcript, event log, sealed envelope, key, plaintext, organizer mapping, model identity, and prior outcome are prohibited and absent.',
    'The handoff is immutable and fail-closed. Every allowed file is byte-counted and SHA-256-bound in a tracked receipt; the verifier requires the exact five-file allowlist, B/A presentation order, neutral source context, blank scorecards and decisions, null outcome state, and no organizer or sealed-envelope fields. Added, removed, modified, nested, or symlinked files fail verification rather than being silently cleared.',
    'Fresh-task decisions now live in a separate working copy. The handoff stays unchanged while Codex scores both sides before recording winner, tie, or insufficient evidence, then supplies its exact revision, runtime, fresh session, completion time, no-prior-outcome statement, context-reset attestation, and judgment attestation.',
    'One atomic command re-verifies the tracked handoff, validates every completed decision in memory, encrypts with AES-256-GCM, creates the sealed envelope and 0600 key exclusively, prints no outcome, and never writes judgment plaintext. Existing outputs cause a hard failure, preventing accidental replacement of retained evidence.',
    'The real v0.16.19 receipt binds all 128 B/A cases and the unchanged prompt hash, but no B/A judgment has occurred. Stable preferences, quality-training rows, learned adapter weights, held-out wins, and paid-reference parity remain zero; public Scion remains the pinned browser-local Gemma base plus the model-neutral compiler.',
  ],
  landingHighlights: [
    'The reverse-order kit contains only blank B/A evidence.',
    'Every allowed byte is bound to one tracked receipt.',
    'The first outcome, mapping, and model identity stay outside the task.',
    'Completion encrypts in memory and writes no judgment plaintext.',
    'The second reading is prepared, not claimed; Scion stays base-only.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.19.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Hand off only the reverse-order evidence',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The kit contains one B/A template, one immutable blank decision skeleton, the exact prompt, instructions, and manifest.',
        'A/B artifacts, organizer mappings, sealed evidence, keys, plaintext, prior outcomes, and unblinded model identity are absent.',
        'Every payload byte is reconstructed from tracked candidates and bound to the frozen packet and prompt hashes.',
        'The exact allowlist and blank-outcome validator reject contamination instead of trying to sanitize it.',
      ],
    },
    {
      label: 'Complete and seal without plaintext',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Codex edits a working decisions copy while the hash-bound handoff remains unchanged.',
        'The atomic path validates completed scorecards and attestations against the immutable B/A template in memory.',
        'AES-256-GCM produces only an outcome-sealed envelope and a 0600 local key; no completed plaintext file exists.',
        'Exclusive creation refuses to overwrite either output and removes a newly created key if envelope creation fails.',
        'The command reports review count, envelope path, sealed disclosure, and plaintextWritten=false—never a winner.',
        'A genuinely fresh task must still perform the 128 judgments before either order can be unsealed or ingested.',
        'Approved preferences, learned quality weights, adapter wins, and model superiority remain unclaimed.',
        'The strict layered release evaluator remains compiler-contract-only; this release changes isolation mechanics, not classroom evidence.',
      ],
    },
  ],
};
