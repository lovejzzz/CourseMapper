...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Judges Without Peeking',
  highlights: [
    'The training-review protocol now includes the exact neutral source context above both anonymous artifacts and uses a hash-bound atom-specific prompt. It scores factual correctness, source fidelity, teachability, coherence, and task quality without pretending atom evidence can prove export, compiler, device, speed, cost, or full-course quality.',
    'Winner-only review is gone. Codex can record a winner, tie, or insufficient evidence without rewriting a legitimate judgment to fit an aggregate score. Scored cards enforce the 1–5 rubric, unscorable cards keep null scores, and below-floor or no-margin relative winners remain explicit non-qualifying evidence instead of being repaired into preferences.',
    'Codex completed the first isolated A/B reading of all 128 source-backed cases. Every source byte, anonymous artifact, scorecard, decision, prompt, judge identity, packet digest, and session attestation passed structural validation.',
    'The complete pass was encrypted with AES-256-GCM, its plaintext was deleted, and only a hash-verifiable outcome-sealed envelope entered the repository. Template regeneration now replaces only its three generated files, while two 0600 local key copies outside that volatile directory passed an exact unseal round trip and remain absent from Git.',
    'One pass proves no stable preference. The reverse order is still missing, approved Codex preferences and quality-training rows remain zero, no quality adapter was trained, and hosted Scion remains the pinned public Gemma base plus the model-neutral compiler. The strict layered evaluator remains compiler-contract-only because independent held-out and instructor evidence are still zero.',
  ],
  landingHighlights: [
    '128 anonymous cases completed their first isolated reading.',
    'The exact source is visible to the judge, not just its hash.',
    'Ties and insufficient evidence never become forced wins.',
    'The first outcome is sealed, recoverable locally, and invisible to Git.',
    'One pass is not a training preference; public Scion stays base-only.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.18.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Judge the evidence that actually exists',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Every review now carries the exact neutral source object as well as its SHA-256 binding.',
        'The atom prompt scores only the five constructs supported by an MC or key-term comparison.',
        'Export, package, compiler, full-course, device, speed, and cost claims remain outside this evidence scope.',
        'Winner, tie, and insufficient-evidence decisions are all first-class, validator-enforced outcomes.',
      ],
    },
    {
      label: 'Seal one order before reading the other',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The A/B pass contains 128 complete score-before-preference reviews and zero structural validation issues.',
        'AES-256-GCM encryption retains the exact pass while hiding all outcomes from the next judge context.',
        'The tracked envelope binds plaintext, ciphertext, key, packet, prompt, and judge identities by SHA-256.',
        'Template regeneration preserves nested evidence instead of clearing its whole output directory.',
        'Two redundant 0600 local key copies passed an exact plaintext-hash round trip and remain outside Git.',
        'The reverse B/A pass must run in a genuinely fresh context before unsealing and ingestion.',
        'Approved preferences, learned quality weights, adapter wins, and model superiority remain unclaimed.',
        'The strict layered release evaluator still fails its independent-evidence tiers; this release does not relabel Codex as an instructor.',
      ],
    },
  ],
};
