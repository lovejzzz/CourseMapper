...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Bounds the Downloaded Delta',
  highlights: [
  ...
  landingHighlights: [
    'Adapter packages must stay below 64 MiB and 2% of the base.',
    'Downloads stream inside exact byte limits.',
    'Dishonest lengths, overruns, and truncation fail closed.',
    'No partial adapter is committed after a failed file.',
    'The retained 52.7 MB smoke remains mechanical evidence only.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.23.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Bound the delta before allocating it',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The absolute browser adapter package ceiling is 67,108,864 bytes; the two-percent pinned-base ceiling of 66,990,282 bytes is stricter today.',
        'The manifest itself is capped at 1 MiB, and a declared response above its applicable ceiling is rejected before the stream reader opens.',
        'Browser installation requires ReadableStream support and never falls back to an unchecked whole-response arrayBuffer allocation.',
        'Headerless streams are counted chunk by chunk; the reader is cancelled on overrun and truncation is rejected at end-of-stream.',
      ],
    },
    {
      label: 'Commit only a complete, verified package',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Every file is downloaded into an exact-size buffer, then checked against its manifest byte count and SHA-256 before it enters staged state.',
        'Only the final all-files-passed path commits the adapter record and files atomically; failed header, stream, size, or digest checks leave the registry unchanged.',
        'Adversarial tests cover early length mismatch, headerless overrun with cancellation, headerless truncation, missing streams, oversized manifests, and digest failure.',
        'The tracked receipt binds the pinned base contract, retained browser smoke evidence, installer, manifest validator, and adversarial test bytes.',
        'The 52,707,007-byte smoke package is 1.573572% of the exact base and has 14,283,275 bytes of effective headroom.',
        'The smoke package is still non-promotable and outside the repository; no quality adapter or production device result exists.',
        'The strict layered evaluator remains compiler-contract-only, and the separate fresh B/A Codex judgment is still the next evidence dependency.',
      ],
    },
  ],
};
