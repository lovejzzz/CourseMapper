...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Turns a Measured Loss Into Safer Course Materials',
  highlights: [
    'The exact 128-case B/A workbook was judged in a fresh task, completed in eight hash-bound chunks, and outcome-sealed without writing a combined plaintext review. Its key remains outside Git and the tracked envelope verifies independently.',
    'Ingestion detected that A/B and B/A used different Codex revision/runtime identities. It now fails closed without discarding the evidence: all 128 cases are quarantined, zero rows are approved for adapter training, and order effects are not claimed because order and judge revision changed together.',
    'The analysis-only result agrees on 113/128 decisions. Among 105 score-qualified stable winners, GPT-5.4-mini wins all 105 and Scion base wins zero across the four research domains represented. This is a measured base-pipeline gap—not an adapter result, not the five-domain promotion benchmark, and not human or independent evidence.',
    'The model-neutral compiler converts the most reliable defect signal into product protection: explicit explanation cues safely realign 16 of 27 judge-identified local answer-key defects, placeholder option sets are rejected, and 11 unresolved cases remain untouched rather than being semantically guessed.',
    'Every future reverse-order workbook can pin the first sealed pass’s public judge identity before scoring starts and refuses a changed revision at completion. No model weights or production adapter changed; hosted Scion remains the pinned public Gemma 4 base plus the improved compiler.',
  ],
  landingHighlights: [
    'A fresh 128-case reverse pass is sealed and reproducible.',
    'Judge revision drift quarantines every training preference.',
    'The paid reference wins all 105 stable qualified comparisons.',
    'Sixteen observed local answer-key defects are safely repaired.',
    'No adapter, weight, human, or production win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.32.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'The loss is evidence, not training data',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The fresh B/A task read the exact frozen 128-case workbook in eight ordered chunks, used the unchanged rubric and prompt, and sealed the completed result directly. The combined review plaintext and encryption key remain outside Git; both tracked envelopes carry self-verifying public metadata and hashes.',
        'The first A/B envelope reports one Codex revision/runtime identity and the fresh B/A envelope reports another. Ingestion therefore reports analysis-only-judge-identity-confounded, approves zero preferences, and names cross-revision winner/tie changes separately from genuine same-identity position sensitivity.',
        'Across the two passes, 113 decisions agree: 105 stable score-qualified winners and eight stable ties. There are twelve winner/tie disagreements, two opposite-winner disagreements, and one below-floor case. Every stable winner favors GPT-5.4-mini across computer science, geology, music theory, and user-experience design. The result compares captured base-plus-compiler atoms, not a trained adapter, and does not cover the frozen five-domain promotion suite.',
      ],
    },
    {
      label: 'Compiler lift without semantic invention',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The defect ledger records 48 answer-key-integrity observations across both judge passes. On the exact 46 local MC artifacts, 27 pairs contain a judge-identified local answer-key defect. Sixteen expose a deterministic affirmative cue—such as “Option B is correct” or an exact option explicitly marked correct—and the compiler now realigns those keys with full provenance.',
        'Explicit cues stop before misconception and contrast prose, conflicting cues refuse repair, and the older lexical-margin fallback remains conservative. Eleven judged key defects do not provide enough deterministic evidence and remain unresolved instead of receiving a guessed answer.',
        'Option sets made only of semantic placeholders such as “index: 0” through “index: 3” now fail admission before they can ship or enter a preference margin. The new rule is model-neutral, so local and user-selected paid models both receive this protection when they use the shared compiler path.',
        'The frozen v0.16.22 replay now evaluates its historical admission and repair counts under the pre-v0.16.32 cue rules while current production admission uses the stronger gate. New protection can improve the product without rewriting an older receipt or making its baseline appear worse retroactively.',
        'A tracked metadata receipt binds both sealed envelopes, public judge identities, aggregate analysis, defect classes, compiler projection, and implementation hashes. Its normal audit needs neither keys nor review plaintext. Future fresh workbooks may read only the first envelope’s public metadata before judgment and must stop if model, revision, runtime, or prompt identity differs.',
        'No new Gemma weights were trained, no adapter was activated, and no model-quality win is claimed. The next valid training campaign must rerun both orders under one honestly available revision-pinned judge identity; only then can stable preferences enter a research adapter dataset.',
      ],
    },
  ],
};
