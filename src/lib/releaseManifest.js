...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.61 Turns More Gaps Into Verified Lessons',
  highlights: [
  ...
  landingHighlights: [
    'Twenty-eight unseen lessons cover every domain and failure family.',
    'Role swaps and partial answer mappings no longer pass as distinct choices.',
    'Source-backed names, quantities, and scope are interpreted more precisely.',
    'The paid reference wins all twenty-eight paired-order comparisons.',
    'Twenty-seven cases earn strict full-lesson training qualification.',
    'One unresolved geology case stays quarantined.',
    'The source-ledger corpus reaches 47 of 100 required lessons.',
    'All seventy-eight frozen semantic losses remain detected.',
    'CI receipts now bind the expanded evidence chain.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.61.json',
    roadmap: 'docs/SCION_ADAPTER_ROADMAP.md',
    browser: 'docs/evidence/SCION_V01661_EXPERIENCE_AUDIT.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Select novelty cumulatively',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'The new selector evaluates novelty against every prior selection rather than only the immediately preceding batch.',
        'The 28-case wave contains four lessons per domain, 17 course groups, 28 source kernels, and all nine diagnosed failure families.',
        'Across the three selected waves, the measured surface now spans 49 cases, 25 course groups, and 47 source kernels.',
      ],
    },
    {
      label: 'Understand claims by their roles',
      icon: 'CHECK',
      color: 'violet',
      items: [
        'Ordered token and clause binding keeps subject/object swaps distinct while still recognizing genuinely reordered duplicate claims.',
        'Paired mappings detect a second source-supported answer only when each subject remains bound to its own predicate; reciprocal proportional statements normalize to the same relationship.',
        'A partial sequence is excluded when the stem explicitly requests the complete sequence and the explanation identifies the missing remainder.',
      ],
    },
    {
      label: 'Keep source admission strict, not noisy',
      icon: 'LIVE',
      color: 'blue',
      items: [
        'Question, option, and explanation fields are evaluated separately so adjacent names cannot combine into a fabricated entity.',
        'Course titles provide naming context without becoming factual support; task-structural counts and source-backed absolute words avoid false warnings.',
        'Predicate-role checks catch unsupported head changes while allowing coordinated subjects and pronoun-led continuations.',
      ],
    },
    {
      label: 'Make every row earn qualification',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'The reference wins all 28 direct A/B plus B/A comparisons without one order reversal; nineteen winners qualify immediately.',
        'Source-only repair contributes seven first-pass rows and one final row. Every accepted artifact passes compiler admission and score floors with zero winner critical defects.',
        'One geology artifact remains out after repeated repair and one score failure, leaving an honest 27/28 expansion result.',
      ],
    },
    {
      label: 'Reach 47 without moving the gate',
      icon: 'CHECK',
      color: 'amber',
      items: [
        'The cumulative corpus contains 47 unique full lessons with exact source ledgers across all seven domains—47% of the minimum training gate.',
        'The frozen semantic ruler remains 78/78 losing atoms detected, 78/78 preferred artifacts eligible, and zero regressions.',
        'The compiler changes are model-neutral and benefit compatible paid routes; local WebGPU runtime, caching, and future adapter activation remain Scion-specific.',
      ],
    },
    {
      label: 'Bind the proof and keep the claim honest',
      icon: 'PROOF',
      color: 'emerald',
      items: [
        'The V0.16.61 receipt binds cumulative selection, immutable captures, current-compiler replay, every repair, sixteen isolated judge sessions, implementation hashes, and all 47 training rows.',
        'The stale source-compiler receipt behind the failed screenshot commit is rebuilt from retained bytes and rechecked with every downstream semantic receipt.',
        'No adapter is trained or active, no Gemma weight changed, and same-identity model-teacher/model-judge evidence is not presented as human, independent, held-out, classroom, or production proof.',
      ],
    },
  ],
};
