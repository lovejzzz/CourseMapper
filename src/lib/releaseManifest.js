...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion V0.16.53 Makes Adapter Evidence Honest',
  highlights: [
    'Every admitted adapter-training row now carries an explicit task family, and the sorted row-complete allowlist is hash-bound through dataset, training plan, package, conversion, and runtime lineage.',
    'The browser loads a proven adapter only for an exact trained family. Out-of-scope work restores and verifies base-only inference; any native-state or canary drift blocks inference until reload.',
    ...
    'The next quality milestone is now precise: collect stable source-grounded lesson-kernel preferences, train a task-matched adapter, and beat exact base-only Scion with route evidence and no burden regression.',
    'No adapter is active and no Gemma weight changed. V0.16.53 strengthens architecture and evaluation validity; it does not claim a new model-quality score or paid-reference parity.',
  ],
  landingHighlights: [
    'Adapter weights can run only on task families they were trained to serve.',
    'Out-of-scope requests prove restoration of the exact public Gemma base.',
    'Request traces show when the adapter was used—and when it was avoided.',
    'Benchmark V5 rejects globally loaded but task-irrelevant adapters.',
    'The current atom corpus is honestly blocked from a whole-course claim.',
    'Lesson-kernel preference evidence is the next training milestone.',
    'Gemma weights remain unchanged and no quality adapter is active.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.53.json',
  ...
  sections: [
    {
      ...
      items: [
        ...
        'The dataset writes the exact family into row provenance and seals a sorted allowlist plus row counts. The training verifier recomputes those counts from every split and rejects a missing, duplicate, unknown, unsorted, or count-incomplete scope.',
        'The training plan copies the exact scope, the adapter manifest requires it for every learned status, and converted GGUF packages must inherit it byte-for-byte from their MLX source lineage.',
        'Manifest schema V4 keeps schema V2 and V3 mechanical history readable but refuses to relabel legacy research, candidate, or promoted artifacts as current learned evidence.',
      ],
    },
    {
      ...
      items: [
        ...
        'An out-of-scope or unclassified task clears LoRA and replays the deterministic activation canary. Exact base output and native inactive state are both required before the user request runs.',
        'If adapter restoration, base restoration, metadata, or canary output drifts, Scion enters recovery-required state and blocks inference until the runtime is unloaded and reloaded.',
        'The runtime emits a route receipt for every request; the Living Compiler trace can retain task family, adapter/base route, reason, adapter identity, manifest identity, scope identity, and native state.',
      ],
    },
    {
      ...
      items: [
        ...
        'The adapter arm must show at least one exact adapter route for lesson-kernel work and an exact base-only route for course-map work. The base arm must keep both families base-only.',
        'An adapter route on a forbidden family, a missing required family, a mismatched adapter or scope digest, or any unclassified call invalidates the course evidence.',
        'A package that merely reports one adapter globally active can no longer pass the paired evidence producer.',
      ],
    },
    {
      ...
      items: [
        ...
        'That corpus has no lesson-kernel family. The task-scope audit therefore passes its integrity contract but marks it ineligible for the whole-course V5 adapter arm.',
