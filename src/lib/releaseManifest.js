...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Makes the Second Reading Resumable',
  highlights: [
    'The 128-case B/A task is now eight immutable 16-case chunks instead of one 543,277-byte review file and one 123,877-byte decisions file. Each working unit is 66,742–70,779 bytes of anonymous evidence plus a 16,021-byte decisions skeleton, so the fresh task can progress and recover one bounded section at a time.',
    'Chunk assignment is deterministic and interleaved by original case index, distributing computer science, geology, music theory, and user-experience design across the schedule. A tracked receipt binds every file, chunk index, pair set, canonical template hash, and exact reconstruction order.',
    'All chunks still constitute one B/A reading, not eight votes. Finalization requires the same Codex revision, runtime, fresh session ID, completion time, no-prior-outcome statement, context-reset attestation, and judgment attestation in every completed decisions file.',
    'The assembler verifies every immutable chunk and completed decision set, reconstructs the original 128-case order in memory, and creates only one AES-256-GCM envelope plus one 0600 key. Working decision chunks contain judgments by design, but no combined completed review pass is written.',
    'The real fresh B/A judgment is still missing. v0.16.21 improves campaign feasibility and failure recovery but creates no outcome, stable preference, training row, adapter weight, held-out win, device result, or paid-reference parity claim; public Scion remains base-only.',
  ],
  landingHighlights: [
    '128 cases become eight bounded 16-case chunks.',
    'Every chunk is hash-bound to one canonical pass.',
    'One fresh judge session must complete the whole workbook.',
    'Finalization seals one pass without a combined plaintext file.',
    'The workflow is ready; the second judgment is still missing.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.21.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Judge in bounded, balanced chunks',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Eight 16-case review templates replace the fragile 128-case monolith while preserving the exact anonymous B/A evidence.',
        'Modulo assignment interleaves the original packet so each chunk covers a mix of the four training domains.',
        'Each template has its own hash-bound blank decisions skeleton, pair-set digest, and original review-index mapping.',
        'The exact allowlist rejects missing, added, changed, nested, or linked files without deleting unknown evidence.',
      ],
    },
    {
      label: 'Reassemble one clean reading',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Every completed chunk must use one identical fresh Codex session and the same judge identity, timestamps, and attestations.',
        'Chunk-local validation identifies the exact failing unit before any envelope or key is created.',
        'The assembler restores canonical packet order in memory and re-runs the full 128-case structural validator.',
        'Only one outcome-sealed envelope and one 0600 key are emitted; no combined completed pass is written to disk.',
        'Working decisions contain judgments and remain sensitive local files; the release does not pretend otherwise.',
        'No real B/A judgment exists yet, so stable preferences, quality rows, adapters, wins, and paid-reference parity remain zero.',
        'The strict layered evaluator remains compiler-contract-only; operational readiness is not model-quality evidence.',
      ],
    },
  ],
};
