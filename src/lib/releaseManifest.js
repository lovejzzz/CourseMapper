...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion’s 128-Case Reverse-Order Judge Kit Is Now Reproducible',
  highlights: [
    ...
    'The complete blank B/A-only evidence kit is now a tracked release artifact: one verified canonical handoff, eight 16-case workbook chunks, blank decision skeletons, the exact judge prompt, instructions, manifests, and receipts. It contains no outcome, organizer mapping, unblinded identity, key, or completed judgment plaintext.',
    'Workbook reconstruction now starts only from the hash-verified frozen v0.16.19 canonical handoff. It refuses a changed canonical receipt instead of silently following a newer review packet, preserving all 128 pair IDs and the original canonical B/A template hash.',
    'The canonical audit verifies both the tracked workbook on disk and a byte-for-byte reconstruction. Receipt errors retain the fail-closed summary and now identify exact drifting JSON paths; focused fixtures prove frozen-source rebuilds and reject field-level substitution.',
    'This release makes the missing fresh B/A judgment executable and reproducible; it does not perform that judgment, disclose the earlier outcome, create stable preferences, train a quality adapter, or claim an adapter win. Hosted Scion remains the pinned public base plus compiler.',
  ],
  landingHighlights: [
    'The exact 128-case B/A packet now survives a clean checkout.',
    'Eight small chunks remain one isolated reverse-order reading.',
    'Rebuilds start from frozen evidence, never the mutable candidate pool.',
    'Receipt drift now names the exact mismatched field.',
    'No judgment, adapter, or model-quality win is claimed.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.30.json',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'The previous audit command regenerated a packet from current source-capture candidates, even though the first A/B pass was sealed against an earlier frozen selection. Later compiler and candidate changes made that regeneration produce 123 cases instead of 128 and correctly triggered a receipt mismatch.',
        'The frozen v0.16.19 B/A canonical handoff and the v0.16.30 chunked workbook now live under evaluation/scion-adapters/handoffs, where Git and clean-checkout CI can carry them. The release no longer depends on ignored local verification-output state.',
        'The active workbook contains 128 exact anonymous, source-bound cases in eight deterministic 16-case chunks. Its manifest binds every file byte, pair-set digest, domain distribution, canonical original order, prompt, and plaintext-free completion contract.',
        'The builder verifies the frozen canonical handoff against its historical receipt before reading the B/A template. A changed source packet, missing file, added file, symlink, nonblank decision, or receipt substitution fails before a workbook is trusted.',
      ],
    },
    {
      label: 'Clean-room execution boundary',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The fresh judge task is allowed to read only the tracked B/A workbook, its prompt, blank decisions, instructions, and manifest. The prior A/B template, transcript, envelope, key, plaintext, organizer mapping, identities, and outcomes remain prohibited.',
        'Each chunk keeps the same exact judge revision, runtime, fresh session ID, completion time, no-prior-outcome statement, context-reset attestation, and judgment attestation. Eight resumable chunks still become one reading, not eight independent votes.',
        'Finalization reconstructs the canonical 128-case pass in memory, validates scoring-before-preference and every scorecard, and writes only one AES-256-GCM envelope plus one 0600 key. It never writes a combined completed plaintext pass.',
        'The audit now checks the committed workbook itself and independently rebuilds it from the frozen canonical handoff. When a receipt differs, diagnostics name bounded exact paths such as $.selectedCases instead of returning only an opaque mismatch.',
        'No completed B/A decision exists in this release. Stable preferences, approved research rows, quality adapter weights, held-out adapter wins, paid-reference parity, and the remaining device profiles all remain unproven.',
      ],
    },
  ],
};
