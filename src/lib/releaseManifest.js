...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Source Orchard',
  highlights: [
  ...
  landingHighlights: [
    'Eight real source-bound courses deepen the research campaign.',
    'Raw Gemma and GPT gaps remain visible after compilation.',
    'Good atoms survive without lowering the quality gate.',
    'Blind cases carry neutral source context, never model labels.',
    'Hosted Scion remains base-only while human evidence grows.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.11.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Source-bound model evidence',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Course depth: two new source-bound groups in Computer Science, Geology, Music Theory, and UX bring the research packet to three exact course inputs per domain without touching the frozen held-out ruler.',
        'Compact protocol: each arm receives 24 source-grounded prompts—three Curriculum Genome kernels for each of eight courses—and is asked for two multiple-choice items plus two key terms per prompt.',
        'Source integrity: exact selected facts, attribution, license, source-packet digest, canonical course-input digest, prompt digest, raw response, admitted response, model identity, and compile graph are retained in every project.',
        'Model integrity: the local arm is the pinned base-only `google/gemma-4-E2B-it-qat-q4_0-unquantized` revision with greedy schema decoding; the reference arm is GPT-5.4-mini with low reasoning and the same strict response contract.',
        'Fail-closed verification: all 16 projects reconstruct their source packet, prompts, response hashes, model hash, compile graph, admitted atoms, burden report, and recovery provenance before they enter review matching.',
        'Raw result: local Gemma generated 92 and admitted 62 of 96 expected atoms; GPT generated 96 and admitted 91. Local raw burden is 34 atoms versus 5, so this campaign records a large base-model deficit rather than a Scion win.',
      ],
    },
    {
      label: 'Compiler harvest and blind review',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'Per-atom admission: a prompt no longer loses its valid multiple-choice item merely because both key terms fail, or vice versa. Each valid sibling is retained under the same source and contract checks.',
        'Bounded recovery: only the single raw call with zero admissible atoms received one decomposed one-item-plus-one-term retry. Compiled local admission reached 63 of 96; the remaining 33-atom burden stays visible.',
        'No denominator laundering: source-capture projects are marked `blind-review-only` in the contrast matrix because they do not contain the full authored lesson and short-answer denominators required by full-course evaluation.',
        'Neutral context: reviewer pages display the exact source claims, attribution, and license above A/B candidates without revealing provider or model identity. Source context also participates in the public case digest.',
        'Campaign depth: 372 available candidates yield a 160-case packet balanced at 40 per domain and three course groups per domain. Sixty-three selected cases come from the new exact source captures.',
        'Frozen boundary: the five held-out domains remain Astronomy, Nutrition, Psychology, World Languages, and World Literature. None appears in the research packet.',
        'Human boundary: zero independent reviews and zero approved preference rows exist. Research training still requires 100 approved pairs, at least 20 per domain, and the completed review threshold; production remains much stricter.',
        'Product boundary: no quality adapter is shipped or implied. Scion V0.16.11 remains the pinned public browser-local Gemma base plus the versioned, model-neutral compiler.',
      ],
    },
  ],
};
