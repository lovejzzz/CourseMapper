...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Now Intercepts 18 of 46 Measured Losses—Source-Grounded and Text-Preserving',
  highlights: [
    'The exact 46-loss replay now intercepts eighteen failures instead of twelve: six contradicted keys are repaired and twelve unusable artifacts are rejected, with zero model-authored text mutation.',
    'Three source facts mislabeled as misconceptions now enter retry only after exact source-context hash binding, three-token support, 75% containment, and 35% whole-sentence overlap; explicit contrast language refuses the rule.',
    'Parenthesized internal markers such as `(Claim 0).` no longer leak into learner-facing fields; four measured losses contain this visible process residue, and one overlaps the misconception set.',
    'Across 192 retained source atoms, all 77 conservative repairs remain while admission moves from 141 to 131 and retry burden moves from 51 to 61; corpus readiness remains 118/464.',
    'Twenty-eight measured losses remain unresolved, only 46/100 judge preferences exist, and no Gemma weight, quality adapter, or paid-reference claim changed.',
  ],
  landingHighlights: [
    'All 46 stable losses now run as compiler regressions.',
    'Six contradicted answer keys are repaired without rewriting text.',
    'Twelve unusable artifacts now retry instead of shipping.',
    'Source facts mislabeled as misconceptions and internal claim markers are rejected.',
    'Twenty-eight measured losses and the 54-row judged-evidence shortfall remain.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.45.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Use the supplied source as the semantic boundary',
      icon: 'AI',
      color: 'blue',
      items: [
        'Every one of the 46 stable paired-order losses now retrieves its exact neutral source context from the retained A/B workbook. Workbook bytes, each source-context hash, and both chosen and rejected artifact hashes must match before the replay can score an interception.',
        'A purported misconception is rejected as a true source restatement only with at least three shared content tokens, 75% shorter-side containment, and 35% whole-sentence overlap. Explicit contrast words—such as “must,” “only,” “not,” and “without”—refuse the rule so legitimate false beliefs can share disciplinary vocabulary safely.',
        'The new gate catches three measured semantic failures: musical form defined as structure, interval described by a physical frequency ratio, and a specific scale described by its characteristic interval pattern and first degree. The retained judge defects independently identify each as incoherent or source-supported rather than a valid misconception.',
        'The exact replay now intercepts 18/46 losses: six answer-index repairs plus twelve bounded-regeneration rejects. Only answer-index fields change during repairs; model-authored text mutations remain zero and twenty-eight measured losses remain unresolved.',
      ],
    },
    {
      label: 'Reject process residue and publish the cost',
      icon: 'CHECK',
      color: 'emerald',
      items: [
        'The claim-marker parser now rejects both bracketed references and learner-facing tails such as `(Claim 0).` or `(Claim 4).` Four stable losses contain this directly visible internal residue; one was explicitly named by the judge and one overlaps the source-restatement set.',
        'Across the same twelve retained-source projects, all 77 conservative repair receipts remain. The source-aware gate admits 131/192 atoms instead of 141/192 and increases retry burden from 51 to 61; the ten additional refusals are regenerated rather than deleted or silently rewritten.',
        'The adapter corpus remains 118/464: 72 deterministic contract margins plus the same 46 paired-order single-model judge preferences. The v0.16.44, v0.16.43, v0.16.42, and v0.16.40 profiles remain independently reproducible instead of being silently rescored.',
        'The source-aware admission is model-neutral and benefits source-grounded paid-model output too; browser-local Scion additionally uses bounded local retry and provenance-preserving key repair. Gemma weights remain unchanged, the adapter remains inactive, and the research gate still requires 54 more stable preferences.',
      ],
    },
  ],
};
