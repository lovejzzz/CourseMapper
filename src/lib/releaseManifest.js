...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Keeps the Complete Thought',
  highlights: [
    'Scion now recovers an explanation only when the model already produced at least one complete sentence before ending mid-thought. The compiler keeps that complete prefix, records the discarded tail, and never invents punctuation, facts, or a replacement rationale.',
    'Browser JSON preprocessing, canonical kernel admission, cached graph attachment, and graph reopen now use the same ordered repair: sentence-boundary recovery first, then only a decisive explanation/key realignment. Every accepted MC repair carries explicit provenance.',
    'A hash-bound replay of 48 real base-Gemma MC responses across computer science, geology, music theory, and UX moves admission from 25/48 historically to 33/48 with conservative key alignment and 45/48 after incomplete-tail recovery.',
    'The replay recovers 20 of 23 historical burden items, an 86.9565% reduction, while all three remaining longest-option cues stay rejected. The four historical capture files remain byte-unchanged and every evidence and implementation file is SHA-256 bound.',
    'This is compiler-contract recovery on retained responses, not a new model run, factual certificate, adapter win, held-out result, or paid-reference comparison. The fresh B/A judgment is still missing and public Scion remains base-only.',
  ],
  landingHighlights: [
    'Incomplete tails are removed only after a real sentence boundary.',
    'No punctuation, fact, or rationale is invented.',
    'One repair order covers browser, compiler, cache, and reopen.',
    'Immutable replay improves admission from 25/48 to 45/48.',
    'The remaining cue failures stay rejected.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.22.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Keep only the complete thought',
      icon: 'AI',
      color: 'emerald',
      items: [
        'A missing final punctuation mark is not repaired by fiat; Scion requires a complete sentence already present in the model output.',
        'The unfinished suffix is removed from the learner-facing explanation but retained verbatim inside semantic repair provenance.',
        'Canonical admission performs tail recovery before explanation/key alignment, so a truncated distractor clause cannot distort the decision.',
        'Browser preprocessing, compiled kernels, cached graph attachment, and graph reopen share the same lightweight repair module.',
      ],
    },
    {
      label: 'Measure the recovery without enlarging the claim',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The replay verifies 24 calls and 48 MC items from four immutable local-model capture files.',
        'Historical admission is 25/48; existing conservative key alignment reaches 33/48; the new incomplete-tail recovery reaches 45/48.',
        'Computer science reaches 12/12, while geology, music theory, and UX each retain one rejected longest-option cue.',
        'The receipt binds every evidence and implementation byte and refuses a changed metric or issue histogram.',
        'No historical project or raw response is rewritten, and incomplete-tail repairs are explicitly ineligible as training preferences.',
        'No real B/A judgment exists yet, so stable preferences, quality rows, adapter wins, held-out wins, device results, and paid-reference parity remain zero.',
        'The strict layered evaluator remains compiler-contract-only; this measured compiler gain is not model-quality proof.',
      ],
    },
  ],
};
