...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Recovers More Source-Bound Work and Freezes a Clean 100-Case Quality Campaign',
  highlights: [
    'A deterministic replay compiles the exact bytes from twelve immutable local Gemma source-capture projects, records every repair, and proves that no model response was mutated.',
    'Conservative incomplete-explanation and answer-key-alignment repairs raise admitted source atoms from 133/192 to 149/192 while reducing compiler burden from 59 atoms to 43.',
    'The neutral comparison ledger now rebuilds exactly to 446 cases, including 138 source-grounded cases across sixteen course groups; the sealed historical campaigns remain unchanged.',
    'A fail-closed source-only builder freezes 100 anonymous, hash-bound cases—25 in each of four training domains, spanning twelve course groups—without filling shortages with ungrounded material.',
    'The packet is ready for two fresh Codex presentation orders, but it contains zero judgments, approves zero preferences, trains zero weights, and establishes no adapter or paid-reference quality win.',
  ],
  landingHighlights: [
    "Exact historical model bytes are replayed through today's compiler.",
    'Admission rises from 69.3% to 77.6% with zero response mutation.',
    'The source-grounded comparison pool grows from 92 to 138 cases.',
    'A strict 100-case, four-domain blind campaign is frozen and reproducible.',
    'No judgment or adapter win is claimed before both fresh orders are complete.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.40.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Compile immutable evidence again',
      icon: 'AI',
      color: 'emerald',
      items: [
        'v0.16.40 reopens twelve retained local Gemma projects from the two immutable source-capture campaigns, verifies their captured provenance, and feeds the exact historical response bytes through the current compiler.',
        'The replay records 67 deterministic repair receipts: twenty incomplete explanation tails and forty-seven answer-key alignments. The original model responses remain byte-identical and the derived projects carry both source and compiler hashes.',
        'Those conservative repairs recover sixteen atoms: admission rises from 133/192 (69.3%) to 149/192 (77.6%), while burden falls from 59/192 (30.7%) to 43/192 (22.4%). These are contract-admission results, not factual or educational-quality scores.',
      ],
    },
    {
      label: 'Freeze the next clean judgment',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The comparison ledger now contains 446 neutral cases, 138 with complete source context. The 46-case source-grounded increase comes from derived compiler outputs coexisting with the immutable historical candidates, not from rewriting old evidence.',
        'The source-only packet builder rejects a short campaign instead of silently adding ungrounded cases. Its frozen packet selects exactly 100 source-grounded comparisons: 25 per domain, three course groups per domain, 52 multiple-choice items, and 48 key terms.',
        'The five held-out benchmark domains remain excluded and disjoint. The current corpus is still smoke-only at 76/418 usable rows with zero same-identity Codex preferences. This task saw organizer metadata while building the packet and therefore must not judge it; a fresh A/B task and a distinct fresh B/A task come next.',
        'No model call, quality judgment, training run, adapter activation, held-out comparison, device result, or paid-reference win is claimed. Hosted Scion remains the pinned public Gemma base plus the model-neutral compiler.',
      ],
    },
  ],
};
