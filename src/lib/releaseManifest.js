...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion Recovers Every Frozen Key-Term Contract Deficit',
  highlights: [
  ...
  landingHighlights: [
    'One key-term contract now governs every compiler path.',
    'Local responses retry semantic defects—not just missing IDs.',
    'Native recovery now revisits incomplete lesson kernels.',
    'The exact frozen deficit set recovered 14/14 in real Chrome.',
    'Public Scion remains the pinned base model plus compiler.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.27.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'One semantic admission boundary',
      icon: 'AI',
      color: 'emerald',
      items: [
        'Compact local JSON, full provider JSON, and the legacy line protocol now normalize through one script-aware key-term contract.',
        'The contract requires lesson-specific term, definition, example, misconception, correction, and valid source indexes. It rejects lesson-title terms, circular or meta definitions, short fields, and a correction that repeats the definition.',
        'The public browser route reads semantic defects before admission, supplies focused feedback, accumulates prior defects so a later attempt cannot silently regress, and stays bounded to two extra calls.',
        'Native Pass B now retries a contract-incomplete lesson kernel as well as a missing one. This generic recovery boundary also applies when the user selects a paid provider.',
        'Cross-attempt field retention is conservative: the compiler can keep only model-authored material that strictly lowers the deterministic issue count, with exact before/after provenance and training eligibility set to false.',
      ],
    },
    {
      label: 'Real browser recovery, narrow claim',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'The receipt reconstructs the exact fourteen v0.16.26 deficits from their immutable manifests, projects, prompts, source packets, and original defect accounting before any new completion is accepted.',
        'Installed Chrome 150 ran the exact revision-pinned 3.35 GB Gemma 4 QAT GGUF with WebGPU and JSPI. The independently recorded cache digest matches the base contract.',
        'All fourteen cases cleared the stronger shared contract: nine on the first attempt and five after one bounded retry. Three accepted responses repaired a malformed replacement field by retaining earlier model-authored material that strictly reduced issue count.',
        'The audit binds every input, message, output, admission decision, source project, baseline receipt, and relevant implementation byte. Rerunning the verifier makes no model call.',
        'This proves targeted browser-local contract recovery only. It does not establish factual correctness, educational superiority, full-course parity, an adapter win, paid-reference quality parity, or human or independent review.',
        'Public Scion remains base-only. The fresh reverse-order B/A judgment, stable training preferences, a quality adapter, three device profiles, and five held-out domain wins remain unfinished.',
      ],
    },
  ],
};
