...
export const CURRENT_RELEASE = {
  ...
  landingTitle: 'Scion’s Next Learning Campaign Starts Blind and Identity-Bound',
  highlights: [
    'The fresh Codex workbook can now produce either judgment order from one frozen anonymous source while retaining the historical B/A reconstruction byte-for-byte.',
    ...
    'The first-order workbook must declare the exact Codex model, revision, runtime, prompt path, and prompt SHA-256 before any scoring can begin.',
    ...
    'This release creates zero judgments, zero preferences, zero training rows, and zero adapter weights; it makes the next same-identity campaign executable without pretending that campaign has happened.',
  ],
  landingHighlights: [
    'The next Scion learning campaign begins with one isolated A/B order.',
    'All 128 anonymous cases are frozen into eight resumable chunks.',
    'Codex identity is bound before scoring, not after seeing results.',
    'Organizer mappings and reverse-order inputs stay outside the judge task.',
    'No judgment or adapter win is claimed before evidence exists.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.35.json',
...
export const CURRENT_RELEASE_CHANGELOG = {
  ...
  sections: [
    {
      label: 'Give the first order its own clean room',
      icon: 'AI',
      color: 'emerald',
      items: [
        'The chunked workbook is now order-aware. It reconstructs the frozen blank canonical payload, reverses presentation and scorecard positions without reading the organizer mapping, and validates either A/B or B/A while preserving the historical v0.16.30 reverse-order receipt.',
        'The new tracked A/B workbook contains 128 source-bound anonymous cases across the four research domains, interleaved into eight immutable chunks. Its canonical template SHA-256 is 5eefcf3a67ffd77387609f8c29223b187c6bafbeb77fe48ac6b360b00f0b28cb and its pair-set SHA-256 is 15498b250985d94e731ffb6f06de30e8f006bcf048334c65f5b8dfdc90240b86.',
        'Only the judge prompt, instructions, A/B review chunks, blank A/B decision skeletons, and manifest are allowed. Added files, links, changed hashes, B/A payloads, organizer data, outcomes, or aggregates fail closed.',
      ],
    },
    {
      label: 'Bind identity before evidence exists',
      icon: 'CHECK',
      color: 'blue',
      items: [
        'A first-order build is refused unless the Codex model, revision, runtime, prompt path, and prompt digest are declared up front. The tracked campaign pins openai/codex, codex-gpt-5-2026-07-15, codex-desktop, and the honest-quality-benchmark-v1 prompt digest.',
        'Every completed chunk must use the same declared identity, fresh session ID, completion time, context-reset statement, and no-prior-outcome attestation. Identity drift stops sealing before any envelope or key is created.',
        'The canonical pass is assembled only in memory and written only as an AES-256-GCM envelope plus a separately held 0600 key. A combined completed plaintext pass is never written by the workflow.',
        'The workbook is campaign readiness, not campaign evidence. No case has been scored in this release, no stable A/B plus B/A preference exists, and the adapter dataset remains below research readiness. The next valid step is one fresh A/B Codex task under the declared identity, followed later by an isolated B/A task pinned from that sealed envelope.',
      ],
    },
  ],
};
