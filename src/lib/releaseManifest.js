...
export const CURRENT_RELEASE = {
  ...
  landingHighlights: [
    'One clear Scion identity: no API key, one versioned model.',
    'Brief-first setup with a guided three-step handoff.',
    'Mobile actions stay reachable and touch-safe.',
    'Source-backed courses reach 99/A with zero P0/P1.',
    'Gemma and Qwen tested under the same fail-closed protocol.',
    'Compiler burden is measured, attributed, and promotion-gated.',
    'All 411 unverified learning rows remain quarantined.',
  ],
  proof: {
    contract: 'release-contracts/v0.16.3.json',
    roadmap: 'docs/V0.16.3_FIRST_FIVE_MINUTES_ROADMAP.md',
  ...
  sections: [
    {
      ...
      color: 'emerald',
      items: [
        'Scion configuration browser proof: six focused Chromium cases pass. The expanded card renders Provider “Scion,” a disabled API field with “No API key required,” Model “Scion V0.16.3,” and the concise customized-course-AI description; the same hierarchy passed direct light- and dark-mode visual inspection.',
        'First-five-minutes proof: the earlier complete browser pass covered 79 desktop, responsive, accessibility, and generated-workspace cases. Desktop and 390×844 flows preserve the Brief/Materials/Generate path, sticky mobile actions, no horizontal page overflow, and 44px workspace controls.',
        'Compiler texture proof on the retained course: FAQ overlap moved 7.7%→4.9% and shared tails 12→1; lesson-plan overlap 6.9%→3.4% and tails 55→14; slide overlap 3.9%→2.0% and tails 42→8; reprojected quiz-bank short-answer tails fell 74→29.',
        'Production-canary proof passes 3/3 retained runs across UX and music theory, including the required public-Scion family. The compiler-hardened 12-lesson UX run (`round-2026-07-12T04-33-57-020Z`) reached 99/A, zero P0/P1, 101 extracted files, 63/63 provider completions, and rendered QA of 39 slides plus 12 quiz pages.',
        'Source-backed local proof: the seven-lesson music route shipped 28/28 source-matched, applied, supported, contrastively explained, answer-aligned MC items; the 12-lesson UX route reached 99/A, zero P0/P1, 38/38 clean export checks, six genome links, and every strict paired-matrix bar.',
        "Matched Luna diagnostic: Scion has 27/48 applied MC items versus Luna's 27/44, so the applied rate still trails. Both reach 100% supported inference, answer alignment, scenario coverage, and concrete materials; Scion reaches 100% contrastive rationales, decision-ready scenarios, cue-free answers, and claim-evidence-boundary answers. This is one course, not a general superiority claim.",
        'Frozen factual canaries: GPT-5.4-mini cold 25/25; Scion-1 cold 23/25; Scion-1 with the verified Curriculum Genome bundle 25/25. The expanded corpus audit now quarantines 411/411 rows, including unknown evidence kinds and post-hoc key realignment.',
        'Blind-review proof: five domain folders contain 50 balanced cases (25 MC, 25 key-term). A real Chromium test completed the offline form, downloaded packet-bound JSON, passed fail-closed ingestion, and observed zero network requests; the packet leaks no organizer mapping or model/source identity. Approved instructor decisions remain 0.',
        'Model bakeoff proof: Gemma 4 E2B and Qwen3.5 4B each scored 23/25 cold twice and 25/25 source-grounded twice. The exact `mlx-community/Qwen3.5-4B-4bit` UX run (`round-2026-07-12T15-35-35-677Z`) reached 99/A, zero P0/P1, 101 extracted files, $0, and 382 seconds. It remains unpromoted at one of five required full-course domains.',
        'Compiler-burden proof: the Qwen package required 85 Scion pass calls (7.08/lesson), 35 rejected quality actions, and 12 accepted regenerations versus 50 calls (4.17/lesson), 21 rejections, and six regenerations in the retained legacy control. The audit marks 1.70× call amplification and shared applied-depth waste as P1 compiler debt.',
        'Compiler repair: every future Scion subcall emits its schema purpose; model promotion requires five domain-matched controls and no more than 1.25× call amplification. On the retained Qwen Pass-B inputs, the corrected depth detector avoids 17/33 unnecessary rewrite targets, while the revised repair prompt requires an open evidence question and forbids option, label, and answer leakage.',
        'Full contract proof: all 40 deterministic course fixtures pass. The release sweep caught repeated evidence cues in all eight data-science decks and repeated decision cues in seven clinical decks; the compiler now anchors both cues to the lesson concept, and both formerly blocked fixtures pass. The layered report remains honestly labeled “compiler-contract-only” until independent instructor reviews exist.',
        'Runtime repair: native SSE now passes through without JSON double-parsing, local browser traffic bypasses the legacy buffer, heartbeats remain visible, loader stderr is always drained, timed-out model starts are killed instead of poisoning the route, and Crucible records the exact source-weight ID rather than trusting a friendly alias.',
        'Audit-runtime repair: Vite SSR modules now load in a deterministic sequence so Firebase/gRPC cannot expose a half-initialized CommonJS export, retained ZIP/model evidence is excluded from file watching, and independent-review templates are idempotent instead of being rewritten on every audit. The compact model-screen and six-pair contrast ledgers are committed under `evaluation/`, so the default audits survive a fresh clone instead of relying on ignored run folders. This keeps the release gate stable as the evidence library grows.',
        'Claim boundary: v0.16.3 improves the branded product, compiler, evaluation, and local research route. It does not claim new foundation-model weights, independent instructor readiness, or a general win over paid models.',
      ],
    },
  ],
};
