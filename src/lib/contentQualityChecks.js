...
function checkSentenceIntegrity(findings, featureId, data) {
  for (const [path, value] of walkStrings(data)) {
    ...
    if (hasAdjacentArticleCollision(trimmed) || FRAMING_ADJECTIVE_DETERMINER_RE.test(trimmed)) {
      pushFinding(findings, 'article-collision', path, trimmed);
    }
