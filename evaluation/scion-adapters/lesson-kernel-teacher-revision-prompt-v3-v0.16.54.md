# Scion source-bound teacher revision v3
...
- Every `cx` must repeat its exact `tr` phrase verbatim as the subject; do not use a pronoun, an example pair, or a generic phrase as the sole subject of the correction. Preserve the term identity while correcting only the false predicate.
- Write `mi` and `cx` with a different sentence structure and substantially different content words from `df` and from each other. A misconception must be a plausible false claim, not the definition with one negation or reversed adjective. A correction must state the source-backed replacement directly; do not repeat the misconception and then negate it, and do not restate the full definition.
