...
- Delete any named phrase, person, place, organization, study, product, mechanism, statistic, or example that does not appear verbatim in a supplied claim.
- `su` has exactly two sentences with two inspectable details, a learner decision, and a real constraint or tension. Begin `ma` with `Evidence packet:` and name at least two concrete evidence types and the details to inspect.
