# Scion source-bound teacher revision v1
...
- Key terms must make five different instructional moves across `tr`, `df`, `eg`, `mi`, and `cx` without repeating fields. `df` must state a real defining distinction. `eg` must apply the term to a small concrete case or comparison using substantially different wording from `df`; do not restate the definition as the example. `cx` must directly correct the false predicate in `mi` using substantially different wording from `df`; do not repeat the definition as the correction.
- Do not copy the full lesson title as a key term. Do not invent named people, places, organizations, studies, products, statistics, causes, mechanisms, or consequences.
