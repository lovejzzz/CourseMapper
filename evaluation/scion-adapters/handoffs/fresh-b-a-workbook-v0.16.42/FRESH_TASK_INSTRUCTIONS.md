...
Read and follow `single-model-training-atom-judge-prompt-v2.md`. The 100-case B/A pass is divided into immutable chunks. Cases are deterministically interleaved from the canonical packet to distribute domains across the schedule.
...
```bash
mkdir -p verification-output/scion-codex-fresh-b-a-working-v0.16.42
cp evaluation/scion-adapters/handoffs/fresh-b-a-workbook-v0.16.42/chunk-*-decisions-b-a.json verification-output/scion-codex-fresh-b-a-working-v0.16.42/
```
...
Start one new ephemeral Codex CLI task with model `gpt-5.6-luna` and reasoning effort `max` selected explicitly. This workbook pins the outcome-independent public judge identity from the sealed first order. Before scoring any case, verify that this task can honestly use model "openai/codex", revision "gpt-5.6-luna@max", runtime "codex-cli-0.144.2", and prompt SHA-256 "0f062d551af9e2704892e5f1ebdf9b4c66a6d79de6ac1c9cf39b7cb4fa15ecd7". The fresh session ID must differ from "d602893b-ff2f-4892-a9c3-4a0d2b607870". If any identity is unavailable or different, stop before judgment; do not substitute another model, reasoning effort, runtime, or label. The revision is an auditable launch-profile token, not a claim about an unexposed provider build revision.
...
npm run complete:scion:codex-fresh-pass -- \
  --handoff evaluation/scion-adapters/handoffs/fresh-b-a-workbook-v0.16.42 \
  --receipt evaluation/scion-adapters/evidence/fresh-b-a-workbook-v0.16.42.json \
  --decisions-dir verification-output/scion-codex-fresh-b-a-working-v0.16.42 \
  --sealed-output verification-output/scion-codex-sealed-passes/v0.16.42-b-a.sealed.json \
  --key-output ~/.codex/scion-secrets/CourseMapper/v0.16.42-b-a.key
```
