...
Read and follow `single-model-training-atom-judge-prompt-v2.md`. The 91-case B/A pass is divided into immutable chunks. Cases are deterministically interleaved from the canonical packet to distribute domains across the schedule.

- `chunk-01-review-b-a.json` + `chunk-01-decisions-b-a.json` — 10 anonymous cases
- `chunk-02-review-b-a.json` + `chunk-02-decisions-b-a.json` — 9 anonymous cases
- `chunk-03-review-b-a.json` + `chunk-03-decisions-b-a.json` — 9 anonymous cases
- `chunk-04-review-b-a.json` + `chunk-04-decisions-b-a.json` — 9 anonymous cases
- `chunk-05-review-b-a.json` + `chunk-05-decisions-b-a.json` — 9 anonymous cases
- `chunk-06-review-b-a.json` + `chunk-06-decisions-b-a.json` — 9 anonymous cases
- `chunk-07-review-b-a.json` + `chunk-07-decisions-b-a.json` — 9 anonymous cases
- `chunk-08-review-b-a.json` + `chunk-08-decisions-b-a.json` — 9 anonymous cases
- `chunk-09-review-b-a.json` + `chunk-09-decisions-b-a.json` — 9 anonymous cases
- `chunk-10-review-b-a.json` + `chunk-10-decisions-b-a.json` — 9 anonymous cases

Create a working directory and copy only the blank decisions skeletons:

```bash
mkdir -p verification-output/scion-codex-fresh-b-a-working-course-group-breadth-v0.16.47
cp evaluation/scion-adapters/handoffs/fresh-b-a-workbook-course-group-breadth-v0.16.47/chunk-*-decisions-b-a.json verification-output/scion-codex-fresh-b-a-working-course-group-breadth-v0.16.47/
```
...
Start one new ephemeral Codex CLI task with model `gpt-5.6-luna` and reasoning effort `max` selected explicitly. This workbook pins the outcome-independent public judge identity from the sealed first order. Before scoring any case, verify that this task can honestly use model "openai/codex", revision "gpt-5.6-luna@max", runtime "codex-cli-0.144.5", and prompt SHA-256 "0f062d551af9e2704892e5f1ebdf9b4c66a6d79de6ac1c9cf39b7cb4fa15ecd7". The fresh session ID must differ from "019f6d2b-fbdc-70a0-8877-78946284fb35". If any identity is unavailable or different, stop before judgment; do not substitute another model, reasoning effort, runtime, or label. The revision is an auditable launch-profile token, not a claim about an unexposed provider build revision.
...
npm run complete:scion:codex-fresh-pass -- \
  --handoff evaluation/scion-adapters/handoffs/fresh-b-a-workbook-course-group-breadth-v0.16.47 \
  --receipt evaluation/scion-adapters/evidence/fresh-b-a-workbook-course-group-breadth-v0.16.47.json \
  --decisions-dir verification-output/scion-codex-fresh-b-a-working-course-group-breadth-v0.16.47 \
  --sealed-output evaluation/scion-adapters/evidence/codex-review-course-group-breadth-v0.16.47-b-a.sealed.json \
  --key-output ~/.codex/scion-secrets/CourseMapper/course-group-breadth-v0.16.47-b-a.key
```
