...
Read and follow `single-model-training-atom-judge-prompt-v2.md`. The 15-case A/B pass is divided into immutable chunks. Process chunks in numeric order. Score both anonymous artifacts before recording `winner`, `tie`, or `insufficient-evidence`. For a positional win, set `preference.decision` to the exact token `winner` and set `winnerPosition` to `1` or `2`; never use `first-artifact` or `second-artifact` as decision tokens. Preserve real ties, low-quality relative winners, and insufficient evidence. Do not manufacture a training preference.

- `chunk-01-review-a-b.json` + `chunk-01-decisions-a-b.json` — 8 anonymous cases
- `chunk-02-review-a-b.json` + `chunk-02-decisions-a-b.json` — 7 anonymous cases

Create a working directory and copy only the blank decisions skeletons:

```bash
mkdir -p verification-output/scion-codex-fresh-a-b-working-readiness-gap-v0.16.47
cp evaluation/scion-adapters/handoffs/fresh-a-b-workbook-readiness-gap-v0.16.47/chunk-*-decisions-a-b.json verification-output/scion-codex-fresh-a-b-working-readiness-gap-v0.16.47/
```
...
npm run complete:scion:codex-fresh-pass -- \
  --handoff evaluation/scion-adapters/handoffs/fresh-a-b-workbook-readiness-gap-v0.16.47 \
  --receipt evaluation/scion-adapters/evidence/fresh-a-b-workbook-readiness-gap-v0.16.47.json \
  --decisions-dir verification-output/scion-codex-fresh-a-b-working-readiness-gap-v0.16.47 \
  --sealed-output evaluation/scion-adapters/evidence/codex-review-readiness-gap-v0.16.47-a-b.sealed.json \
  --key-output ~/.codex/scion-secrets/CourseMapper/readiness-gap-v0.16.47-a-b.key
```
