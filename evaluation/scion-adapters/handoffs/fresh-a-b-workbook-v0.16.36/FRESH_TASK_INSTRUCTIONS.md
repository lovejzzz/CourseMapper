...
Start a newly created Codex Desktop task with model `gpt-5.5` and reasoning effort `xhigh` selected explicitly. The decision files must record revision `gpt-5.5@xhigh` and runtime `codex-desktop` exactly. `gpt-5.5@xhigh` is an auditable launch-profile token, not a claim about an unexposed provider build revision. Codex Desktop does not expose that internal build revision to this task, and this workbook records that limitation instead of inventing one.

Before scoring any case, verify that this task can honestly use model "openai/codex", revision "gpt-5.5@xhigh", runtime "codex-desktop", and prompt SHA-256 "0f062d551af9e2704892e5f1ebdf9b4c66a6d79de6ac1c9cf39b7cb4fa15ecd7". If any identity is unavailable or different, stop before judgment. Do not substitute a different model, reasoning effort, runtime, or label.
...
```bash
mkdir -p verification-output/scion-codex-fresh-a-b-working-v0.16.36
cp evaluation/scion-adapters/handoffs/fresh-a-b-workbook-v0.16.36/chunk-*-decisions-a-b.json verification-output/scion-codex-fresh-a-b-working-v0.16.36/
```
...
npm run complete:scion:codex-fresh-pass -- \
  --handoff evaluation/scion-adapters/handoffs/fresh-a-b-workbook-v0.16.36 \
  --receipt evaluation/scion-adapters/evidence/fresh-a-b-workbook-v0.16.36.json \
  --decisions-dir verification-output/scion-codex-fresh-a-b-working-v0.16.36 \
  --sealed-output verification-output/scion-codex-sealed-passes/v0.16.36-a-b.sealed.json \
  --key-output ~/.codex/scion-secrets/CourseMapper/v0.16.36-a-b.key
```
