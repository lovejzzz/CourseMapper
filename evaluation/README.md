...
The pre-review boundary uses the same unit. `audit:scion:review-candidates` gives every atom an exact-input course-group ID and digest; one explicit label cannot be reused across changed inputs. It now builds into a temporary directory and byte-compares the result with the tracked ledger, so CI cannot conceal admission drift by rewriting the input. The current ledger contains 400 neutral atoms across sixteen distinct courses, four per research domain; 92 carry source context that also passes the stronger current item-admission gate. `audit:scion:review-packet` first verifies all source-capture projects, then balances by domain, group, and kind. Neutral packet protocol v4 hashes each public case, separately hashes the private source row and A/B mapping, folds the organizer digest into the packet hash, and keeps the optional instructor-review protocol distinct. The historical sealed organizer remains bound to its original 437-atom ledger and 160 selected cases. Source-first round-robin selection retained 128 cases with the same neutral source claims above both anonymous candidates; 32 legacy fill cases remained visible in that packet but were excluded from Codex training review. Both isolated 128-case orders are now outcome-sealed, but the A/B and B/A passes used different Codex revision/runtime identities. Ingestion therefore quarantines all 128 rows and approves zero preferences; order and revision effects are confounded, so this remains analysis rather than training evidence.
...
The tracked [`fresh-a-b-workbook-v0.16.36`](scion-adapters/handoffs/fresh-a-b-workbook-v0.16.36/) and its [`receipt`](scion-adapters/evidence/fresh-a-b-workbook-v0.16.36.json) remain a reproducible historical blank handoff, but they are no longer the next campaign input. They contain 128 source-bound anonymous cases created before the stronger shared item-admission gate; the current deterministic rebuild admits only 92 source-grounded cases. Scoring that workbook would spend judge effort on rows that cannot satisfy today's training boundary and could not reach the 100-preference research floor even with perfect agreement. The next campaign must first add at least eight new admissible source-grounded cases, then build new A/B and B/A workbooks under one honestly available judge identity. The historical workbook records **0 completed judgments and 0 preferences** and must remain unscored; it is infrastructure history, not adapter, model, human, independent, or paid-reference evidence.

The historical v0.16.35 A/B workbook remains byte-reconstructable through `npm run audit:scion:codex-first-order:v0.16.35`, but its declared `codex-gpt-5-2026-07-15` revision was not a verifiable Codex Desktop launch identity. It was superseded before scoring and must not be used as campaign input.
...
The current promotion ruler is `evaluation/scion-adapters/held-out-course-benchmark-v16.json`. It preserves the five disjoint World Languages, World Literature, Psychology, Nutrition, and Astronomy courses, task routes, course inputs, source packets, and native-attempt boundary from V15, then freezes deep-quality grader 1.10.27 and texture metric 1.1.0. The grader recognizes legitimate genetics vocabulary, rejects a manifest-promised graded artifact when its document is only a no-brief shell, and scores long-title mail-merge repetition in briefs and lesson plans. Candidate runs must prove adapter use for `source-grounded-lesson-kernel`; both arms must prove base-only `course-map` and `lesson-kernel-synthesis` routes; unclassified work is forbidden. Every course has 12–15 lessons and binds its complete prompt-only course input and source packet. The ruler binds the exact QAT base contract plus a canonical transitive receipt over the grader wrapper and every relative implementation module. Older rulers remain historical diagnostics and cannot provide a V16 score or promotion result. Any benchmark domain or course group present in the candidate dataset blocks the run instead of triggering a convenient fixture substitution.
...
The isolated Chrome runner uses the immutable public base URL, independently hashes the exact cached 3,349,514,112-byte OPFS file, and retains a browser trace, console log, sanitized hardware probe, and redacted runtime snapshot. The July 14 Apple M4 Max/48 GiB run passed WebGPU/JSPI capability; 35,626 ms cold recovery and 1,041 ms warm load; 285 ms base and 316 ms adapter first tokens; 5,312 MiB peak Chrome working set; native scale-16 activation with a changed output; exact base/project rollback; a real network abort at 8,731,096 bytes; adapter cache eviction/redownload; real Chrome GPU-process restart with observed inference failure; and successful reload/completion. The fresh v0.16.39 run repeats that lifecycle on the exact seeded-training lineage at 5,606 MiB peak. Its audit accepts the three known runtime/page errors only when the GPU-process restart, observed failed completion, and successful base recovery are all present; any other error blocks the receipt. Trace finalization scrubs the actual workspace, profile, and home paths, while the retained-evidence audit rejects generic local user paths and network secrets before accepting the receipt.
