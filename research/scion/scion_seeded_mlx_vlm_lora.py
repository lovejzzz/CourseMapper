...
def _mlx_completion_target_mask(input_ids, attention_mask, mx):
    """Mask the predicted TARGET, including its attention bit, not its input.

    Right padding must not add a PAD prediction to the ORPO objective. Keeping
    the target attention bits also handles left padding and masked holes.
    This remains entirely in MLX's differentiation graph.
    """
    steps = mx.arange(input_ids.shape[1] - 1)[None, :]
    starts = _mlx_completion_prediction_starts(input_ids, mx)
    return mx.logical_and(
        steps >= starts[:, None], attention_mask[:, 1:] != 0
    )
...
def launch(seed: int, validation_split: str, forwarded: list[str]) -> None:
    ...
    def get_logps_with_exact_completion_boundary(
        ...
        mask = _mlx_completion_target_mask(input_ids, attention_mask, mx)
