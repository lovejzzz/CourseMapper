...
| **6** | **Ships as a Default** | **Achieved ← current** | Default item-author for Researcher-Zero — the $0 path had **no items at all** before, so this is a capability unlock, not just a saving. (Paid runs keep it opt-in.) |
| 7 | Wins Everywhere | **Locked** | Parity/win on *every* domain incl. lexically-dense. **Measured negative:** prompt-hardening (v2) *regressed* dense 10→4 — not a prompt problem. Real quest: preference data. |
...
| 9 | Learns From Its Mistakes | **Locked** | Self-improves via preference (DPO) on its own rejects — the *only* way up, since SFT is retired (taught it to copy). Blocked: stable mlx-lm has no DPO; only 123 natural pairs so far. |
| 10 | Human-Anchored | **Locked** | Two human readers confirm the output teaches. The final boss — every number above is SIMULATED until this is read. |
