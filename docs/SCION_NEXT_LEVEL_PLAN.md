...
**Status:** historical v0.16.6 adapter infrastructure and compiler-efficiency pass implemented; later releases completed deterministic exact-QAT training, frozen paired evidence, sixteen course-group-bound inputs, two outcome-sealed but cross-revision Codex orders, dual-envelope plaintext-free ingestion, one exact seeded-training-to-real-browser lineage on Apple Silicon, and a strict 100-case source-grounded campaign built from a 446-case reproducible ledger. Research and production training remain correctly blocked by zero stable same-identity approved preferences; fresh A/B and distinct-task B/A judging, the other three device profiles, and all adapter quality wins remain unproven.
**North star:** Scion produces a more teachable, more internally coherent course than a paid frontier baseline at a fraction of the cost, and anonymous hash-bound evidence survives reversed-order Codex judgment.
