...
**Status:** exact-QAT smoke training, deterministic GGUF conversion, browser activation and rollback, a dual size budget with bounded streaming and atomic adapter installation, frozen paired evaluation, a provenance-bound Codex comparison and training-preference lane, 128 source-bound review cases across sixteen course groups, one outcome-sealed 128-case A/B Codex pass, an immutable B/A-only fresh-task handoff, an eight-chunk one-session B/A workbook with canonical in-memory assembly, dual-envelope plaintext-free ingestion, deterministic incomplete-explanation recovery with four-domain replay proof, separate optional human research lanes, and a semantic four-profile device protocol are implemented; the fresh B/A judgment, stable training preferences, quality adapter, and five-domain adapter win do not exist yet
...
| Dataset boundary   | Pair audit, deduplication, explicit domain/course grouping, deterministic group split, model-judge distribution, overlap check, file hashes, and quarantine ledger                                                   | `npm run build:scion:adapter-dataset`                                                                                                         |
...
| Browser delivery   | Pinned runtime assets load the public 3.35 GB GGUF; separate adapters are capped at 64 MiB and 2% of that base, require exact bounded streaming, and commit atomically only after every byte and digest passes       | `npm run audit:scion:browser-base && npm run audit:scion:browser-lora && npm run audit:scion:adapter-delivery`                                |
...
| Promotion          | Five unique hash-paired held-out domains on one clean compiler/grader protocol, exact adapter/base state, 99/A and zero P0/P1, per-domain call ceiling, 20% median call reduction, and the declared Codex comparison | `npm run audit:scion:adapter:promotion -- --manifest ... --candidate ... --base ...`                                                          |
