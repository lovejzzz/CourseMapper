...
**Inference lane:** public Scion, the browser-local provider, and the production-compatible OpenAI shim project an exact trusted ledger with `modelCalls: 0`. Base Gemma still authors the course skeleton and limited generative surfaces. The fresh Mandarin package uses two actual Gemma generations; the complete five-course matrix uses 28 actual generations with zero model failures, rather than reopening the former 72-call single-course architecture.
...
**Artifact lane:** the 99/A matrix was not accepted until representative DOCX files were opened. That inspection found and fixed duplicated assessment weights in World Literature, a severed misconception narrator in Psychology, and constructed-response keys that copied learner directions instead of selecting decisive case evidence. The answer projection now keeps the relevant scenario sentence and admitted source claim, states a bounded conclusion, and drops prompt echo.
...
**Exact proof:** Mandarin, World Literature, Psychology, Nutrition, and Astronomy each complete every requested lesson, all ten material sections, and 38/38 export checks at 99/A with zero P0/P1/P2 findings, blockers, or warnings. Their tested archives contain 127, 117, 125, 117, and 101 extracted files. Console receipts contain no warning or error events. The local release gate passes 434 test files and 5,321 tests, format, lint, production build, locked bundle budgets, and release-history audit.
