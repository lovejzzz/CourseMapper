...
**Canary diagnosis:** 54 assessment items still use conservative, source-review-required compiler recovery; only 26% of multiple-choice stems reach the applied-evidence depth target; and one lesson misses a visible Hanzi–Pinyin pair across all learner surfaces. A stricter experiment that made pair co-location a whole-kernel admission condition collapsed enrichment coverage to 5/15. It was rejected. The next repair belongs in compiler projection, where paired language can be carried into learner artifacts without discarding valid lesson knowledge.
...
**Frozen ruler V12:** V12 preserves the five disjoint held-out courses, split task routes, and native-attempt boundary from V11. It narrowly expands Mandarin citation vocabulary and binds the new paired-language projection diagnostic. Both arms prove base-only course-map and synthesis work; only the candidate may prove an active adapter on source-grounded refinement. V12 inherits no V11 score or promotion result.

**Next gates:** retrain the exact-scope adapter from a clean commit; run one bounded V12 canary to verify the staged route; only then run all five candidate and base arms, package and device checks, and the frozen anonymous quality comparison. Promote nothing unless the exact candidate improves held-out quality without unacceptable native-inference, download, memory, or runtime cost.
