...
**Status:** the exact-QAT base, hash-bound training and conversion chain, browser adapter lifecycle, source-strict semantic compiler gates, frozen holdout firewall, and paired-order preference protocol are implemented. The research corpus still contains 102 qualified source-grounded preferences and rebuilds into 100 group-isolated production rows across seven domains. The first complete task-scoped candidate remains rejected and inactive: it produced no cross-domain learner-facing win, added factual defects, ran 2.81× longer, and exceeded the browser adapter budget. V0.16.76 freezes production on exact base-only Scion after one V21 run brings all 70 lessons across five domains to 99/A with zero blockers, warnings, P0/P1/P2 findings, retries, or failed exports. That base is now the promotion floor: an adapter must beat complete learner-facing artifacts without regressing quality, speed, browser size, or source fidelity. Anonymous World Literature review still prefers the paid reference in both presentation orders, so paid-reference parity remains an explicit unearned gate. Gemma weights are unchanged.
