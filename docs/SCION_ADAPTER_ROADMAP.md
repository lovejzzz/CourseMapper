...
**Status:** exact-QAT smoke training, a timestamp-independent dataset identity with source and split receipts, a schema-v4 frozen-holdout firewall with course-ID relabel protection, a pinned Apple MLX toolchain, dual-seeded ORPO launch, hash-bound plan and completion receipts, schema-v3 learned-adapter provenance through GGUF conversion, deterministic GGUF conversion, browser activation and rollback, a dual size budget with bounded streaming and atomic adapter installation, one registry-coordinated canary/install/activation/deactivation path with rollback quarantine, frozen paired evaluation, a promotion-independent package identity, semantic factual and production canaries, a provenance-bound Codex comparison and training-preference lane, semantic promotion verification against both base and the pinned paid reference, 128 source-bound review cases across sixteen course groups, outcome-sealed 128-case A/B and B/A Codex passes, fail-closed cross-revision analysis with zero approved preferences, a tracked A/B-only first-order clean room with judge identity declared before scoring, deterministic explicit-cue answer-key repair, placeholder-option rejection, a judged high-confidence cross-field key-term gate, dual-envelope plaintext-free ingestion, deterministic incomplete-explanation recovery, a two-campaign cross-arm compiler-lift replay, shared key-term semantics, bounded local defect recovery, conservative cross-attempt field retention, Scion-backed advisory Agent chat, one six-stage observable build meter, separate optional human research lanes, a semantic four-profile device protocol, and one real passing Apple-Silicon recovery profile are implemented; a completed new A/B judgment, stable same-revision training preferences, a quality adapter, native Scion Agent tool calling, the other three device profiles, and a five-domain adapter win do not exist yet

**Release boundary:** no current public Scion request claims to use trained weights

## v0.16.35 — The First Order Gets a Clean Room
...
**Release boundary:** this milestone makes the first valid campaign input reproducible; it does not perform the A/B judgment, create a reverse-order agreement, add a training row, train or activate an adapter, improve Gemma weights, beat base-only Scion, match GPT-5.4-mini, add a device profile, or establish human, independent, instructor, classroom, or multi-judge evidence. The next task must judge only this A/B workbook in one fresh matching Codex session and seal it without inspecting outcomes; a later distinct task may then build B/A from the sealed public identity.
