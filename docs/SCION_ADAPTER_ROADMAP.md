...
Every adapter/base course comparison is now a cryptographically paired experiment rather than two records that merely share a domain label. Both arms must bind the same course input, source packet, clean compiler commit, compiler configuration, grader version, active base-weight digest, course identity, and 12-lesson scope. The candidate arm must use the manifest's exact adapter and scale; the control must prove base-only state. Every candidate domain needs a control and vice versa, and the adapter may not regress package grade or P2 findings. Duplicate domain records, reused pair IDs, dirty compiler trees, unmatched domains, and mismatched settings fail the promotion audit instead of allowing a favorable record to overwrite an unfavorable one.
...
- The promotion audit now requires one unique, clean, hash-paired adapter/base course per domain. It rejects duplicate or reused comparisons, different inputs or sources, different compiler or grader settings, different active base weights, dirty worktrees, mismatched adapter scales, and controls that are not demonstrably base-only.
