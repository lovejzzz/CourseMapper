...
**Inference lane:** public Scion, the browser-local provider, and the production-compatible OpenAI shim project an exact trusted ledger with `modelCalls: 0`. Base Gemma still authors the course skeleton and limited generative surfaces. The fresh Mandarin package uses two actual Gemma generations; the complete five-course matrix uses 30 actual generations with zero model failures, rather than reopening the former 72-call single-course architecture.
