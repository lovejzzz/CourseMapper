...
**Status:** the exact-QAT base, hash-bound training and conversion chain, browser adapter lifecycle, source-strict semantic compiler gates, frozen holdout firewall, and paired-order preference protocol are implemented. The research corpus still contains 102 qualified source-grounded preferences and rebuilds into 100 group-isolated production rows across seven domains. The first complete task-scoped candidate remains rejected and inactive: it produced no cross-domain learner-facing win, added factual defects, ran 2.81× longer, and exceeded the browser adapter budget. V0.16.76 remains the immutable base-only adapter-promotion floor. V0.16.77 adds the model-neutral experiential-activity IR. V0.16.78 adds a matched visible-quality settlement: the same base responses compile into less repetitive slides and win both anonymous presentation orders across six complete course packages, but the candidate timing is slower and no model-weight, adapter, classroom, or paid-reference win is claimed. Gemma weights are unchanged.

## V0.16.79 — A Round That Explains Its Own Clock

V0.16.78 recorded a 2.9× round slowdown (baseline digest median 49,335 ms, candidate 169,110 ms) that it could not explain, and honestly declined to claim a latency win. Six controlled runs on `astro-101` now attribute it. Cold model load costs about 70 s per course; filmstrip capture costs 2 s and nothing in-page; and the V0.16.77 → V0.16.78 code delta costs nothing at all — both versions generate the same course in 48.6 s under identical warm conditions. **No product latency regression existed.** The remaining cost was GPU contention: two concurrent courses on one device pushed per-course generation from 48.6 s to 84.8 s and 128.8 s.

Browser-local providers therefore default to one course at a time. Public Scion and the local runtime execute inference on this machine's single GPU, so concurrency contends rather than overlaps; remote providers keep the parallel default of 2, and an explicit `--concurrency` still wins. The same two-course cold round finishes in 5m34s instead of 16m01s — 2.87× faster — with identical 99/A grades, zero P0/P1 findings, and 101 extracted files per course.

Every round now explains its own clock. `round.json` and `ROUND_REPORT.md` record the configuration that decides comparability (concurrency and whether it was explicit, filmstrip capture, Scion profile mode, authoring, voice) and split each course into model load, of which download, generation, and total. The split is a pure function over the console log each run already saves, with unit coverage for cold, warm, and remote-provider rounds, so nothing was added to the live browser path and the measurement cannot perturb the timings it reports.

**Release boundary:** this milestone changes evaluation tooling and round wall-clock only. No generated course content, compiler output, or quality score changes. Gemma weights are unchanged, the research adapter remains inactive and non-promotable, and no factual, teachability, instructor, accessibility, classroom, adapter, or paid-reference claim is made. The absolute model-load seconds are specific to the measuring machine; the attribution is not.

## V0.16.78 — Make Every Slide Earn Its Place

The settlement freezes the v0.16.77 source commit, five production courses, one unseen Environmental Policy course, the public base route, inactive-adapter state, grader, bundle ceilings, compiler-size ceiling, npm-script count, release-contract count, tracked model weights, and large-binary policy before changing learner-facing output. Texture metric 1.2.0 measures exact and skeleton repetition over visible artifact units and comparable windows. It remains diagnostic beside the frozen quality score.

The one causal intervention is compiler-owned. Slide agendas vary the evidence decision, fact-ledger leads expose an admitted claim to test, discussion frames rotate source boundaries, criteria, counterevidence, artifact decisions, and revision consequences, and instruction-shaped objectives are compacted before becoming labels. The model responses and facts are reused. Baseline and candidate both use 29 provider calls and zero retries.

The target slide-deck skeleton median falls from 22.22% to 17.57%, a 4.65-point and 20.9% relative reduction, with all six courses improving. Package skeleton and exact-repetition medians also fall. In two new isolated GPT-5.6-sol XHigh sessions, the candidate wins every course in both anonymous presentation orders: 12/12 course-order verdicts with no candidate factual or source-boundary regression.

All six candidate packages pass first attempt at full scope with 100% kernel coverage, 9/9 compiled material families, A grades from 98 to 99, zero P0/P1/P2 findings, blockers, warnings, retries, or failed calls, and 38/38 export checks. All matched archives pass integrity testing. Rendered inspection covers 221 PPTX slides and 30 DOCX pages and closes four real layout failures. A separate resumed-browser proof reaches 99/A with Texture 97, returns a new lesson-grounded Scion Agent answer, downloads a valid physical ZIP, and passes desktop plus 390×844 light/dark inspection with a clean console.

**Adapter implication:** this is the evidence pattern a future adapter must beat. A candidate must improve complete anonymous artifacts under both orders while preserving factual/source boundaries, release health, download budget, and runtime. The v0.16.78 compiler win does not rescue the rejected research adapter or authorize its production use.

**Claim boundary:** the instrumented candidate round is materially slower than the baseline despite unchanged provider calls and retries, so no latency win is claimed. The AI-only comparison is not human instructor review, independent factual verification, accessibility certification, classroom-outcome evidence, or paid-reference parity. Shared defects surfaced by the judge remain future compiler or authoring targets. The complete machine-readable receipt is `docs/evidence/SCION_V01678_SETTLEMENT_ACCEPTANCE.json`.

## V0.16.77 — One Activity, Every Surface

The compiler now recognizes only explicit experiential requests: simulation, laboratory investigation, studio critique, case exercise, structured debate, field exercise, and role-play. A qualifying lesson asks the model for one compact `scion-experiential-activity-v1` blueprint inside its existing lesson-kernel response. An ordinary lesson receives no activity instruction or activity schema at all; this keeps the archived 148-case production campaign byte-identical and preserves its downstream evidence receipts.

The canonical IR owns the course-specific situation, participant or functional working roles, role goals and constraints, inspectable evidence, evolving phases or updates, required decisions or actions, one named artifact and its requirements, debrief prompts, a safety/evidence/realism boundary, and authored timing weights. A generic selector retains only the exact source-brief clause that matches the lesson's requested activity kind, keeping the instructor's mechanics lesson-local without copying the entire course prompt into every call. The template then seeds only the activity label from the requested lesson title and explicit form and requires the model to preserve it exactly. These source-derived atoms reduce small-model drift without adding a discipline scenario; admission rejects any substituted form. The public Scion compact schema and the general paid-provider prompt both target this same representation. Semantic admission rejects missing ids, unexpected or duplicate blueprints, placeholders, meta language, weak or overlong fields, out-of-range array counts, ungrounded evidence, mistyped timing, duplicate phase names, and incomplete mechanics.

After admission, one deterministic projection expands the IR into the lesson-plan outline, activity-specific objectives and accessibility support, slide frames, a student activity packet, the editable in-app preview, Assignment Brief DOCX, ZIP, and deep-quality grader. The compiler adds generic mechanics only: evidence-log fields, equivalent participation modes, format scaffolding, safe boundaries, and exact timing normalization. It contains no fixed actors, facts, discipline story, or lesson scenario.

Timing has one authority. Model-authored phase minutes are treated as relative weights and normalized once, with a deterministic largest-remainder allocation, to the actual course session length. Lesson-plan segments and slide frames consume the same normalized phase names and minutes. They do not independently rebalance the clock. A requested activity whose blueprint fails admission falls back to the standard lesson compiler with an explicit non-claim; the compiler does not synthesize a generic activity from title keywords.

Generality is covered by five unrelated fixtures: a 50-minute international-relations negotiation, 60-minute engineering lab, 75-minute UX studio critique, 90-minute counseling role-play, and 120-minute transit-policy case. The test matrix asserts admission, exact timing, content retention, activity-packet uniqueness, lesson-plan and slide projection, responsive UI rendering, DOCX and ZIP handoff, and score-bearing rejection of hollow activity text.

The final fresh browser proof compiled a three-lesson UX Research course in 117.957 seconds using eight model requests. It admitted 3/3 lesson kernels, compiled 9/9 materials, finished at Quality 99/A and Texture 96 with zero findings, zero finish blockers or warnings, and no warning/error console entries. The same model-authored 75-minute studio critique appeared in the lesson plan, four-phase slide sequence, single student packet, DOCX, PPTX, and physical ZIP. That downloaded archive passed integrity and structural checks and independently regraded 99/A with zero findings. The exact run ids and ZIP SHA-256 are retained in the V0.16.77 acceptance receipt.

**Adapter implication:** this capability does not require a new adapter. A future task-scoped adapter may learn to author the compact activity IR more reliably, but it must still beat the exact base on the same complete exported artifacts. The compiler capability is valuable now because Scion and compatible paid providers both inherit it.

**Claim boundary:** V0.16.77 proves encoded cross-domain generality, deterministic projection, and release artifacts for this activity contract. It does not prove that every generated fact is correct, that an instructor would adopt every activity, that classroom outcomes improved, that the research adapter is better, or that Scion reached paid-reference parity.

## V0.16.76 — Freeze the Base That Earned Green

The production route is fixed to `google/gemma-4-E2B-it-qat-q4_0-unquantized` at revision `1ca4dd94b623b6e0dd9da00c2239ab84b4f3e5ce`, with no adapter installed or active. The ruler is `evaluation/scion-adapters/held-out-course-benchmark-v21.json`, SHA-256 `509a82e89936aa5dd070f57b06688e3557eab80e6bd42e9e0c0ee0a4303040c8`. V21 fixes the five course identities, source packets, grader receipt, compiler boundary, and runtime-task policy; it inherits no earlier score or promotion decision.

One same-code Crucible run completes Mandarin (15 lessons), World Literature (14), Psychology (15), Nutrition (14), and Astronomy (12) at 99/A each. Every requested lesson and lesson kernel is present. All five packages have zero blockers, warnings, scored P0/P1/P2 findings, retry calls, flagged checks, failed export checks, or export warnings; each passes 38/38 verification and physical ZIP testing. The round uses 41 provider calls for 70 lessons and no repair cascade.

Artifact review is part of the gate. The extracted learner-facing text is clear of the exact punctuation seams, raw modality identifiers, malformed discussion grammar, clipped source prose, metadata punctuation failures, repeated topic identities, and false dangling-fragment findings exposed during focused review. Across the five packages, 589 structured teaching visuals bind a visual kind to evidence, artifact purpose, student action, and accessibility text.

A separate cold-start browser proof exercises the path a new user sees: the public base downloads from 0% to ready, the Living Course Compiler advances through Model, Map, Enrich, Compile, Verify, and Grade, a new three-lesson Epidemiology course finishes in about 116 seconds with four calls and no retry, the Agent summarizes the compiled sequence, and one Download ZIP action creates a valid 39-file archive. The inspected console contains no warning or error. Frame review also catches and repairs fixed-pixel slide artboards that previously became clipped inside a narrow dark workspace.

**Promotion rule from this freeze:** no adapter becomes production because it is smaller, novel, or better on an aggregate score. It must beat the V0.16.76 base on complete anonymous learner-facing artifacts under both presentation orders, preserve the zero-defect release gates, remain within the browser download budget, and avoid a material latency regression. Until then, production stays base-only.

**Shared benefit boundary:** admission, sequencing, source-backed projection, compiler language repair, assessment reconciliation, grading, Agent evidence, persistence, and export verification are model-neutral and benefit compatible providers. Public model download, WebGPU inference, local cache, and exact base identity verification are Scion-specific.

**Claim boundary:** this is a bounded five-course and first-use browser proof. It is not universal factual correctness, accessibility, teachability, instructor approval, classroom outcomes, adapter superiority, statistical confidence, or paid-reference parity.
...
**Goal:** treat Scion as a living authoring system rather than a one-time text generator. The exact named readings, assessment weights, comparative evidence contract, quality receipt, Agent evidence, persisted project, exporter provenance, and downloadable files must survive the entire browser journey.

**Exact browser proof:** the eight-week World Literature scenario completes from the cached pinned base in 51 seconds with one course skeleton call, eight lesson enrichments, and zero rewrite or repair calls. It reaches 8/8 mapped lessons, 8/8 admitted kernels, 9/9 material families, 99/A with texture 93, zero P0/P1/P2 findings, ten of ten checked sections, one Download ZIP action, and no browser warning or error. The first-use path also exposes real model download and preparation progress before the same compiler stages.

**Instructor-contract lane:** all eight named readings remain visible with edition-neutral language. The assessment registry retains Comparative Reading Responses 30%, Comparative Essay Proposal 15%, and Final Comparative Paper 55%. The first response pairs _The Odyssey_ with _The Thousand and One Nights_; the second pairs _Antigone_ with _Things Fall Apart_. Briefs and rubrics require locatable evidence from both works, a credible counter-reading against the paired evidence, substantive revision, and an explicit claim limit.

**Agent and persistence lane:** the Agent locates that exact comparison in Lesson 2 Assignment Briefs and explains the whole evidence contract directly from compiled course state without another provider call. A completed 5.5 MB anonymous project writes its exact state to IndexedDB and leaves a 232-byte localStorage recovery marker. A real reload restores 99/A and 9/9 materials in about 1.9 seconds without recompiling or regenerating.

**Export and migration lane:** a resumed project falls back from the live run-digest ref to the persisted digest, so its 38/38 export-verification receipt remains attached. The manifest records the current exporter and original generator versions separately when a project crosses a release boundary. The final 1.29 MB ZIP reports exporter V0.16.74, generation V0.16.73, Scion V0.16.74, base-only adapter status, zero blockers and warnings, all eight readings, and the 30/15/55 assessment contract. It passes compressed-data testing and macOS extraction.

**Responsive lane:** at 390×844, Content, Agent, and Export remain explicit reachable modes. The header, Living Course Compiler, course content, Agent, quality state, and single download action stay readable without collapsing the desktop workspace into unusable rails.

**Frozen ruler V21:** deep-quality grader 1.10.35 binds its thirteen-file transitive implementation receipt at `42b67270f7b17916857eb6ce583d5cc1400e1120f8a7733c859b4f948e7c0f40`. V21 preserves V20’s five disjoint course and source fixtures but inherits no score, adapter result, or promotion decision.

**Cross-domain compiler lane:** the full 40-course contract audit found one valid defect outside the browser scenario. Bare “synthesis matrix” language in an Information Literacy lesson activated the deterministic linear-algebra fallback, injected a `2 × 3` matrix example, expanded that deck to seventeen slides, and left no live-practice time. Matrix language is now ambiguous unless it carries actual mathematical context such as algebra, equations, matrix operations, or a named linear-algebra construct. The exact Information Literacy regression returns to zero blockers and warnings, quantitative neighbors remain green, and the complete contract closes at 40/40.

**Release-hardening lane:** whole-course schedule/readings answers are isolated as a 1.7 KiB question-triggered Agent leaf; the ordinary compiled-course answer remains 8.0 KiB and the initial landing route remains 255.9 KiB raw / 81.3 KiB gzip without relaxing a bundle ceiling. The former one-assessment native-authoring hang fixture now repairs to fifteen assessment anchors, passes the semantic contract with zero blockers, and compiles its syllabus. The final stable suite passes 447 files and 5,494 tests, with 16 files and 162 tests intentionally skipped.

**Lane:** named-reading identity, assessment reconciliation, comparative compilation, Agent evidence, exact persistence, grading, and resumed export are model-neutral and benefit compatible paid-provider output. The public GGUF download, WebGPU runtime, local cache, and browser-local inference remain Scion-specific.

**Release Boundary:** this release proves bounded automated browser, persistence, package, and encoded artifact-defect behavior for the recorded World Literature journey. The external adoption audit finds no package blocker but remains capped without professor, public-source benchmark, or independent-review evidence. It does not prove universal factual correctness, accessibility, teachability, instructor approval, classroom outcomes, human preference, adapter superiority, statistical confidence, or paid-reference parity. The adapter remains inactive and Gemma weights are unchanged.
...
**Goal:** complete one reproducible base-only browser freeze across Mandarin, World Literature, Psychology, Nutrition, and Astronomy with every requested lesson, 100% usable lesson knowledge, zero readiness or encoded artifact-quality findings, and physically valid exports.

**Baseline defects:** the first five-domain attempt left Mandarin and Psychology blocked. Mandarin's prompt requested fifteen lessons but named fourteen content clauses; the small model split those clauses inconsistently, shifted later identities, admitted only 14/15 kernels, spent two retries, and finished 89/B. Psychology paired an earlier artifact noun with a later “check on” phrase and created a false prompt-contamination blocker; its downstream artifact reconciliation also ignored stable assessment IDs, and the learner-paper readability signal counted hidden planning metadata.

**Source and projection lane:** World Literature comparative-reading and comparative-essay integrators project only verbatim facts from earlier admitted assigned-text kernels, with explicit provenance. The exact Mandarin contract inserts one bounded vocabulary-and-grammar retrieval bridge between the first grammar unit and later content clauses. Unicode-aware substantiveness counting recognizes Hanzi and other non-Latin learner-language evidence instead of treating a valid grammar statement as too short to teach.

**Assessment lane:** stable assessment IDs resolve before rewritten titles across blueprint, assignments, rubrics, map, and syllabus candidates. All cross-concept exam variants explicitly require independent concept selection, a distinct evidence detail for each concept, an articulated relationship, and a bounded inference. Learner-paper readability excludes internal blueprint and instructor-feedback fields, while visible questions, answer guidance, samples, and scoring directions use shorter sentences.

**Failure-evidence lane:** a package blocked during finalization is preserved as an in-memory forensic ZIP, and the browser harness waits through autosave before capturing the failure snapshot. This turns a gate failure into inspectable artifact evidence without weakening the gate or forcing another complete generation.

**Exact proof:** `round-2026-07-22T23-04-02-354Z` runs the shipped V0.16.72 code and completes all five held-out courses at 99/A. Every digest reports ready, 100% enrichment coverage, zero blockers, warnings, scored P0/P1/P2 findings, retry calls, failed export checks, or flagged checks. Each package passes 38/38 export verification and physical ZIP testing. The archives contain 127 Mandarin, 117 World Literature, 125 Psychology, 117 Nutrition, and 101 Astronomy files. The complete round uses 40 provider calls and zero retries.

**Lane:** sequencing, evidence projection, semantic admission, stable assessment reconciliation, learner-paper readability, constructed-response depth, deterministic compilation, grading, and export are model-neutral and benefit compatible paid-provider output. The exact Mandarin ledger, public-weight browser download, WebGPU inference, local cache, and Scion recovery route remain Scion-specific.

**Release Boundary:** this release proves bounded browser, compiler, grader, export, and encoded artifact-defect behavior on five frozen courses under the exact pinned base. It does not prove universal factual correctness, accessibility, teachability, instructor approval, classroom outcomes, human preference, adapter superiority, statistical confidence, or paid-reference parity. The adapter remains inactive and Gemma weights are unchanged.

## V0.16.71 — Make Ready Mean Ready

**Goal:** close the exact supplied Physical Geology blocker, ensure a genuinely clean package receives one confident green terminal state, deepen the weakest learner-facing literature surfaces, and keep the remaining model-quality gap visible through anonymous review.

**Exact incident closure:** the supplied V0.16.67 Geology archive is physically healthy, but its registered `Final (50%)` assessment has no corresponding exam content, creating the 74/C P0. Explicit exam titles now outrank contradictory generic assessment kinds across graph, IR, registry, compiler, and export boundaries. The fresh pinned-base run completes in 55 seconds with three provider calls, 14/14 admitted kernels, nine compiled material families, 38/38 export checks, a valid 118-file archive, and 99/A with zero P0/P1/P2 findings, blockers, warnings, retries, console errors, or archive errors.

**Premium trust state:** score and actionable readiness are no longer conflated. A zero-finding A/B package stays transparent about a 99 score or 95 texture meter but receives the green `Ready to download` state after export verification passes. A real quality finding, low grade, source caveat, export warning, or failed export still produces review or blocked treatment. The package has one `Download ZIP` owner and current Scion provenance says `AI-generated`.

**Export identity lane:** current-tab export now scopes an assignment by its explicit related lesson before looking at week-like metadata. A Lesson 1 brief due in Week 4 therefore remains a Lesson 1 artifact instead of disappearing as a fictional Lesson 4 item. The exact browser regression now reaches `Ready` and downloads both CSV and DOCX.

**Persistence lane:** when a complete workspace exceeds localStorage quota even after deliverables are omitted, autosave persists a compact, re-compilable course-map snapshot. The user keeps the completed map and can reconstruct downstream materials without spending another model run.

**World Literature intervention:** named readings receive locatable-passage tasks, counter-readings, contextual boundaries, and reading-specific concepts. The grading plan gives the final paper 16% and its proposal 5%; rubric self-assessment, analysis, revision, and submission language rotate by lesson without changing criteria. A fresh browser package reaches 98/A with zero P0/P1 and no blocker or retry; the final no-model compiler replay removes its remaining repetition P2.

**Anonymous gap measurement:** complete syllabus and learner-facing artifacts for the selected literature lessons are reviewed A/B and reversed B/A. The paid reference wins 71–44 and 63–45. Scion’s evidence/revision structure and coherent culmination weight are real strengths; shallow concept definitions, circular quiz language, and factual overreach around _The Library of Babel_ remain disqualifying parity gaps.

**Frozen ruler V18:** V18 preserves V17’s five disjoint course and source packets but binds deep-quality grader 1.10.31 and its 15-file transitive implementation receipt after reader-visible seam checks entered the path. It inherits no V17 score, adapter result, or promotion decision.

**Release gate:** 434 unit-test files pass with 5,361 tests, 147/147 Chromium tests pass, and format, lint, production build, bundle budgets, archive integrity, and release-history audit are green. The final exact V0.16.71 Geology round is `round-2026-07-22T19-44-27-370Z`; its 129-entry ZIP hashes to `2bf4179de87c15bf2996851d1bedb380cfffc66be6549ca0d1a0ae5862186ffe`.

**Lane:** concept projection, assessment planning, rubric variation, sentence-seam repair, trust semantics, local-save recovery, grading, and export are model-neutral and benefit compatible paid-provider output. Browser WebGPU loading, public-weight download, caching, and local inference remain Scion-specific.

**Release Boundary:** this release proves the supplied incident, the corrected browser package, the named compiler repairs, and the anonymous gap result. It does not prove universal factual correctness, instructor approval, classroom outcomes, human preference, statistical confidence, paid-model parity, or a universal 99/A result. The adapter remains inactive and Gemma weights are unchanged.
...
**Goal:** make the five-domain frozen baseline artifact-clean as well as grader-green, spend Gemma inference only on knowledge Course Mapper does not already own, and remove unfinished language from the current Scion experience without hiding a real blocker.

**Exact-source lane:** CHN101 Elementary Mandarin I now supplies three canonical lesson facts for each of 15 requested identities, with URL, CC BY-NC-SA license, and attribution carried as one compiler-owned ledger. Instructor facts still outrank the built-in source. The exact route activates only for a matching trusted contract; arbitrary courses continue through normal source-grounded synthesis.

**Inference lane:** public Scion, the browser-local provider, and the production-compatible OpenAI shim project an exact trusted ledger with `modelCalls: 0`. Base Gemma still authors the course skeleton and limited generative surfaces. The fresh Mandarin package uses two actual Gemma generations; the complete five-course matrix uses 30 actual generations. The six-course proof totals 34 with zero retries or model failures, rather than reopening the former 72-call single-course architecture.

**Admission lane:** display pairs and canonical facts are NFKC-normalized before target-language safety comparison, preserving Chinese facts that differ only by full-width punctuation. Direct lesson title/topic identities outrank broad review anchors, word boundaries prevent substring collisions, and the canonical negation example is `我不喜欢苹果。`.

**Artifact lane:** the 99/A matrix was not accepted until representative DOCX files were opened. That inspection found and fixed duplicated assessment weights in World Literature, a severed misconception narrator in Psychology, constructed-response keys that copied learner directions, missing apostrophes in title-case possessives, possessive-plus-article collisions, and a focus label assembled across a conjunction. The answer projection now keeps the relevant scenario sentence and admitted source claim, states a bounded conclusion, and drops prompt echo. The final scan opens every DOCX across all six archives and finds none of the encoded defect patterns.
...
**Exact proof:** Mandarin, World Literature, Psychology, Nutrition, and Astronomy each complete every requested lesson, all ten material sections, and 38/38 export checks at 99/A with zero P0/P1/P2 findings, blockers, or warnings. Physical Geology independently reaches the same gate. Their tested archives contain 127, 117, 125, 117, 101, and 117 extracted files. Console receipts contain no warning or error events, and every ZIP passes physical archive testing. The local release gate passes 434 test files and 5,324 tests, format, lint, production build, locked bundle budgets, and release-history audit.
...
**Release Boundary:** the adapter remains inactive and Gemma weights are unchanged. This release proves bounded software, browser, archive, and encoded package-defect behavior across five frozen courses. It does not establish factual correctness, instructor approval, classroom outcomes, human preference, adapter superiority, paid-model parity, or a universal 99/A result.

## V0.16.68 — Make Every Package Feel Finished

**Goal:** remove every deterministic blocker exposed by the attached Physical Geology package, preserve recovery under browser storage pressure, and present one finished-package export action only after the compiler, grader, and archive agree.

**Production reconstruction:** the V0.16.67 ZIP was structurally valid and passed its physical export checks, yet it graded 74/C. A nutrition classifier used the bare word `mineral` as a domain signal and injected diet-analysis language into Geology. A generic model kind overrode the explicit assessment title `Final (50%)`, leaving the manifest and compiled exam registry inconsistent. Lesson 14 then used source-bound assessment recovery. Section-fragment source ids lost trusted base-entry metadata, and the compact autosave retry still carried the full graph.

**Compiler, source, and persistence lane:** Physical Geology now resolves before Nutrition and uses field, sample, Earth-process, and geologic-interpretation language. Explicit exam titles override contradictory generic kinds at native graph, course IR, registry, and compiler boundaries. Fragment-specific source keys inherit URL, license, and attribution from the exact base bibliography entry. A quota failure falls back to a small recompile-capable course-map snapshot that omits compiled graph, material payloads, and history.

**Premium export contract:** the package panel owns one `Download ZIP` action and confirms `ZIP downloaded`. Review metadata remains inside the archive without branding the package as unfinished. Ready still means the checked course is ready: wording changes do not hide blockers, weaken the grader, or convert an unresolved package to green.

**Exact proof:** the pinned public Gemma base with the adapter explicitly inactive completes 14/14 lessons, 14/14 admitted kernels, 9/9 material families, and 38/38 export checks in 57,418 ms with four model calls and zero retries. The 2,245,613-byte ZIP contains 117 files, extracts cleanly, and grades 99/A with texture 94, citation score 100, and zero P0/P1/P2 findings. No Geology artifact contains the audited nutrition-language contamination.

**Frozen ruler V17:** Astronomy, Mandarin, Nutrition, Psychology, and World Literature each grade 99/A with zero findings on the same pinned base and inactive adapter. The matrix found an evaluator false positive that parsed a standalone Creative Commons attribution paragraph as a reading; the repair excludes only shared attribution metadata and preserves relevance checks for actual encyclopedia/archive source entries. V17 binds the updated transitive grader receipt and inherits no V16 score, adapter result, or promotion decision.

**Lane:** discipline inference, assessment classification, semantic admission, source resolution, deterministic compilation, persistence recovery, grading, readiness, and export are model-neutral and benefit compatible paid-provider output. Browser WebGPU loading, public Gemma download, local caching, and inference recovery remain Scion-specific.

**Release Boundary:** this release proves bounded software, browser, archive, persistence, and encoded package-defect behavior for the reproduced incident and frozen matrix. It does not prove factual correctness, instructor approval, classroom outcomes, human preference, adapter superiority, or a universal 99/A result. Gemma weights are unchanged and the research adapter remains inactive until it earns a held-out cross-domain quality win.

## V0.16.67 — Fix the Package Before Presenting It

**Goal:** make the exact attached Physics incident finish as a clean, downloadable package by repairing every causal compiler and renderer defect before the workspace presents a ready state.

**Production reconstruction:** the V0.16.66 archive contained 123 graded course files and passed physical export verification, yet it graded 74/C with one P0 blocker. A Lesson 5 fallback rubric without explicit lesson identity occupied the third array position and leaked into Lesson 3. The canonical classifier recognized `exam` but not `examination`, so Midterm Examination II became a blocking assignment no-brief handoff. One cumulative exam repeated the same definition-recognition frame twelve times. Prerequisite definitions rendered `X: X` term echoes and one lesson printed the same cited primer twice.

**Compiler and export lane:** lesson-aware slicing resolves explicit lesson identity before positional fallback; standard and fallback rubrics carry `lessonNumber`; native formative checks do not invent rubric obligations; `exam` and `examination` share one classifier; cumulative exams rotate visible framing while retaining the same admitted definition and key; DOCX primer rendering bolds the definition's real lead instead of prepending it and deduplicates identical dependency edges at the document boundary.

**Exact proof:** the rebuilt production bundle completes the same 15-week calculus-based Physics brief in 59.9 seconds with three browser-local model calls. It reaches 15/15 course-map lessons, 15/15 admitted knowledge kernels, 9/9 material families, 38/38 export checks, Grade 99/A, and zero P0/P1/P2 findings. The 2.32 MB downloaded ZIP contains 123 graded course files plus manifest and reports; an independent offline regrade returns 99/A and zero findings. The final DOM contains Ready to export, Verified · Grade A, and Download ZIP, with no blocker, warning, needs-attention, draft-ZIP, failed, or error language.

**Lane:** lesson identity, assessment classification, deterministic compilation, rendered repetition control, grading, readiness, and export are model-neutral and benefit compatible paid-provider output. Browser WebGPU loading, public Gemma download, local caching, and inference recovery remain Scion-specific.

**Release Boundary:** this release proves bounded software, browser, archive, and encoded package-defect behavior for the reproduced incident. It does not prove factual correctness, instructor approval, classroom outcomes, human preference, or a universal 99/A result. Gemma weights are unchanged and the research adapter remains inactive until it earns a frozen held-out multi-domain win.

## V0.16.66 — Make Every Course Finish and Download Cleanly

**Goal:** preserve useful partial model work, finish an incomplete public course map whenever bounded continuation keeps making progress, and prove that the exact failed production brief reaches a physically valid export.

**Production reconstruction:** the V0.16.65 Research Methods log contains three successful browser-local course-map responses. Admission advanced from six lessons to eight and then eleven, but a fixed two-continuation limit stopped the requested twelve-lesson course. The terminal error classified the explicit `11 of 12` quality condition as `unknown` and non-retryable even though the model had never returned a no-progress response.

**Recovery contract:** public Scion budgets at most one bounded continuation per missing lesson, capped at twelve, and terminates after two consecutive no-progress responses. Each accepted lesson remains in the canonical map, continuation output is sliced to the remaining scope, and incomplete `X of Y` maps classify as retryable quality failures. Remote provider routes retain their established two-attempt policy. Captured compact-wire field splices are normalized before visible compilation.

**Compiler audit:** research-methods language stays in its actual discipline: qualitative coding and an optional Python notebook cannot silently convert the course into a programming lab. Weekly assessment cells count as alignment evidence when they contain real assessment language; questionnaire, informed-consent, and citation vocabulary remain legitimate research-method concepts; deterministic FAQ, speaker-note, technical-help, and transfer-task surfaces vary across lessons without changing facts.

**Exact browser and export proof:** the production brief completes 12/12 course-map lessons, 12/12 admitted knowledge kernels, all nine material families, and the full Living Course Compiler at 100% in 89 seconds. The package grades 99/A with texture 93, citations 100, discipline 100, and zero P0/P1/P2 findings. A fresh 1,923,516-byte ZIP contains 99 material files plus the manifest and two reports, passes 38/38 export checks with zero failures or warnings, and extracts without compressed-data errors.

**Release Boundary:** this exact incident is causally repaired and browser/export verified. It is not instructor approval, factual validation, classroom evidence, or proof that every course will score 99/A. The pinned Gemma weights are unchanged and the research adapter remains inactive until a fresh exact-lineage candidate beats base-only Scion on the frozen multi-domain ruler.
