...
**Inference lane:** public Scion, the browser-local provider, and the production-compatible OpenAI shim project an exact trusted ledger with `modelCalls: 0`. Base Gemma still authors the course skeleton and limited generative surfaces. The fresh Mandarin package uses two actual Gemma generations; the complete five-course matrix uses 31 actual generations, including one bounded content retry, with zero model failures. The six-course proof totals 35, rather than reopening the former 72-call single-course architecture.
