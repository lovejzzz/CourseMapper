...
**Status:** exact-QAT smoke training, a timestamp-independent dataset identity with source and split receipts, a schema-v4 frozen-holdout firewall with course-ID relabel protection, a pinned Apple MLX toolchain, dual-seeded ORPO launch, hash-bound plan and completion receipts, schema-v3 learned-adapter provenance through GGUF conversion, deterministic GGUF conversion, an audited one-artifact chain from the exact seeded MLX run through 52.7 MB GGUF conversion and a fresh real Apple-Silicon browser lifecycle, browser activation and rollback, a dual size budget with bounded streaming and atomic adapter installation, one registry-coordinated canary/install/activation/deactivation path with rollback quarantine, frozen paired evaluation, a promotion-independent package identity, semantic factual and production canaries, a provenance-bound Codex comparison and training-preference lane, v2 promotion evidence with recomputable full-review scorecards and isolated order sessions, semantic promotion verification against both base and the pinned paid reference, a current 400-candidate ledger across sixteen course groups, outcome-sealed historical A/B and B/A Codex passes, fail-closed cross-revision analysis with zero approved preferences, a replacement A/B-only first-order clean room pinned to the selectable `gpt-5.5` + `xhigh` launch profile, deterministic explicit-cue answer-key repair, placeholder-option rejection, a judged high-confidence cross-field key-term gate, dual-envelope plaintext-free ingestion, deterministic incomplete-explanation recovery, a two-campaign cross-arm compiler-lift replay, shared key-term semantics, bounded local defect recovery, conservative cross-attempt field retention, Scion-backed advisory Agent chat, one six-stage observable build meter, separate optional human research lanes, a semantic four-profile device protocol, and one real passing Apple-Silicon recovery profile are implemented; a completed same-identity two-order judgment, stable training preferences, a quality adapter, native Scion Agent tool calling, the other three device profiles, and a five-domain adapter win do not exist yet
...
- The current matched-corpus audit deterministically rebuilds 400 neutral atoms across Computer Science, Geology, Music Theory, and UX after excluding World Literature atoms with mismatched course inputs and applying the stronger shared admission gate. Only 92 current atoms remain source-grounded, so the blank historical 128-case successor workbook must not be scored and at least eight new admissible source-grounded cases are needed before a new campaign. The sealed organizer remains bound to its original 437-atom ledger. Both historical completed orders used different Codex identities, so all 128 rows remain analysis-only and research and production datasets stay correctly blocked.
