...
**Dataset identity:** schema-v3 datasets now retain a regular-file receipt for every present source and an explicit missing receipt for absent optional sources. Their canonical identity excludes generation time and binds the verified source bytes, admitted train/validation/test bytes and row counts, evidence and domain distributions, course-group split identities, leakage result, and production/research gate state. Training re-parses every JSONL row and independently checks all bytes, hashes, counts, source receipts, and the canonical identity.

**Pinned trainer:** `evaluation/scion-adapters/training-toolchain-v1.json` pins Python 3.13.3, MLX 0.31.2, MLX-VLM 0.6.3, NumPy 2.5.1, Transformers 5.13.0, Hugging Face Hub 1.22.0, Safetensors 0.8.0, and exact hashes for `mlx_vlm.lora`, the LoRA layer implementation, and the ORPO trainer. The CI-safe contract audit exercises the wrapper without Apple ML dependencies; the live audit imports the installed stack and compares its actual versions and module bytes.
