...
- ChatGPT Developer mode and CSP enforcement are enabled. Two private registrations use the approved public OAuth client, exact ChatGPT callback, S256 PKCE and five authoring scopes. Auth0 records successful login and authorization-code exchange for the correct resource audience, including the fresh registration's attempt at `2026-09-20T05:17:31.764Z`.
- **ChatGPT integration is not yet working end to end.** Its private app metadata contains zero actions, and the account connection is not retained. After the latest successful provider exchange, Cloud Run records no authenticated MCP request. The plugin directory temporarily stalled and later recovered, but the connection failure remained. The root cause of the callback handoff is not established. Initial empty-content discovery probes returning 415 are not evidence that normal JSON MCP calls fail.

Remote writes and application reservations remain disabled. Real ChatGPT course generation, authenticated remote review/application/recovery, second-account live isolation and other target-host acceptance remain pending. These provider, transport and local test results do not establish public availability or a complete teacher workflow.
