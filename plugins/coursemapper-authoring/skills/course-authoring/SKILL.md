...
For an interrupted conversation, locate the existing authorized request and draft, then read cm_v2_get_draft_status before continuing. Distinguish received draft content, validation/preview status and a recorded application receipt. A missing receipt does not prove that local application failed: direct the teacher to the website recovery controls when application is uncertain. Never create a replacement draft or resubmit completed lessons merely because the conversation stopped. Resume only missing work using fresh contracts and current revisions.
